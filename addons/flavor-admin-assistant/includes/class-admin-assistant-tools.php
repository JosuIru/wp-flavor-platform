...
class Chat_IA_Admin_Assistant_Tools {
    ...
    private function tool_obtener_comparativa_periodos($args) {
        ...
        if (!$periodo1['success'] || !$periodo2['success']) {
            return ['success' => false, 'error' => 'Error al obtener datos de períodos'];
        }
    ...
    private function tool_modificar_limite_plazas($args) {
        ...
        if (empty($fecha) || empty($tipo_ticket)) {
            return ['success' => false, 'error' => 'Fecha y tipo de ticket requeridos'];
        }
    ...
    private function tool_bloquear_ticket($args) {
        ...
        if ($ticket['blocked']) {
            return ['success' => false, 'error' => 'El ticket ya está bloqueado'];
        }
    ...
    private function tool_desbloquear_ticket($args) {
        ...
        if (!$ticket['blocked']) {
            return ['success' => false, 'error' => 'El ticket no está bloqueado'];
        }
    ...
    private function tool_obtener_detalle_cliente($args) {
        ...
        if (empty($reservas)) {
            return [
                'success' => false,
                'error' => 'No se encontró cliente con ese email',
            ];
        }
    ...
    private function tool_exportar_datos_csv($args) {
        ...
        switch ($tipo_datos) {
            ...
            default:
                return ['success' => false, 'error' => 'Tipo de datos no válido'];
        }
    ...
    private function tool_asignar_estado_calendario($args) {
        ...
        if (empty($fecha) && (empty($fecha_inicio) || empty($fecha_fin))) {
            return [
                'success' => false,
                'error' => 'Debes especificar una fecha única o un rango (fecha_inicio y fecha_fin)',
            ];
        }
    ...
    private function tool_resetear_calendario($args) {
        ...
        if (!$confirmar) {
            return [
                'success' => false,
                'error' => 'Debes confirmar el reseteo estableciendo confirmar=true',
            ];
        }
    ...
    private function tool_eliminar_estado_calendario($args) {
        ...
        if (!$confirmar) {
            return ['success' => false, 'error' => 'Debes confirmar la eliminación'];
        }
    ...
    private function tool_crear_tipo_ticket($args) {
        ...
        if ($plazas < 0) {
            return ['success' => false, 'error' => 'Las plazas no pueden ser negativas'];
        }
    ...
    private function tool_restaurar_backup($args) {
        ...
        if (empty($backup_id)) {
            return ['success' => false, 'error' => 'backup_id requerido'];
        }
    ...
    private function tool_importar_estados_texto($args) {
        ...
        if (empty($importaciones)) {
            return [
                'success' => false,
                'error' => 'No se encontraron entradas validas',
                'errores' => $errores,
            ];
        }
    ...
    private function tool_actualizar_precio_ticket($args) {
        ...
        if (empty($ticket_slug)) {
            return ['success' => false, 'error' => 'ticket_slug requerido'];
        }
    ...
    private function tool_obtener_comparativa_rapida($args) {
        ...
        switch ($tipo) {
            ...
            default:
                return ['success' => false, 'error' => 'Tipo de comparativa no valido'];
        }
