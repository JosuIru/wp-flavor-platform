...
class Flavor_Network_Admin {
    ...
    public function render_main_page() {
        ...
        $tabs = [
            'dashboard'       => __('Dashboard', FLAVOR_PLATFORM_TEXT_DOMAIN),
            'mi-nodo'         => __('Mi Nodo', FLAVOR_PLATFORM_TEXT_DOMAIN),
    ...
    private function render_tab_modulos($nodo_local) {
        ...
        <div class="flavor-network-modulos">
            ...
            <div class="notice notice-info" style="margin: 15px 0;">
                <?php if ($nodo_local): ?>
                    <p>
                        <strong><?php _e('Nodo local:', FLAVOR_PLATFORM_TEXT_DOMAIN); ?></strong>
                        <?php echo esc_html($nodo_local->nombre ?? __('Sin nombre', FLAVOR_PLATFORM_TEXT_DOMAIN)); ?>
                        <?php if (!empty($modulos_activos)): ?>
                            <br>
                            <strong><?php _e('Módulos activos:', FLAVOR_PLATFORM_TEXT_DOMAIN); ?></strong>
                            <?php echo esc_html(implode(', ', $modulos_activos)); ?>
                        <?php else: ?>
                            <br>
                            <strong><?php _e('Módulos activos:', FLAVOR_PLATFORM_TEXT_DOMAIN); ?></strong>
                            <?php _e('Ninguno', FLAVOR_PLATFORM_TEXT_DOMAIN); ?>
                        <?php endif; ?>
                        <?php if (!empty($nodo_local->updated_at)): ?>
                            <br>
                            <strong><?php _e('Última actualización:', FLAVOR_PLATFORM_TEXT_DOMAIN); ?></strong>
                            <?php echo esc_html($nodo_local->updated_at); ?>
                        <?php endif; ?>
                    </p>
                <?php else: ?>
                    <p><?php _e('Primero configura tu nodo local para poder activar módulos.', FLAVOR_PLATFORM_TEXT_DOMAIN); ?></p>
                <?php endif; ?>
            </div>
