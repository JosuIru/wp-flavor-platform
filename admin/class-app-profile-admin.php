...
class Flavor_App_Profile_Admin {
    ...
    private function renderizar_modal_preview_plantilla() {
        // El modal se renderiza vacio y se llena via AJAX
        // Esto es un placeholder para Alpine.js
        ?>
        <template x-if="plantillaSeleccionadaData">
            <div class="flavor-template-modal"
                ...
                <div class="flavor-template-modal-content" @click.stop>

                    <!-- Header -->
                    <header class="flavor-template-modal-header">
                        <div class="flavor-template-modal-header-info">
                            <div class="flavor-template-modal-icon"
                                 :style="{ background: (plantillaSeleccionadaData.color || '#2271b1') + '15' }">
                                <span class="dashicons"
                                      :class="plantillaSeleccionadaData.icono || 'dashicons-admin-generic'"
                                      :style="{ color: plantillaSeleccionadaData.color || '#2271b1' }"></span>
                            </div>
                            <div>
                                <h2 id="modal-titulo-plantilla" x-text="plantillaSeleccionadaData.nombre || 'Plantilla'"></h2>
                                <p class="descripcion-corta" x-text="plantillaSeleccionadaData.descripcion || ''"></p>
                            </div>
                        </div>
                        <button type="button"
                    ...
                    <div class="flavor-template-preview">
                        ...
                        <section class="flavor-template-section">
                            ...
                            <ul class="flavor-template-modules"
                                x-show="(plantillaSeleccionadaData.modulos_requeridos || []).length > 0 || (plantillaSeleccionadaData.modulos_opcionales || []).length > 0">
                                <template x-for="modulo in (plantillaSeleccionadaData.modulos_requeridos || [])" :key="'req-' + modulo">
                                    <li class="modulo-item requerido">
                                        <label>
                                            <input type="checkbox" checked disabled>
                                            <span class="modulo-nombre" x-text="modulo.replace(/_/g, ' ').replace(/^./, c => c.toUpperCase())"></span>
                                            <span class="modulo-badge requerido"><?php _e('Requerido', 'flavor-chat-ia'); ?></span>
                                        </label>
                                    </li>
                                </template>

                                <template x-for="modulo in (plantillaSeleccionadaData.modulos_opcionales || [])" :key="'opt-' + modulo">
                            ...
                            <p class="flavor-template-empty"
                               x-show="(plantillaSeleccionadaData.modulos_requeridos || []).length === 0 && (plantillaSeleccionadaData.modulos_opcionales || []).length === 0">
                                <?php _e('Esta plantilla no incluye modulos predefinidos.', 'flavor-chat-ia'); ?>
                            </p>
                        </section>

                        <!-- Seccion de Paginas -->
                        <section class="flavor-template-section" x-show="plantillaSeleccionadaData.paginas && Object.keys(plantillaSeleccionadaData.paginas).length > 0">
                            <h3>
                            ...
                            <ul class="flavor-template-pages">
                                <template x-for="(pagina, id) in (plantillaSeleccionadaData.paginas || {})" :key="id">
                                    <li class="pagina-item">
                                        <span class="dashicons" :class="pagina.icono || 'dashicons-media-document'"></span>
                                        <span class="pagina-titulo" x-text="pagina.titulo || id"></span>
                                        <span class="pagina-slug" x-text="'/' + (pagina.slug || id) + '/'"></span>
                                    </li>
                                </template>
                            </ul>
                        </section>
                        ...
                        <section class="flavor-template-section" x-show="plantillaSeleccionadaData.landing && plantillaSeleccionadaData.landing.secciones">
                            <h3>
                            ...
                            <div class="flavor-template-landing-preview">
                                <template x-for="(seccion, id) in (plantillaSeleccionadaData.landing?.secciones || {})" :key="id">
                                    <div class="landing-seccion-mini" :title="seccion.titulo || id">
                                        <span class="dashicons dashicons-layout"></span>
                                        <span class="seccion-nombre" x-text="seccion.titulo || id.replace(/_/g, ' ').replace(/^./, c => c.toUpperCase())"></span>
                                    </div>
                                </template>
                            </div>
                        </section>
                        ...
                        <section class="flavor-template-section flavor-template-demo-option">
                            <label class="flavor-demo-checkbox">
                                <input type="checkbox" x-model="cargarDatosDemo" value="1">
                                <span class="checkbox-label">
                                    <strong><?php _e('Cargar datos de demostracion', 'flavor-chat-ia'); ?></strong>
                                </span>
                            </label>
                            <p class="description">
                                <?php _e('Incluye contenido de ejemplo para probar rapidamente la plantilla. Puedes eliminar estos datos mas tarde.', 'flavor-chat-ia'); ?>
                            </p>
                        </section>
