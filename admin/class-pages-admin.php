...
class Flavor_Pages_Admin {
    ...
    public function render_admin_page() {
        ...
        ?>
        <div class="wrap">
            <h1><?php _e('Crear Páginas de Módulos', FLAVOR_PLATFORM_TEXT_DOMAIN); ?></h1>

            <div class="notice notice-info">
                <p><strong><?php _e('Sistema de Páginas Automáticas', FLAVOR_PLATFORM_TEXT_DOMAIN); ?></strong></p>
                <p><?php _e('Esta pantalla combina la foto del creador legacy basado en listado fijo con el estado actual de la migración modular V3.', FLAVOR_PLATFORM_TEXT_DOMAIN); ?></p>
            </div>

            <?php if (isset($_GET['created'])): ?>
                <div class="notice notice-success is-dismissible">
                    <p><strong><?php _e('Páginas creadas exitosamente!', FLAVOR_PLATFORM_TEXT_DOMAIN); ?></strong></p>
                    <p><?php printf(__('Se crearon %d páginas. Ya puedes navegar a las URLs de los formularios.', FLAVOR_PLATFORM_TEXT_DOMAIN), intval($_GET['created'])); ?></p>
                </div>
            <?php endif; ?>

            <?php if (isset($_GET['deleted'])): ?>
                <div class="notice notice-warning is-dismissible">
                    <p><strong><?php _e('Páginas eliminadas', FLAVOR_PLATFORM_TEXT_DOMAIN); ?></strong></p>
                    <p><?php printf(__('Se eliminaron %d páginas.', FLAVOR_PLATFORM_TEXT_DOMAIN), intval($_GET['deleted'])); ?></p>
                </div>
            <?php endif; ?>

            <?php if ($estado_v3['available']): ?>
                <div class="card" style="max-width: 1100px; margin-top: 20px;">
                    <h2><?php _e('Estado Modular V3', FLAVOR_PLATFORM_TEXT_DOMAIN); ?></h2>
                    <p style="max-width: 900px;">
                        <?php _e('La arquitectura actual del plugin ya permite que los módulos definan sus propias páginas con `get_pages_definition()`. Esta lectura refleja qué parte del ecosistema ya usa ese sistema.', FLAVOR_PLATFORM_TEXT_DOMAIN); ?>
                    </p>
                    ...
                    <div style="display: grid; grid-template-columns: repeat(4, 1fr); gap: 20px; margin: 20px 0;">
                        <div style="text-align: center; padding: 20px; background: var(--flavor-bg, #f0f0f1); border-radius: var(--flavor-radius-md, 8px);">
                            ...
                            <div style="color: var(--flavor-text-secondary, #666);"><?php _e('Módulos cargados', FLAVOR_PLATFORM_TEXT_DOMAIN); ?></div>
                        </div>
                        <div style="text-align: center; padding: 20px; background: var(--flavor-success-light, #d1e7dd); border-radius: var(--flavor-radius-md, 8px);">
                            ...
                            <div style="color: var(--flavor-success, #0f5132);"><?php _e('Módulos con páginas V3', FLAVOR_PLATFORM_TEXT_DOMAIN); ?></div>
                        </div>
                        <div style="text-align: center; padding: 20px; background: #e8f1ff; border-radius: var(--flavor-radius-md, 8px);">
                            ...
                            <div style="color: #135e96;"><?php _e('Páginas definidas en V3', FLAVOR_PLATFORM_TEXT_DOMAIN); ?></div>
                        </div>
                        <div style="text-align: center; padding: 20px; background: #fff7e6; border-radius: var(--flavor-radius-md, 8px);">
                            ...
                            <div style="color: #8a5300;"><?php _e('Páginas creadas con owner V3', FLAVOR_PLATFORM_TEXT_DOMAIN); ?></div>
                        </div>
                    </div>

                    <div style="margin: 18px 0 24px;">
                        <strong><?php _e('Cobertura V3', FLAVOR_PLATFORM_TEXT_DOMAIN); ?>:</strong>
                    ...
                    <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 20px;">
                        <div style="background: #fff; padding: 16px; border-left: 4px solid var(--flavor-success, #0f5132);">
                            <h3 style="margin-top: 0;"><?php printf(__('Módulos ya migrados (%d)', FLAVOR_PLATFORM_TEXT_DOMAIN), $estado_v3['migrated_modules']); ?></h3>
                                ...
                                <ul style="margin-bottom: 0;">
                                    <?php foreach ($estado_v3['migrated_preview'] as $module_info): ?>
                                        <li>
                                            ...
                                            <?php printf(__('(%d páginas)', FLAVOR_PLATFORM_TEXT_DOMAIN), intval($module_info['page_count'])); ?>
                                        </li>
                                    <?php endforeach; ?>
                                </ul>
                            <?php else: ?>
                                <p style="margin-bottom: 0;"><?php _e('Todavía no hay módulos con definiciones V3 activas.', FLAVOR_PLATFORM_TEXT_DOMAIN); ?></p>
                            <?php endif; ?>
                        </div>

                        <div style="background: #fff; padding: 16px; border-left: 4px solid #dba617;">
                            <h3 style="margin-top: 0;"><?php printf(__('Módulos aún sin páginas V3 (%d)', FLAVOR_PLATFORM_TEXT_DOMAIN), $estado_v3['pending_modules']); ?></h3>
                            ...
                            <?php else: ?>
                                <p style="margin-bottom: 0;"><?php _e('Todos los módulos cargados exponen ya páginas V3.', FLAVOR_PLATFORM_TEXT_DOMAIN); ?></p>
                            <?php endif; ?>
                        </div>
                    </div>
                    ...
                    <p style="margin-top: 20px;">
                        <a class="button" href="<?php echo esc_url(admin_url('admin.php?page=flavor-systems-panel')); ?>">
                            <?php _e('Abrir Panel de Sistemas', FLAVOR_PLATFORM_TEXT_DOMAIN); ?>
                        </a>
                    </p>
                </div>
            <?php else: ?>
                <div class="notice notice-warning" style="margin-top: 20px;">
                    <p><?php _e('No se ha podido obtener el estado de migración V3 en esta carga de admin. Se muestra solo el creador legacy.', FLAVOR_PLATFORM_TEXT_DOMAIN); ?></p>
                </div>
            <?php endif; ?>

            <!-- Estado actual -->
            <div class="card" style="max-width: 800px; margin-top: 20px;">
                <h2><?php _e('Estado Legacy', FLAVOR_PLATFORM_TEXT_DOMAIN); ?></h2>
                <p><?php _e('Este bloque sigue mostrando el catálogo histórico de páginas que el creador clásico sabe generar de forma masiva.', FLAVOR_PLATFORM_TEXT_DOMAIN); ?></p>
                ...
                <div style="display: grid; grid-template-columns: repeat(3, 1fr); gap: 20px; margin: 20px 0;">
                    <div style="text-align: center; padding: 20px; background: var(--flavor-bg, #f0f0f1); border-radius: var(--flavor-radius-md, 8px);">
                        ...
                        <div style="color: var(--flavor-text-secondary, #666);"><?php _e('Total de Páginas', FLAVOR_PLATFORM_TEXT_DOMAIN); ?></div>
                    </div>
                    <div style="text-align: center; padding: 20px; background: var(--flavor-success-light, #d1e7dd); border-radius: var(--flavor-radius-md, 8px);">
                        ...
                        <div style="color: var(--flavor-success, #0f5132);"><?php _e('Creadas', FLAVOR_PLATFORM_TEXT_DOMAIN); ?></div>
                    </div>
                    <div style="text-align: center; padding: 20px; background: <?php echo $conteo_faltantes > 0 ? 'var(--flavor-error-light, #f8d7da)' : 'var(--flavor-success-light, #d1e7dd)'; ?>; border-radius: var(--flavor-radius-md, 8px);">
                        ...
                        <div style="color: <?php echo $conteo_faltantes > 0 ? 'var(--flavor-error, #842029)' : 'var(--flavor-success, #0f5132)'; ?>;"><?php _e('Faltantes', FLAVOR_PLATFORM_TEXT_DOMAIN); ?></div>
                    </div>
                </div>

                <?php if ($conteo_faltantes > 0): ?>
                    <h3><?php printf(__('Páginas Faltantes (%d)', FLAVOR_PLATFORM_TEXT_DOMAIN), $conteo_faltantes); ?></h3>
                    ...
                    <form method="post" action="<?php echo esc_url(admin_url('admin-post.php')); ?>" style="margin-top: 20px;">
                        ...
                        <button type="submit" class="button button-primary button-hero">
                            <?php printf(__('Crear %d Páginas Faltantes', FLAVOR_PLATFORM_TEXT_DOMAIN), $conteo_faltantes); ?>
                        </button>
                    </form>
                <?php else: ?>
                    <div style="background: var(--flavor-success-light, #d1e7dd); color: var(--flavor-success, #0f5132); padding: 15px; border-radius: var(--flavor-radius-md, 8px); margin-top: 20px;">
                        <strong><?php _e('Todas las páginas están creadas', FLAVOR_PLATFORM_TEXT_DOMAIN); ?></strong>
                        <p style="margin: 10px 0 0 0;"><?php _e('Todos los formularios de módulos tienen sus páginas correspondientes.', FLAVOR_PLATFORM_TEXT_DOMAIN); ?></p>
                    </div>
                <?php endif; ?>
            </div>
            ...
            <?php if ($conteo_creadas > 0): ?>
                <div class="card" style="max-width: 800px; margin-top: 20px;">
                    <h2><?php printf(__('Páginas Creadas (%d)', FLAVOR_PLATFORM_TEXT_DOMAIN), $conteo_creadas); ?></h2>
                        ...
                        <tbody>
                            <?php foreach ($estado_paginas['exists'] as $pagina_existente): ?>
                                <tr>
                                    ...
                                    <td><a href="<?php echo esc_url($pagina_existente['url']); ?>" target="_blank"><?php _e('Ver página', FLAVOR_PLATFORM_TEXT_DOMAIN); ?> &rarr;</a></td>
                                    <td><a href="<?php echo esc_url(get_edit_post_link(url_to_postid($pagina_existente['url']))); ?>"><?php _e('Editar', FLAVOR_PLATFORM_TEXT_DOMAIN); ?></a></td>
                                </tr>
                            <?php endforeach; ?>
                        </tbody>
                    </table>
                ...
                <div class="card" style="max-width: 800px; margin-top: 20px; border-left: 4px solid var(--flavor-error, #d63638);">
                    <h2 style="color: var(--flavor-error, #d63638);"><?php _e('Zona Peligrosa', FLAVOR_PLATFORM_TEXT_DOMAIN); ?></h2>
                    <p><?php _e('Elimina todas las páginas creadas automáticamente.', FLAVOR_PLATFORM_TEXT_DOMAIN); ?> <strong><?php _e('Esta acción no se puede deshacer.', FLAVOR_PLATFORM_TEXT_DOMAIN); ?></strong></p>
                    <form method="post" action="<?php echo esc_url(admin_url('admin-post.php')); ?>" onsubmit="return confirm('<?php esc_attr_e('¿Estás seguro de que quieres eliminar TODAS las páginas creadas? Esta acción no se puede deshacer.', FLAVOR_PLATFORM_TEXT_DOMAIN); ?>');">
                        <input type="hidden" name="action" value="flavor_delete_pages">
                        <?php wp_nonce_field('flavor_delete_pages_nonce'); ?>
                        <button type="submit" class="button button-secondary"><?php _e('Eliminar Todas las Páginas', FLAVOR_PLATFORM_TEXT_DOMAIN); ?></button>
                    </form>
                </div>
            <?php endif; ?>
        </div>
