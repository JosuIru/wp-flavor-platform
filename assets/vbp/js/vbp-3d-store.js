...
(function() {
    ...
    function decodePayload(encoded) {
        ...
        try {
            ...
            console.warn('[VBP 3D] Error decodificando payload:', error);
            return null;
        }
    }

    function applySceneObjects(scene, objects) {
        if (!scene || !Array.isArray(objects)) {
            return;
        }

        objects.forEach(function(objectConfig) {
            if (!objectConfig || !objectConfig.id) {
                return;
            }

            scene.addObject(objectConfig.id, objectConfig);
        });
    }
    ...
    function bootstrapRuntimeScenes() {
        ...
        var runtimeNodes = document.querySelectorAll('[data-vbp-3d-scene="1"]');

        runtimeNodes.forEach(function(node) {
            var sceneId = node.getAttribute('data-vbp-element-id');
            ...
            node.setAttribute('id', sceneId);
            Array.from(node.querySelectorAll('canvas, .vbp-3d-runtime-error')).forEach(function(child) {
                child.remove();
            });
            ...
            scene.init().then(function(initializedScene) {
                ...
                console.error('[VBP 3D] Error inicializando escena:', error);
                node.setAttribute('data-vbp-runtime-ready', '0');
                node.insertAdjacentHTML('beforeend', '<div class="vbp-3d-runtime-error" style="position:absolute;inset:0;display:flex;align-items:center;justify-content:center;color:#fecaca;background:rgba(127,29,29,0.28);z-index:3;font-size:13px;padding:16px;text-align:center;">No se pudo cargar la escena 3D</div>');
            });
        });
    }
