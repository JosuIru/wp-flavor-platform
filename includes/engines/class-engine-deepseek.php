<?php
/**
 * Motor de IA para DeepSeek
 *
 * DeepSeek ofrece modelos potentes con tier gratuito (~500K tokens/día)
 *
 * @package ChatIAAddon
 */
...
class Chat_IA_Engine_DeepSeek extends Chat_IA_Engine_Base {
    ...
    public function get_description() {
        return __('DeepSeek ofrece modelos potentes con precio muy bajo. Tier gratuito disponible (~500K tokens/día).', 'chat-ia-addon');
    }
    ...
    public function send_message($messages, $system_prompt, $tools = []) {
        ...
        $model = $this->get_config('model', 'deepseek-chat');
        $max_tokens = (int) $this->get_config('max_tokens', 1000);

        if (empty($api_key)) {
            return [
                'success' => false,
                'error' => __('API key de DeepSeek no configurada', FLAVOR_PLATFORM_TEXT_DOMAIN),
    ...
    /**
     * Envía mensaje con streaming via API DeepSeek
     */
    public function send_message_stream($messages, $system_prompt, $callback) {
        ...
        if (empty($api_key)) {
            return ['success' => false, 'error' => __('API key de DeepSeek no configurada', FLAVOR_PLATFORM_TEXT_DOMAIN), 'error_code' => 'no_api_key'];
        }
    ...
    public function get_settings_fields() {
        return [
            'api_key' => [
                ...
                'description' => sprintf(
                    __('Obtén tu API key gratis en %s', 'chat-ia-addon'),
    ...
    /**
     * Formatea mensajes al formato OpenAI/DeepSeek
     */
