...
class Chat_IA_Engine_OpenAI extends Chat_IA_Engine_Base {
    ...
    public function send_message($messages, $system_prompt, $tools = []) {
        ...
        if (empty($api_key)) {
            return [
                'success' => false,
                'error' => __('API key no configurada', FLAVOR_PLATFORM_TEXT_DOMAIN),
    ...
    public function send_message_stream($messages, $system_prompt, $callback) {
        ...
        if (empty($api_key)) {
            return ['success' => false, 'error' => __('API key no configurada', FLAVOR_PLATFORM_TEXT_DOMAIN), 'error_code' => 'no_api_key'];
        }
