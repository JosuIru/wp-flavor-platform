...
<section class="flavor-campanias-calendario">
    <header>
        <h2><?php esc_html_e('Calendario de acciones', FLAVOR_PLATFORM_TEXT_DOMAIN); ?></h2>
        <form method="get" aria-label="<?php echo esc_attr__('Formulario de filtro del calendario de campanias', FLAVOR_PLATFORM_TEXT_DOMAIN); ?>" style="display:flex;gap:0.5rem;align-items:flex-end;flex-wrap:wrap;">
            <p>
                <label for="cal_mes"><?php esc_html_e('Mes', FLAVOR_PLATFORM_TEXT_DOMAIN); ?></label><br>
                <input id="cal_mes" type="number" name="mes" min="1" max="12" value="<?php echo esc_attr($mes); ?>">
            </p>
            <p>
                <label for="cal_anio"><?php esc_html_e('Anio', FLAVOR_PLATFORM_TEXT_DOMAIN); ?></label><br>
    ...
    <?php if (empty($acciones_por_dia)): ?>
        <p><?php esc_html_e('No hay acciones registradas para este periodo.', FLAVOR_PLATFORM_TEXT_DOMAIN); ?></p>
    <?php else: ?>
