        ...
        <div class="flavor-campania-meta">
            <?php if ($campania->creador): ?>
                <span>
                    ...
                    <?php
                    printf(
                        esc_html__('Organiza: %s', FLAVOR_PLATFORM_TEXT_DOMAIN),
    ...
    <div class="flavor-campania-body">
        ...
        <?php if ($campania->objetivo_descripcion): ?>
            <div style="margin-top: 2rem; padding: 1.5rem; background: #fef2f2; border-radius: 8px;">
                <h3 style="margin: 0 0 0.5rem; color: #991b1b;"><?php esc_html_e('Objetivo', FLAVOR_PLATFORM_TEXT_DOMAIN); ?></h3>
        ...
        <?php if ($campania->tipo === 'recogida_firmas'): ?>
            <div style="margin-top: 2rem;">
                <h3><?php esc_html_e('Firmas recogidas', FLAVOR_PLATFORM_TEXT_DOMAIN); ?></h3>
                ...
                <div class="flavor-firma-form" style="margin-top: 2rem;">
                    <h3><?php esc_html_e('Firma esta campaña', FLAVOR_PLATFORM_TEXT_DOMAIN); ?></h3>
                    <form method="post" class="flavor-firma-detalle-form" novalidate aria-label="<?php echo esc_attr__('Formulario para firmar campaña desde el detalle', FLAVOR_PLATFORM_TEXT_DOMAIN); ?>">
                        ...
                        <div class="flavor-form-group">
                            <label for="firma_nombre"><?php esc_html_e('Nombre completo', FLAVOR_PLATFORM_TEXT_DOMAIN); ?> *</label>
                        ...
                        <div class="flavor-form-group">
                            <label for="firma_email"><?php esc_html_e('Email', FLAVOR_PLATFORM_TEXT_DOMAIN); ?> *</label>
                        ...
                        <div class="flavor-form-group">
                            <label for="firma_localidad"><?php esc_html_e('Localidad', FLAVOR_PLATFORM_TEXT_DOMAIN); ?></label>
                            <input type="text" id="firma_localidad" name="localidad">
                        </div>

                        <div class="flavor-form-group">
                            <label for="firma_comentario"><?php esc_html_e('Comentario (opcional)', FLAVOR_PLATFORM_TEXT_DOMAIN); ?></label>
                        ...
                        <button type="submit" class="flavor-btn flavor-btn-primary flavor-btn-block">
                            ...
                            <?php esc_html_e('Firmar campaña', FLAVOR_PLATFORM_TEXT_DOMAIN); ?>
                        </button>
        ...
        <?php if (!empty($campania->acciones)): ?>
            <div class="flavor-campania-acciones-lista">
                <h3><?php esc_html_e('Próximas acciones', FLAVOR_PLATFORM_TEXT_DOMAIN); ?></h3>
        ...
        <?php if (!empty($campania->participantes)): ?>
            <div style="margin-top: 2rem;">
                <h3>
                    <?php
                    printf(
                        esc_html__('%s participantes', FLAVOR_PLATFORM_TEXT_DOMAIN),
            ...
            <?php if (is_user_logged_in()): ?>
                <?php if ($es_participante): ?>
                    <span class="flavor-estado flavor-estado-activa">
                        <?php
                        printf(
                            esc_html__('Participas como %s', FLAVOR_PLATFORM_TEXT_DOMAIN),
                    ...
                    <?php if ($rol_usuario !== 'organizador'): ?>
                        <button class="flavor-btn flavor-btn-secondary flavor-btn-abandonar" data-campania-id="<?php echo esc_attr($campania->id); ?>">
                            <?php esc_html_e('Abandonar campaña', FLAVOR_PLATFORM_TEXT_DOMAIN); ?>
                        </button>
                    <?php endif; ?>
                <?php else: ?>
                    <button class="flavor-btn flavor-btn-primary flavor-btn-participar" data-campania-id="<?php echo esc_attr($campania->id); ?>">
                        <span class="dashicons dashicons-groups"></span>
                        <?php esc_html_e('Unirme a la campaña', FLAVOR_PLATFORM_TEXT_DOMAIN); ?>
                    </button>
                <?php endif; ?>
            <?php else: ?>
                <p>
                    ...
                    printf(
                        wp_kses(
                            __('%1$sInicia sesión%2$s para participar en esta campaña.', FLAVOR_PLATFORM_TEXT_DOMAIN),
                            ['a' => ['href' => []]]
                        ),
        ...
        <?php if (!empty($campania->actualizaciones)): ?>
            <div style="margin-top: 2rem;">
                <h3><?php esc_html_e('Actualizaciones', FLAVOR_PLATFORM_TEXT_DOMAIN); ?></h3>
        ...
        try {
            ...
            if (!json.success) {
                if (statusEl) {
                    statusEl.textContent = (json.data && json.data.error)
                        ? json.data.error
                        : '<?php echo esc_js(__('No se pudo registrar la firma.', FLAVOR_PLATFORM_TEXT_DOMAIN)); ?>';
                }
                return;
            }
            ...
            if (statusEl) {
                statusEl.textContent = (json.data && json.data.mensaje)
                    ? json.data.mensaje
                    : '<?php echo esc_js(__('Firma registrada.', FLAVOR_PLATFORM_TEXT_DOMAIN)); ?>';
            }
            form.reset();
        } catch (error) {
            if (statusEl) {
