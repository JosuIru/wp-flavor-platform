...
$tipos_campania = [
    ...
    'concentracion' => __('Concentración', FLAVOR_PLATFORM_TEXT_DOMAIN),
    'boicot' => __('Boicot', FLAVOR_PLATFORM_TEXT_DOMAIN),
    'denuncia_publica' => __('Denuncia pública', FLAVOR_PLATFORM_TEXT_DOMAIN),
    'sensibilizacion' => __('Sensibilización', FLAVOR_PLATFORM_TEXT_DOMAIN),
    'accion_legal' => __('Acción legal', FLAVOR_PLATFORM_TEXT_DOMAIN),
    'otra' => __('Otra', FLAVOR_PLATFORM_TEXT_DOMAIN),
];
?>

<div class="flavor-campanias-wrapper">
    <form method="get" class="flavor-campanias-filtros" aria-label="<?php echo esc_attr__('Formulario de filtros de campañas', FLAVOR_PLATFORM_TEXT_DOMAIN); ?>" style="margin-bottom: 1.5rem; display: flex; gap: 1rem; flex-wrap: wrap; align-items: end;">
        <?php if (isset($_GET['page'])): ?>
            <input type="hidden" name="page" value="<?php echo esc_attr(sanitize_text_field(wp_unslash($_GET['page']))); ?>">
        <?php endif; ?>
        <div>
            <label for="flavor-campanias-tipo" class="screen-reader-text"><?php esc_html_e('Filtrar por tipo', FLAVOR_PLATFORM_TEXT_DOMAIN); ?></label>
        ...
        <div>
            <label for="flavor-campanias-estado" class="screen-reader-text"><?php esc_html_e('Filtrar por estado', FLAVOR_PLATFORM_TEXT_DOMAIN); ?></label>
            <select id="flavor-campanias-estado" name="estado" class="flavor-campanias-filtro flavor-filtro-estado">
                <option value="activa" <?php selected($estado_filtro, 'activa'); ?>><?php esc_html_e('Activas', FLAVOR_PLATFORM_TEXT_DOMAIN); ?></option>
                <option value="planificada" <?php selected($estado_filtro, 'planificada'); ?>><?php esc_html_e('Planificadas', FLAVOR_PLATFORM_TEXT_DOMAIN); ?></option>
                <option value="completada" <?php selected($estado_filtro, 'completada'); ?>><?php esc_html_e('Completadas', FLAVOR_PLATFORM_TEXT_DOMAIN); ?></option>
                <option value=""><?php esc_html_e('Todas', FLAVOR_PLATFORM_TEXT_DOMAIN); ?></option>
            </select>
        </div>
        <button type="submit" class="button"><?php esc_html_e('Aplicar filtros', FLAVOR_PLATFORM_TEXT_DOMAIN); ?></button>
    </form>
    ...
    <?php if (empty($campanias)): ?>
        <div class="flavor-empty-state" style="text-align: center; padding: 3rem;">
            ...
            <p style="color: #6b7280; margin-top: 1rem;"><?php esc_html_e('No hay campañas que mostrar.', FLAVOR_PLATFORM_TEXT_DOMAIN); ?></p>
            <?php if (is_user_logged_in()): ?>
                <a href="<?php echo esc_url(home_url('/campanias/crear/')); ?>" class="flavor-btn flavor-btn-primary">
                    <?php esc_html_e('Crear nueva campaña', FLAVOR_PLATFORM_TEXT_DOMAIN); ?>
                </a>
            <?php endif; ?>
        </div>
    <?php else: ?>
                            ...
                            <div class="flavor-firmas-progress">
                                ...
                                <div class="flavor-firmas-count">
                                    <span class="flavor-firmas-actual">
                                        <?php
                                        printf(
                                            esc_html__('%s firmas', FLAVOR_PLATFORM_TEXT_DOMAIN),
                                    ...
                                    <span class="flavor-firmas-objetivo" data-objetivo="<?php echo esc_attr($campania->objetivo_firmas); ?>">
                                        <?php
                                        printf(
                                            esc_html__('Objetivo: %s', FLAVOR_PLATFORM_TEXT_DOMAIN),
