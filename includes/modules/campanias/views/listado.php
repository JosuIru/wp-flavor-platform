...
$tipos_campania = [
    ...
    'concentracion' => __('Concentración', 'flavor-platform'),
    'boicot' => __('Boicot', 'flavor-platform'),
    'denuncia_publica' => __('Denuncia pública', 'flavor-platform'),
    'sensibilizacion' => __('Sensibilización', 'flavor-platform'),
    'accion_legal' => __('Acción legal', 'flavor-platform'),
    'otra' => __('Otra', 'flavor-platform'),
];
?>

<div class="flavor-campanias-wrapper">
    <form method="get" class="flavor-campanias-filtros" aria-label="<?php echo esc_attr__('Formulario de filtros de campañas', 'flavor-platform'); ?>" style="margin-bottom: 1.5rem; display: flex; gap: 1rem; flex-wrap: wrap; align-items: end;">
        <?php if (isset($_GET['page'])): ?>
            <input type="hidden" name="page" value="<?php echo esc_attr(sanitize_text_field(wp_unslash($_GET['page']))); ?>">
        <?php endif; ?>
        <div>
            <label for="flavor-campanias-tipo" class="screen-reader-text"><?php esc_html_e('Filtrar por tipo', 'flavor-platform'); ?></label>
        ...
        <div>
            <label for="flavor-campanias-estado" class="screen-reader-text"><?php esc_html_e('Filtrar por estado', 'flavor-platform'); ?></label>
            <select id="flavor-campanias-estado" name="estado" class="flavor-campanias-filtro flavor-filtro-estado">
                <option value="activa" <?php selected($estado_filtro, 'activa'); ?>><?php esc_html_e('Activas', 'flavor-platform'); ?></option>
                <option value="planificada" <?php selected($estado_filtro, 'planificada'); ?>><?php esc_html_e('Planificadas', 'flavor-platform'); ?></option>
                <option value="completada" <?php selected($estado_filtro, 'completada'); ?>><?php esc_html_e('Completadas', 'flavor-platform'); ?></option>
                <option value=""><?php esc_html_e('Todas', 'flavor-platform'); ?></option>
            </select>
        </div>
        <button type="submit" class="button"><?php esc_html_e('Aplicar filtros', 'flavor-platform'); ?></button>
    </form>
    ...
    <?php if (empty($campanias)): ?>
        <div class="flavor-empty-state" style="text-align: center; padding: 3rem;">
            ...
            <p style="color: #6b7280; margin-top: 1rem;"><?php esc_html_e('No hay campañas que mostrar.', 'flavor-platform'); ?></p>
            <?php if (is_user_logged_in()): ?>
                <a href="<?php echo esc_url(home_url('/campanias/crear/')); ?>" class="flavor-btn flavor-btn-primary">
                    <?php esc_html_e('Crear nueva campaña', 'flavor-platform'); ?>
                </a>
            <?php endif; ?>
        </div>
    <?php else: ?>
