...
class Flavor_Circulos_Cuidados_Widget extends Flavor_Dashboard_Widget_Base {
    ...
    public function __construct($module) {
        ...
        $this->title = __('Círculos de Cuidados', FLAVOR_PLATFORM_TEXT_DOMAIN);
    ...
    public function get_widget_data(): array {
        ...
        if (!$user_id) {
            return [
                'stats' => [],
                'items' => [],
                'empty_state' => __('Inicia sesión para ver tus círculos de cuidados', FLAVOR_PLATFORM_TEXT_DOMAIN),
            ];
        }
        ...
        foreach ($necesidades_urgentes as $necesidad) {
            ...
            $items[] = [
                ...
                'badge' => $necesidad->urgencia === 'urgente' ? __('Urgente', FLAVOR_PLATFORM_TEXT_DOMAIN) : null,
        ...
        return [
            'stats' => [
                ...
                [
                    ...
                    'label' => __('Horas donadas', FLAVOR_PLATFORM_TEXT_DOMAIN),
                    'color' => 'green',
                ],
                [
                    'icon' => 'dashicons-groups',
                    'valor' => $stats['necesidades_ayudadas'],
                    'label' => __('Ayudas', FLAVOR_PLATFORM_TEXT_DOMAIN),
                    'color' => 'purple',
                ],
            ],
            'items' => $items,
            'empty_state' => __('No hay necesidades pendientes en tus círculos', FLAVOR_PLATFORM_TEXT_DOMAIN),
            'footer' => [
                [
                    'label' => $this->is_admin_context() ? __('Ver panel', FLAVOR_PLATFORM_TEXT_DOMAIN) : __('Ver círculos', FLAVOR_PLATFORM_TEXT_DOMAIN),
    ...
    public function render_custom_content(array $data): void {
        ...
        $urgentes = array_filter($data['items'], function($item) {
            return ($item['badge'] ?? '') === __('Urgente', FLAVOR_PLATFORM_TEXT_DOMAIN);
        });
