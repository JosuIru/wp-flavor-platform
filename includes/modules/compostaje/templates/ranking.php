...
switch ($periodo) {
    case 'semana':
        ...
        $periodo_label = __('Esta semana', FLAVOR_PLATFORM_TEXT_DOMAIN);
        break;
    case 'mes':
        $periodo_sql = "AND fecha_aportacion >= DATE_SUB(NOW(), INTERVAL 1 MONTH)";
        $periodo_label = __('Este mes', FLAVOR_PLATFORM_TEXT_DOMAIN);
        break;
    case 'anio':
        $periodo_sql = "AND fecha_aportacion >= DATE_SUB(NOW(), INTERVAL 1 YEAR)";
        $periodo_label = __('Este año', FLAVOR_PLATFORM_TEXT_DOMAIN);
        break;
    default:
        $periodo_label = __('Todo el tiempo', FLAVOR_PLATFORM_TEXT_DOMAIN);
}
...
$niveles = [
    1 => ['nombre' => __('Semilla', FLAVOR_PLATFORM_TEXT_DOMAIN), 'kg_min' => 0, 'icono' => '🌱'],
    2 => ['nombre' => __('Brote', FLAVOR_PLATFORM_TEXT_DOMAIN), 'kg_min' => 10, 'icono' => '🌿'],
    3 => ['nombre' => __('Planta', FLAVOR_PLATFORM_TEXT_DOMAIN), 'kg_min' => 50, 'icono' => '🌳'],
    4 => ['nombre' => __('Árbol', FLAVOR_PLATFORM_TEXT_DOMAIN), 'kg_min' => 150, 'icono' => '🌲'],
    5 => ['nombre' => __('Bosque', FLAVOR_PLATFORM_TEXT_DOMAIN), 'kg_min' => 500, 'icono' => '🏔️'],
    6 => ['nombre' => __('Ecosistema', FLAVOR_PLATFORM_TEXT_DOMAIN), 'kg_min' => 1000, 'icono' => '🌍'],
];
...
<div class="compostaje-ranking-wrapper">
    <div class="ranking-header">
        <h2><?php esc_html_e('Ranking de Compostaje', FLAVOR_PLATFORM_TEXT_DOMAIN); ?></h2>
        <p><?php esc_html_e('Los mejores contribuidores de la comunidad', FLAVOR_PLATFORM_TEXT_DOMAIN); ?></p>
    </div>
    ...
    <div class="ranking-filtros">
        ...
        <a href="<?php echo esc_url(add_query_arg('periodo', 'anio')); ?>"
           class="filtro-btn <?php echo $periodo === 'anio' ? 'active' : ''; ?>">
            <?php esc_html_e('Año', FLAVOR_PLATFORM_TEXT_DOMAIN); ?>
        </a>
        <a href="<?php echo esc_url(remove_query_arg('periodo')); ?>"
           class="filtro-btn <?php echo $periodo === 'todo' ? 'active' : ''; ?>">
            <?php esc_html_e('Todo', FLAVOR_PLATFORM_TEXT_DOMAIN); ?>
        </a>
    </div>
    ...
    <?php if ($usuario_actual && $mis_stats && $mis_stats->total_puntos > 0): ?>
        <div class="mi-posicion-card">
            <div class="mi-posicion-rank">
                <span class="rank-label"><?php esc_html_e('Tu posición', FLAVOR_PLATFORM_TEXT_DOMAIN); ?></span>
            ...
            <div class="mi-posicion-stats">
                <div class="stat">
                    ...
                    <span class="stat-label"><?php esc_html_e('puntos', FLAVOR_PLATFORM_TEXT_DOMAIN); ?></span>
                </div>
                <div class="stat">
                    <span class="stat-value"><?php echo esc_html(number_format($mis_stats->total_kg, 1)); ?> kg</span>
                    <span class="stat-label"><?php esc_html_e('compostado', FLAVOR_PLATFORM_TEXT_DOMAIN); ?></span>
                </div>
                <div class="stat">
                    <?php
    ...
    <?php if (count($ranking) >= 3): ?>
        <div class="ranking-podio">
            <?php for ($i = 0; $i < 3; $i++):
                ...
                $nombre_usuario = $user_data ? $user_data->display_name : __('Usuario', FLAVOR_PLATFORM_TEXT_DOMAIN);
                ...
                <div class="ranking-item <?php echo $es_yo ? 'es-yo' : ''; ?>">
                    ...
                    <div class="ranking-info">
                        <span class="ranking-nombre">
                            ...
                            <?php if ($es_yo): ?>
                                <span class="badge-yo"><?php esc_html_e('Tú', FLAVOR_PLATFORM_TEXT_DOMAIN); ?></span>
                            <?php endif; ?>
                        </span>
    ...
    <?php if (empty($ranking)): ?>
        <div class="ranking-empty">
            ...
            <h3><?php esc_html_e('Aún no hay participantes', FLAVOR_PLATFORM_TEXT_DOMAIN); ?></h3>
            <p><?php esc_html_e('¡Sé el primero en aparecer en el ranking!', FLAVOR_PLATFORM_TEXT_DOMAIN); ?></p>
    ...
    <div class="ranking-niveles">
        <h3><?php esc_html_e('Niveles de Compostador', FLAVOR_PLATFORM_TEXT_DOMAIN); ?></h3>
        <div class="niveles-grid">
            <?php foreach ($niveles as $nivel => $datos): ?>
                <div class="nivel-item">
                    ...
                    <span class="nivel-req"><?php echo $datos['kg_min'] > 0 ? esc_html($datos['kg_min']) . '+ kg' : esc_html__('Inicio', FLAVOR_PLATFORM_TEXT_DOMAIN); ?></span>
                </div>
            <?php endforeach; ?>
        </div>
    </div>
