...
class Flavor_Crowdfunding_Frontend_Controller {
    ...
    public function shortcode_listado($atts) {
        ...
        echo '<p>' . __('Módulo Crowdfunding - Listado', FLAVOR_PLATFORM_TEXT_DOMAIN) . '</p>';
        echo '</div>';
        return ob_get_clean();
    }
    ...
    public function render_tab_principal() {
        ...
        echo '<h3>' . esc_html__('Crowdfunding', FLAVOR_PLATFORM_TEXT_DOMAIN) . '</h3>';
        echo '<p>' . esc_html__('Contenido del tab de Crowdfunding.', FLAVOR_PLATFORM_TEXT_DOMAIN) . '</p>';
        echo '</div>';
    }
}
