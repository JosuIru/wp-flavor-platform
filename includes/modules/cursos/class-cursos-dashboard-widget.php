...
class Flavor_Cursos_Dashboard_Widget extends Flavor_Dashboard_Widget_Base {
    ...
    public function __construct() {
        ...
        $this->title = __('Cursos', FLAVOR_PLATFORM_TEXT_DOMAIN);
        $this->description = __('Formación y aprendizaje continuo', FLAVOR_PLATFORM_TEXT_DOMAIN);
    ...
    private function fetch_widget_data(int $user_id): array {
        ...
        $stats = [
            [
                ...
                'label' => __('Cursos disponibles', FLAVOR_PLATFORM_TEXT_DOMAIN),
                'color' => 'primary',
        ...
        return [
            'stats' => $stats,
            'items' => $items,
            'empty_state' => __('No hay cursos disponibles actualmente', FLAVOR_PLATFORM_TEXT_DOMAIN),
            'footer' => [
                [
    ...
    private function get_cursos_disponibles(int $limite): array {
        ...
        foreach ($cursos as $curso) {
            ...
            $meta_text = $curso->fecha_inicio ? date_i18n('j M', strtotime($curso->fecha_inicio)) : __('Abierto', FLAVOR_PLATFORM_TEXT_DOMAIN);
            if ($plazas_disponibles !== null) {
                $meta_text .= ' - ' . sprintf(__('%d plazas', FLAVOR_PLATFORM_TEXT_DOMAIN), $plazas_disponibles);
            }
