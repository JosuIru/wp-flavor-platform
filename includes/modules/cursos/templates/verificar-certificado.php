...
<div class="verificar-certificado-wrapper" style="max-width: 500px; margin: 3rem auto; padding: 2rem;">
    <div style="background: #fff; border-radius: 12px; box-shadow: 0 4px 6px -1px rgba(0,0,0,0.1); padding: 2rem; text-align: center;">
        ...
        <h2 style="margin: 0 0 0.5rem 0; color: #1f2937;"><?php _e('Verificar Certificado', FLAVOR_PLATFORM_TEXT_DOMAIN); ?></h2>
        <p style="color: #6b7280; margin-bottom: 1.5rem;">
            <?php _e('Introduce el código de verificación para comprobar la autenticidad del certificado.', FLAVOR_PLATFORM_TEXT_DOMAIN); ?>
        </p>
        ...
        <form action="" method="get" style="display: flex; gap: 0.5rem;">
            <input
                type="text"
                name="codigo"
                placeholder="<?php esc_attr_e('Ej: CERT-ABC12345', FLAVOR_PLATFORM_TEXT_DOMAIN); ?>"
                required
            ...
            >
                <?php _e('Verificar', FLAVOR_PLATFORM_TEXT_DOMAIN); ?>
            </button>
        </form>
    </div>
</div>
