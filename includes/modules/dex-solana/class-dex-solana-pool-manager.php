...
class Flavor_Dex_Solana_Pool_Manager {
    ...
    public function crear_pool($token_a_simbolo, $token_b_simbolo, $reserva_a_inicial, $reserva_b_inicial, $fee_porcentaje = 0.30) {
        ...
        if (!$token_a_datos || !$token_b_datos) {
            flavor_platform_log( 'Tokens no encontrados en registry: ' . $token_a_simbolo . ', ' . $token_b_simbolo, 'warning', 'DEX-Solana' );
            return false;
        }
        ...
        if (!$mint_address_token_a || !$mint_address_token_b) {
            flavor_platform_log( 'Mint addresses no encontradas para tokens: ' . $token_a_simbolo . ', ' . $token_b_simbolo, 'warning', 'DEX-Solana' );
            return false;
        }
