...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['em_settings_nonce'])) {
    if (wp_verify_nonce($_POST['em_settings_nonce'], 'em_save_settings')) {
        ...
        echo '<div class="notice notice-success is-dismissible"><p>' . __('Configuración guardada.', 'flavor-chat-ia') . '</p></div>';
    }
}
?>
...
<div class="wrap em-configuracion">
    ...
    <form method="post">
        ...
        <div class="em-config-tabs">
            <button type="button" class="em-config-tab active" data-tab="general"><?php _e('General', 'flavor-chat-ia'); ?></button>
            <button type="button" class="em-config-tab" data-tab="smtp"><?php _e('SMTP', 'flavor-chat-ia'); ?></button>
            <button type="button" class="em-config-tab" data-tab="apariencia"><?php _e('Apariencia', 'flavor-chat-ia'); ?></button>
            <button type="button" class="em-config-tab" data-tab="paginas"><?php _e('Páginas', 'flavor-chat-ia'); ?></button>
        </div>
        ...
        <div class="em-config-panel active" data-tab="general">
            <table class="form-table">
                <tr>
                    <th><label for="remitente_nombre"><?php _e('Nombre del remitente', 'flavor-chat-ia'); ?></label></th>
                    <td>
                        ...
                        <p class="description"><?php _e('Nombre que aparece como remitente en los emails.', 'flavor-chat-ia'); ?></p>
                    </td>
                </tr>
                <tr>
                    <th><label for="remitente_email"><?php _e('Email del remitente', 'flavor-chat-ia'); ?></label></th>
                    <td>
                ...
                <tr>
                    <th><label for="responder_a"><?php _e('Responder a', 'flavor-chat-ia'); ?></label></th>
                    <td>
                        <input type="email" name="responder_a" id="responder_a" class="regular-text"
                               value="<?php echo esc_attr($settings['responder_a']); ?>">
                        <p class="description"><?php _e('Email donde se recibirán las respuestas.', 'flavor-chat-ia'); ?></p>
                    </td>
                </tr>
                <tr>
                    <th><label for="emails_por_hora"><?php _e('Límite de envíos por hora', 'flavor-chat-ia'); ?></label></th>
                    <td>
                        ...
                        <p class="description"><?php _e('Máximo de emails a enviar por hora (depende de tu proveedor).', 'flavor-chat-ia'); ?></p>
                    </td>
                </tr>
                <tr>
                    <th><?php _e('Opciones', 'flavor-chat-ia'); ?></th>
                    <td>
                        <label>
                            <input type="checkbox" name="doble_optin_global" value="1"
                                <?php checked($settings['doble_optin_global']); ?>>
                            <?php _e('Requerir doble opt-in por defecto', 'flavor-chat-ia'); ?>
                        </label>
                        <br><br>
                        <label>
                            <input type="checkbox" name="tracking_aperturas" value="1"
                                <?php checked($settings['tracking_aperturas']); ?>>
                            <?php _e('Registrar aperturas de email', 'flavor-chat-ia'); ?>
                        </label>
                        <br><br>
                        <label>
                            <input type="checkbox" name="tracking_clicks" value="1"
                                <?php checked($settings['tracking_clicks']); ?>>
                            <?php _e('Registrar clicks en enlaces', 'flavor-chat-ia'); ?>
                        </label>
                    </td>
                </tr>
            </table>
        ...
        <div class="em-config-panel" data-tab="smtp" style="display:none;">
            ...
            <div class="em-smtp-config" style="<?php echo $settings['proveedor_smtp'] !== 'smtp' ? 'display:none;' : ''; ?>">
                <table class="form-table">
                    <tr>
                        <th><label for="smtp_host"><?php _e('Host SMTP', 'flavor-chat-ia'); ?></label></th>
                        <td>
                    ...
                    <tr>
                        <th><label for="smtp_puerto"><?php _e('Puerto', 'flavor-chat-ia'); ?></label></th>
                        <td>
                            <input type="number" name="smtp_puerto" id="smtp_puerto" class="small-text"
                                   value="<?php echo esc_attr($settings['smtp_puerto']); ?>">
                            <p class="description"><?php _e('Común: 587 (TLS), 465 (SSL), 25 (sin encriptación)', 'flavor-chat-ia'); ?></p>
                        </td>
                    </tr>
                    <tr>
                        <th><label for="smtp_usuario"><?php _e('Usuario', 'flavor-chat-ia'); ?></label></th>
                        <td>
                    ...
                    <tr>
                        <th><label for="smtp_password"><?php _e('Contraseña', 'flavor-chat-ia'); ?></label></th>
                        <td>
                    ...
                    <tr>
                        <th><label for="smtp_encriptacion"><?php _e('Encriptación', 'flavor-chat-ia'); ?></label></th>
                        <td>
                    ...
                    <tr>
                        ...
                        <td>
                            <button type="button" class="button em-btn-test-smtp">
                                <?php _e('Probar conexión SMTP', 'flavor-chat-ia'); ?>
                            </button>
        ...
        <div class="em-config-panel" data-tab="apariencia" style="display:none;">
            <table class="form-table">
                <tr>
                    <th><label for="color_primario"><?php _e('Color primario', 'flavor-chat-ia'); ?></label></th>
                    <td>
                ...
                <tr>
                    <th><label for="color_secundario"><?php _e('Color secundario', 'flavor-chat-ia'); ?></label></th>
                    <td>
                ...
                <tr>
                    <th><label for="pie_email_global"><?php _e('Pie de email global', 'flavor-chat-ia'); ?></label></th>
                    <td>
                        ...
                        <p class="description"><?php _e('Texto que aparece al final de todos los emails. Puedes incluir HTML.', 'flavor-chat-ia'); ?></p>
                    </td>
                </tr>
            </table>
        </div>
        ...
        <div class="em-config-panel" data-tab="paginas" style="display:none;">
            <p class="description">
                <?php _e('Selecciona o crea las páginas que contendrán los shortcodes necesarios.', 'flavor-chat-ia'); ?>
            </p>

            <table class="form-table">
                <tr>
                    <th><?php _e('Página de confirmación', 'flavor-chat-ia'); ?></th>
                    <td>
                        <?php
                        wp_dropdown_pages([
                ...
                <tr>
                    <th><?php _e('Página de preferencias', 'flavor-chat-ia'); ?></th>
                    <td>
                        <?php
                        wp_dropdown_pages([
                ...
                <tr>
                    <th><?php _e('Página de baja', 'flavor-chat-ia'); ?></th>
                    <td>
                        <?php
                        wp_dropdown_pages([
                            'name' => 'pagina_baja',
