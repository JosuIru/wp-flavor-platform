...
<div class="wrap em-suscriptores">
    <h1>
        <?php _e('Suscriptores', 'flavor-chat-ia'); ?>
        <button type="button" class="page-title-action em-btn-importar"><?php _e('Importar', 'flavor-chat-ia'); ?></button>
        <button type="button" class="page-title-action em-btn-exportar"><?php _e('Exportar', 'flavor-chat-ia'); ?></button>
    </h1>
    ...
    <div class="em-filtros">
        <form method="get">
            ...
            <div class="em-filtro-grupo">
                <input type="search" name="s" placeholder="<?php esc_attr_e('Buscar por email o nombre...', 'flavor-chat-ia'); ?>"
                       value="<?php echo esc_attr($buscar); ?>">
            </div>

            <div class="em-filtro-grupo">
                <select name="lista">
                    <option value=""><?php _e('Todas las listas', 'flavor-chat-ia'); ?></option>
            ...
            <div class="em-filtro-grupo">
                <select name="estado">
                    ...
                    <option value="activo" <?php selected($estado_filtro, 'activo'); ?>><?php _e('Activo', 'flavor-chat-ia'); ?></option>
                    <option value="pendiente" <?php selected($estado_filtro, 'pendiente'); ?>><?php _e('Pendiente', 'flavor-chat-ia'); ?></option>
                    <option value="baja" <?php selected($estado_filtro, 'baja'); ?>><?php _e('Baja', 'flavor-chat-ia'); ?></option>
                    <option value="rebotado" <?php selected($estado_filtro, 'rebotado'); ?>><?php _e('Rebotado', 'flavor-chat-ia'); ?></option>
                </select>
            </div>
    ...
    <div class="em-stats-bar">
        <span>
            ...
            <?php _e('suscriptores encontrados', 'flavor-chat-ia'); ?>
        </span>
    </div>
    ...
    <?php if (empty($suscriptores)): ?>
        <div class="em-empty-state">
            ...
            <h2><?php _e('No hay suscriptores', 'flavor-chat-ia'); ?></h2>
            <p><?php _e('Importa tu lista de suscriptores o añádelos manualmente.', 'flavor-chat-ia'); ?></p>
        ...
        <table class="wp-list-table widefat fixed striped em-table-suscriptores">
            <thead>
                <tr>
                    ...
                    <th class="column-email"><?php _e('Email', 'flavor-chat-ia'); ?></th>
                    <th class="column-nombre"><?php _e('Nombre', 'flavor-chat-ia'); ?></th>
                    <th class="column-estado"><?php _e('Estado', 'flavor-chat-ia'); ?></th>
                    <th class="column-listas"><?php _e('Listas', 'flavor-chat-ia'); ?></th>
                    <th class="column-stats"><?php _e('Estadísticas', 'flavor-chat-ia'); ?></th>
                    <th class="column-fecha"><?php _e('Suscrito', 'flavor-chat-ia'); ?></th>
                </tr>
            </thead>
            <tbody>
                <?php foreach ($suscriptores as $suscriptor): ?>
                    ...
                    <tr data-id="<?php echo esc_attr($suscriptor->id); ?>">
                        ...
                        <td class="column-stats">
                            <span title="<?php esc_attr_e('Emails enviados', 'flavor-chat-ia'); ?>">
                                <span class="dashicons dashicons-email"></span>
                                <?php echo $suscriptor->total_emails_enviados; ?>
                            </span>
                            <span title="<?php esc_attr_e('Aperturas', 'flavor-chat-ia'); ?>">
                                <span class="dashicons dashicons-visibility"></span>
                                <?php echo $suscriptor->total_abiertos; ?>
                            </span>
                            <span title="<?php esc_attr_e('Clicks', 'flavor-chat-ia'); ?>">
        ...
        <?php if ($total_paginas > 1): ?>
            <div class="em-paginacion">
                ...
                <span class="em-paginacion-info">
                    <?php printf(__('Página %d de %d', 'flavor-chat-ia'), $pagina, $total_paginas); ?>
                </span>
...
<div class="em-modal" id="em-modal-importar" style="display:none;">
    <div class="em-modal-content em-modal-lg">
        <h3><?php _e('Importar suscriptores', 'flavor-chat-ia'); ?></h3>
        <form id="em-form-importar" enctype="multipart/form-data">
            <div class="em-form-section">
                <label><?php _e('Archivo CSV', 'flavor-chat-ia'); ?></label>
                <input type="file" name="archivo" accept=".csv" required>
                <p class="description"><?php _e('Columnas requeridas: email. Opcionales: nombre, apellidos', 'flavor-chat-ia'); ?></p>
            </div>

            <div class="em-form-section">
                <label><?php _e('Añadir a lista', 'flavor-chat-ia'); ?></label>
            ...
            <div class="em-form-section">
                <label>
                    ...
                    <?php _e('Actualizar suscriptores existentes', 'flavor-chat-ia'); ?>
                </label>
            </div>

            <div class="em-modal-actions">
                <button type="button" class="button em-modal-close"><?php _e('Cancelar', 'flavor-chat-ia'); ?></button>
                <button type="submit" class="button button-primary"><?php _e('Importar', 'flavor-chat-ia'); ?></button>
            </div>
        </form>
    </div>
</div>
...
<div class="em-modal" id="em-modal-suscriptor" style="display:none;">
    <div class="em-modal-content em-modal-lg">
        <div class="em-modal-loading"><?php _e('Cargando...', 'flavor-chat-ia'); ?></div>
