...
<div class="flavor-empresas-listado">
    <div class="flavor-listado-header" style="margin-bottom:24px;">
        <h2 style="margin:0 0 8px;"><?php esc_html_e('Directorio de empresas', FLAVOR_PLATFORM_TEXT_DOMAIN); ?></h2>
        <p style="color:#666;margin:0;"><?php printf(esc_html__('%d empresas registradas', FLAVOR_PLATFORM_TEXT_DOMAIN), count($empresas)); ?></p>
    </div>
    ...
    <div class="flavor-card" style="margin-bottom:24px;">
        <form method="get" style="display:flex;gap:12px;flex-wrap:wrap;align-items:flex-end;">
            <div style="flex:1;min-width:200px;">
                <label style="display:block;font-size:12px;color:#666;margin-bottom:4px;"><?php esc_html_e('Buscar', FLAVOR_PLATFORM_TEXT_DOMAIN); ?></label>
                <input type="text" name="s" value="<?php echo esc_attr($_GET['s'] ?? ''); ?>" placeholder="<?php esc_attr_e('Nombre de empresa...', FLAVOR_PLATFORM_TEXT_DOMAIN); ?>" class="flavor-input" />
            </div>
            <div>
                <label style="display:block;font-size:12px;color:#666;margin-bottom:4px;"><?php esc_html_e('Sector', FLAVOR_PLATFORM_TEXT_DOMAIN); ?></label>
    ...
    <div class="flavor-grid flavor-grid-3">
        ...
        <div class="flavor-card flavor-empresa-card">
            ...
            <div class="flavor-empresa-contacto">
                ...
                <?php endif; ?>
                <?php if ($emp->web): ?>
                <a href="<?php echo esc_url($emp->web); ?>" target="_blank"><span class="dashicons dashicons-admin-site"></span> <?php esc_html_e('Web', FLAVOR_PLATFORM_TEXT_DOMAIN); ?></a>
                <?php endif; ?>
            </div>
    ...
    <div class="flavor-card" style="text-align:center;padding:60px;">
        ...
        <h3><?php esc_html_e('No hay empresas disponibles', FLAVOR_PLATFORM_TEXT_DOMAIN); ?></h3>
        <p style="color:#666;"><?php esc_html_e('No se encontraron empresas con los filtros seleccionados.', FLAVOR_PLATFORM_TEXT_DOMAIN); ?></p>
    </div>
    <?php endif; ?>
</div>
...
function solicitarUnirse(empresaId) {
    if (confirm('<?php echo esc_js(__('¿Solicitar unirse a esta empresa?', FLAVOR_PLATFORM_TEXT_DOMAIN)); ?>')) {
        ...
        jQuery.post(flavorAjax.url, {
            ...
            if (response.success) {
                ...
                alert(response.data.message || '<?php echo esc_js(__('Error al procesar la solicitud.', FLAVOR_PLATFORM_TEXT_DOMAIN)); ?>');
            }
        });
    }
}
