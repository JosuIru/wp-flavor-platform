...
<div class="wrap flavor-modulo-page">
    <a href="<?php echo esc_url(admin_url('admin.php?page=empresas-listado&action=ver&id=' . $empresa->id)); ?>" class="button" style="margin-bottom:16px;">
        ← <?php esc_html_e('Volver a la empresa', FLAVOR_PLATFORM_TEXT_DOMAIN); ?>
    </a>
        ...
        <div style="display:flex;justify-content:space-between;align-items:center;">
            <div>
                <h1 style="margin:0;font-size:20px;">
                    <?php printf(esc_html__('Miembros de %s', FLAVOR_PLATFORM_TEXT_DOMAIN), esc_html($empresa->nombre)); ?>
                </h1>
                <p style="margin:8px 0 0;color:#666;"><?php printf(esc_html__('%d miembros en total', FLAVOR_PLATFORM_TEXT_DOMAIN), count($miembros)); ?></p>
            </div>
            <button type="button" class="button button-primary" onclick="document.getElementById('modal-agregar').style.display='flex';">
                ...
                <?php esc_html_e('Agregar miembro', FLAVOR_PLATFORM_TEXT_DOMAIN); ?>
            </button>
        </div>
    </div>
    ...
    <table class="widefat striped" style="background:#fff;border-radius:8px;overflow:hidden;">
        <thead>
            <tr>
                <th style="padding:12px;"><?php esc_html_e('Miembro', FLAVOR_PLATFORM_TEXT_DOMAIN); ?></th>
                <th style="padding:12px;"><?php esc_html_e('Rol', FLAVOR_PLATFORM_TEXT_DOMAIN); ?></th>
                <th style="padding:12px;"><?php esc_html_e('Cargo', FLAVOR_PLATFORM_TEXT_DOMAIN); ?></th>
                <th style="padding:12px;"><?php esc_html_e('Contacto corporativo', FLAVOR_PLATFORM_TEXT_DOMAIN); ?></th>
                <th style="padding:12px;text-align:center;"><?php esc_html_e('Estado', FLAVOR_PLATFORM_TEXT_DOMAIN); ?></th>
                <th style="padding:12px;"><?php esc_html_e('Fecha alta', FLAVOR_PLATFORM_TEXT_DOMAIN); ?></th>
                ...
                <tr>
                    ...
                    <td style="padding:12px;text-align:center;">
                        <form method="post" style="display:inline;">
                            ...
                            <button type="submit" name="accion_miembro" value="activar" class="button button-small" title="<?php esc_attr_e('Activar', FLAVOR_PLATFORM_TEXT_DOMAIN); ?>">
                                <span class="dashicons dashicons-yes" style="color:#10b981;"></span>
                            </button>
                            <?php endif; ?>

                            <?php if ($m->estado === 'activo'): ?>
                            <button type="submit" name="accion_miembro" value="suspender" class="button button-small" title="<?php esc_attr_e('Suspender', FLAVOR_PLATFORM_TEXT_DOMAIN); ?>" onclick="return confirm('<?php esc_attr_e('¿Suspender este miembro?', FLAVOR_PLATFORM_TEXT_DOMAIN); ?>');">
                                <span class="dashicons dashicons-controls-pause" style="color:#f59e0b;"></span>
                            </button>
                            <?php endif; ?>

                            <?php if ($m->estado === 'suspendido'): ?>
                            <button type="submit" name="accion_miembro" value="activar" class="button button-small" title="<?php esc_attr_e('Reactivar', FLAVOR_PLATFORM_TEXT_DOMAIN); ?>">
                            ...
                            <button type="submit" name="accion_miembro" value="cambiar_rol" class="button button-small" title="<?php esc_attr_e('Cambiar rol', FLAVOR_PLATFORM_TEXT_DOMAIN); ?>">
                                <span class="dashicons dashicons-update"></span>
                            </button>

                            <button type="submit" name="accion_miembro" value="dar_baja" class="button button-small" title="<?php esc_attr_e('Dar de baja', FLAVOR_PLATFORM_TEXT_DOMAIN); ?>" onclick="return confirm('<?php esc_attr_e('¿Dar de baja definitiva?', FLAVOR_PLATFORM_TEXT_DOMAIN); ?>');" style="margin-left:8px;">
            ...
            <?php else: ?>
                <tr>
                    <td colspan="7" style="padding:40px;text-align:center;color:#666;">
                        <?php esc_html_e('Esta empresa no tiene miembros.', FLAVOR_PLATFORM_TEXT_DOMAIN); ?>
                    </td>
                </tr>
            <?php endif; ?>
        </tbody>
...
<div id="modal-agregar" style="display:none;position:fixed;inset:0;background:rgba(0,0,0,0.5);align-items:center;justify-content:center;z-index:100000;">
    <div style="background:#fff;padding:24px;border-radius:12px;max-width:500px;width:90%;">
        <h3 style="margin:0 0 20px;"><?php esc_html_e('Agregar miembro', FLAVOR_PLATFORM_TEXT_DOMAIN); ?></h3>
        <form method="post">
            <?php wp_nonce_field('agregar_miembro'); ?>

            <p>
                <label><strong><?php esc_html_e('Usuario', FLAVOR_PLATFORM_TEXT_DOMAIN); ?></strong></label><br>
                <select name="user_id" required style="width:100%;">
                    <option value=""><?php esc_html_e('Seleccionar usuario...', FLAVOR_PLATFORM_TEXT_DOMAIN); ?></option>
                    <?php
            ...
            <p>
                <label><strong><?php esc_html_e('Rol', FLAVOR_PLATFORM_TEXT_DOMAIN); ?></strong></label><br>
            ...
            <p>
                <label><strong><?php esc_html_e('Cargo', FLAVOR_PLATFORM_TEXT_DOMAIN); ?></strong></label><br>
            ...
            <div style="margin-top:20px;display:flex;gap:8px;justify-content:flex-end;">
                ...
                <button type="submit" name="agregar_miembro" class="button button-primary">
                    <?php esc_html_e('Agregar', FLAVOR_PLATFORM_TEXT_DOMAIN); ?>
                </button>
            </div>
        </form>
    </div>
