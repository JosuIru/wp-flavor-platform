...
$estados_cierre = [
    'borrador' => ['label' => __('Borrador', FLAVOR_PLATFORM_TEXT_DOMAIN), 'color' => '#6b7280'],
    'pendiente' => ['label' => __('Pendiente', FLAVOR_PLATFORM_TEXT_DOMAIN), 'color' => '#f59e0b'],
    'procesando' => ['label' => __('Procesando', FLAVOR_PLATFORM_TEXT_DOMAIN), 'color' => '#3b82f6'],
    'completado' => ['label' => __('Completado', FLAVOR_PLATFORM_TEXT_DOMAIN), 'color' => '#10b981'],
    'anulado' => ['label' => __('Anulado', FLAVOR_PLATFORM_TEXT_DOMAIN), 'color' => '#ef4444'],
];
?>
...
<div class="energia-cierres" x-data="energiaCierres()">
    ...
    <div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(200px, 1fr)); gap: 16px; margin-bottom: 24px;">
        <div style="background: linear-gradient(135deg, #f59e0b 0%, #d97706 100%); color: #fff; padding: 20px; border-radius: 12px;">
            <div style="font-size: 11px; opacity: 0.8; text-transform: uppercase;"><?php esc_html_e('Cierres este año', FLAVOR_PLATFORM_TEXT_DOMAIN); ?></div>
        ...
        <div style="background: linear-gradient(135deg, #ef4444 0%, #dc2626 100%); color: #fff; padding: 20px; border-radius: 12px;">
            <div style="font-size: 11px; opacity: 0.8; text-transform: uppercase;"><?php esc_html_e('Pendientes liquidar', FLAVOR_PLATFORM_TEXT_DOMAIN); ?></div>
        ...
        <div style="background: linear-gradient(135deg, #10b981 0%, #059669 100%); color: #fff; padding: 20px; border-radius: 12px;">
            <div style="font-size: 11px; opacity: 0.8; text-transform: uppercase;"><?php esc_html_e('kWh repartidos (año)', FLAVOR_PLATFORM_TEXT_DOMAIN); ?></div>
        ...
        <div style="background: linear-gradient(135deg, #8b5cf6 0%, #7c3aed 100%); color: #fff; padding: 20px; border-radius: 12px;">
            <div style="font-size: 11px; opacity: 0.8; text-transform: uppercase;"><?php esc_html_e('Ahorro total (año)', FLAVOR_PLATFORM_TEXT_DOMAIN); ?></div>
    ...
    <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 20px;">
        <h2 style="margin: 0;"><?php esc_html_e('Histórico de Cierres', FLAVOR_PLATFORM_TEXT_DOMAIN); ?></h2>
        <button class="button button-primary" @click="showModalCierre = true">
            <span class="dashicons dashicons-plus-alt2"></span>
            <?php esc_html_e('Nuevo Cierre de Período', FLAVOR_PLATFORM_TEXT_DOMAIN); ?>
        </button>
    </div>
            ...
            <div style="padding: 16px;">
                <div style="display: grid; grid-template-columns: repeat(3, 1fr); gap: 12px; margin-bottom: 16px;">
                    <div style="text-align: center;">
                        ...
                        <div style="font-size: 11px; color: #666;"><?php esc_html_e('kWh generados', FLAVOR_PLATFORM_TEXT_DOMAIN); ?></div>
                    </div>
                    <div style="text-align: center;">
                        <div style="font-size: 20px; font-weight: bold; color: #3b82f6;">
                            <?php echo intval($cierre->num_participantes); ?>
                        </div>
                        <div style="font-size: 11px; color: #666;"><?php esc_html_e('Participantes', FLAVOR_PLATFORM_TEXT_DOMAIN); ?></div>
                    </div>
                    <div style="text-align: center;">
                        <div style="font-size: 20px; font-weight: bold; color: #8b5cf6;">
                            <?php echo number_format($cierre->total_kwh_repartidos * 0.18, 0); ?>€
                        </div>
                        <div style="font-size: 11px; color: #666;"><?php esc_html_e('Valor', FLAVOR_PLATFORM_TEXT_DOMAIN); ?></div>
                    </div>
                </div>
                ...
                <div style="background: #f3f4f6; padding: 8px 12px; border-radius: 8px; margin-bottom: 12px; font-size: 13px;">
                    <strong><?php esc_html_e('Modelo:', FLAVOR_PLATFORM_TEXT_DOMAIN); ?></strong>
                ...
                <div style="display: flex; gap: 8px;">
                    ...
                    <button class="button button-primary" @click="procesarCierre(<?php echo $cierre->id; ?>)">
                        <?php esc_html_e('Procesar', FLAVOR_PLATFORM_TEXT_DOMAIN); ?>
                    </button>
                    <?php endif; ?>
                    <a href="<?php echo admin_url('admin-post.php?action=energia_comunitaria_exportar_liquidacion&cierre_id=' . $cierre->id . '&_wpnonce=' . wp_create_nonce('export_liquidacion')); ?>"
                       class="button" title="<?php esc_attr_e('Exportar CSV', FLAVOR_PLATFORM_TEXT_DOMAIN); ?>">
    ...
    <div style="text-align: center; padding: 60px 20px; background: #f9fafb; border-radius: 12px;">
        ...
        <h3><?php esc_html_e('Sin cierres registrados', FLAVOR_PLATFORM_TEXT_DOMAIN); ?></h3>
        <p style="color: #666;"><?php esc_html_e('Crea el primer cierre de período para repartir la energía generada.', FLAVOR_PLATFORM_TEXT_DOMAIN); ?></p>
        <button class="button button-primary" @click="showModalCierre = true">
            <?php esc_html_e('Crear Cierre', FLAVOR_PLATFORM_TEXT_DOMAIN); ?>
        </button>
    </div>
    <?php endif; ?>
    ...
    <div style="margin-top: 24px; padding: 20px; background: #eff6ff; border-radius: 12px; border: 1px solid #bfdbfe;">
        <h4 style="margin: 0 0 12px; color: #1e40af;">
            ...
            <?php esc_html_e('Modelos de reparto disponibles', FLAVOR_PLATFORM_TEXT_DOMAIN); ?>
        </h4>
        <div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(250px, 1fr)); gap: 16px;">
            <div>
                <strong><?php esc_html_e('Proporcional', FLAVOR_PLATFORM_TEXT_DOMAIN); ?></strong>
                <p style="margin: 4px 0 0; font-size: 13px; color: #1e3a8a;">
                    <?php esc_html_e('Reparto según el coeficiente de cada participante (inversión, consumo estimado, etc.)', FLAVOR_PLATFORM_TEXT_DOMAIN); ?>
                </p>
            </div>
            <div>
                <strong><?php esc_html_e('Igualitario', FLAVOR_PLATFORM_TEXT_DOMAIN); ?></strong>
                <p style="margin: 4px 0 0; font-size: 13px; color: #1e3a8a;">
                    <?php esc_html_e('Todos los participantes reciben la misma cantidad de energía.', FLAVOR_PLATFORM_TEXT_DOMAIN); ?>
                </p>
            </div>
            <div>
                <strong><?php esc_html_e('Por consumo real', FLAVOR_PLATFORM_TEXT_DOMAIN); ?></strong>
                <p style="margin: 4px 0 0; font-size: 13px; color: #1e3a8a;">
                    <?php esc_html_e('Se prioriza el autoconsumo y se reparte el excedente.', FLAVOR_PLATFORM_TEXT_DOMAIN); ?>
                </p>
            </div>
        </div>
    </div>
    ...
    <div x-show="showModalCierre" x-cloak
        ...
        <div style="background: #fff; border-radius: 12px; padding: 24px; max-width: 500px; width: 90%; max-height: 90vh; overflow-y: auto;">
            <h2 style="margin: 0 0 20px;"><?php esc_html_e('Nuevo Cierre de Período', FLAVOR_PLATFORM_TEXT_DOMAIN); ?></h2>
...
document.addEventListener('alpine:init', () => {
    Alpine.data('energiaCierres', () => ({
        ...
        procesarCierre(id) {
            if (!confirm('<?php echo esc_js(__('¿Procesar este cierre? Se generarán las liquidaciones para cada participante.', FLAVOR_PLATFORM_TEXT_DOMAIN)); ?>')) {
                return;
            }
