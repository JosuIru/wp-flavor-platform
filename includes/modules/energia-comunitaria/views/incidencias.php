...
$prioridades = [
    'critica' => ['label' => __('Crítica', FLAVOR_PLATFORM_TEXT_DOMAIN), 'color' => '#ef4444', 'icon' => '🔴'],
    'alta' => ['label' => __('Alta', FLAVOR_PLATFORM_TEXT_DOMAIN), 'color' => '#f59e0b', 'icon' => '🟠'],
    'media' => ['label' => __('Media', FLAVOR_PLATFORM_TEXT_DOMAIN), 'color' => '#3b82f6', 'icon' => '🔵'],
    'baja' => ['label' => __('Baja', FLAVOR_PLATFORM_TEXT_DOMAIN), 'color' => '#10b981', 'icon' => '🟢'],
];

$estados = [
    'abierta' => ['label' => __('Abierta', FLAVOR_PLATFORM_TEXT_DOMAIN), 'color' => '#ef4444'],
    'en_progreso' => ['label' => __('En progreso', FLAVOR_PLATFORM_TEXT_DOMAIN), 'color' => '#f59e0b'],
    'pendiente_piezas' => ['label' => __('Pendiente piezas', FLAVOR_PLATFORM_TEXT_DOMAIN), 'color' => '#8b5cf6'],
    'resuelta' => ['label' => __('Resuelta', FLAVOR_PLATFORM_TEXT_DOMAIN), 'color' => '#10b981'],
    'cerrada' => ['label' => __('Cerrada', FLAVOR_PLATFORM_TEXT_DOMAIN), 'color' => '#6b7280'],
];
?>
...
<div class="energia-incidencias" x-data="energiaIncidencias()">
    ...
    <div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(150px, 1fr)); gap: 12px; margin-bottom: 20px;">
        <div style="background: #fee2e2; padding: 16px; border-radius: 8px; text-align: center;">
            ...
            <div style="font-size: 12px; color: #991b1b;"><?php esc_html_e('Abiertas', FLAVOR_PLATFORM_TEXT_DOMAIN); ?></div>
        </div>
        <div style="background: #fef3c7; padding: 16px; border-radius: 8px; text-align: center;">
            <div style="font-size: 28px; font-weight: bold; color: #d97706;"><?php echo $stats['en_progreso']; ?></div>
            <div style="font-size: 12px; color: #92400e;"><?php esc_html_e('En progreso', FLAVOR_PLATFORM_TEXT_DOMAIN); ?></div>
        </div>
        <div style="background: #dcfce7; padding: 16px; border-radius: 8px; text-align: center;">
            ...
            <div style="font-size: 12px; color: #166534;"><?php esc_html_e('Resueltas (mes)', FLAVOR_PLATFORM_TEXT_DOMAIN); ?></div>
        </div>
        ...
        <div style="background: #fecaca; padding: 16px; border-radius: 8px; text-align: center; animation: pulse 2s infinite;">
            ...
            <div style="font-size: 12px; color: #7f1d1d;"><?php esc_html_e('Críticas', FLAVOR_PLATFORM_TEXT_DOMAIN); ?></div>
        </div>
        <?php endif; ?>
    </div>
    ...
    <div style="display: flex; gap: 12px; margin-bottom: 20px; flex-wrap: wrap; align-items: center;">
        ...
        <select x-model="filtroPrioridad" @change="filtrar()" style="padding: 8px 12px;">
            <option value=""><?php esc_html_e('Todas las prioridades', FLAVOR_PLATFORM_TEXT_DOMAIN); ?></option>
        ...
        <div class="incidencia-card" style="background: #fff; border: 1px solid #e5e7eb; border-left: 4px solid <?php echo esc_attr($pri_info['color']); ?>; border-radius: 8px; padding: 16px;">
            ...
            <div style="display: flex; justify-content: space-between; align-items: center; padding-top: 12px; border-top: 1px solid #f3f4f6;">
                <div style="font-size: 12px; color: #9ca3af;">
                    <?php esc_html_e('Reportado por', FLAVOR_PLATFORM_TEXT_DOMAIN); ?>
                    <strong><?php echo esc_html($inc->reportado_por_nombre ?: __('Anónimo', FLAVOR_PLATFORM_TEXT_DOMAIN)); ?></strong>
                    •
                ...
                <div style="display: flex; gap: 8px;">
                    ...
                    <button class="button button-small" @click="cambiarEstado(<?php echo $inc->id; ?>, 'en_progreso')">
                        <?php esc_html_e('En progreso', FLAVOR_PLATFORM_TEXT_DOMAIN); ?>
                    </button>
    ...
    <div style="text-align: center; padding: 60px 20px; background: #f0fdf4; border-radius: 12px;">
        ...
        <h3 style="color: #166534;"><?php esc_html_e('Sin incidencias pendientes', FLAVOR_PLATFORM_TEXT_DOMAIN); ?></h3>
        <p style="color: #15803d;"><?php esc_html_e('Todas las instalaciones funcionan correctamente.', FLAVOR_PLATFORM_TEXT_DOMAIN); ?></p>
    </div>
    <?php endif; ?>
    ...
    <div x-show="showModalIncidencia" x-cloak
        ...
        <div style="background: #fff; border-radius: 12px; padding: 24px; max-width: 500px; width: 90%; max-height: 90vh; overflow-y: auto;">
            <h2 style="margin: 0 0 20px;"><?php esc_html_e('Reportar Incidencia', FLAVOR_PLATFORM_TEXT_DOMAIN); ?></h2>
