...
<div class="ec-cesiones" data-nonce="<?php echo esc_attr($nonce); ?>">
    <div class="ec-cesiones__header">
        <div class="ec-cesiones__titulo-wrap">
            ...
            <h3 class="ec-cesiones__titulo"><?php esc_html_e('Reservas Compartidas', FLAVOR_PLATFORM_TEXT_DOMAIN); ?></h3>
        </div>
        <p class="ec-cesiones__descripcion">
            <?php esc_html_e('Espacios cedidos por vecinos solidarios. ¡Aprovéchalos!', FLAVOR_PLATFORM_TEXT_DOMAIN); ?>
        </p>
    </div>
    ...
    <?php if (empty($cesiones)): ?>
        <div class="ec-cesiones__vacio">
            ...
            <p><?php esc_html_e('No hay cesiones disponibles en este momento.', FLAVOR_PLATFORM_TEXT_DOMAIN); ?></p>
            <small><?php esc_html_e('Las cesiones aparecerán aquí cuando otros usuarios compartan sus reservas.', FLAVOR_PLATFORM_TEXT_DOMAIN); ?></small>
        </div>
    <?php else: ?>
                ...
                <div class="ec-cesion <?php echo $cesion->es_solidaria ? 'ec-cesion--solidaria' : ''; ?>" data-id="<?php echo esc_attr($cesion->id); ?>">
                    <?php if ($cesion->es_solidaria): ?>
                        <span class="ec-cesion__badge-solidaria">
                            ...
                            <?php esc_html_e('Solidaria', FLAVOR_PLATFORM_TEXT_DOMAIN); ?>
                        </span>
                    <?php endif; ?>
                    ...
                    <?php if ($cesion->capacidad): ?>
                        <div class="ec-cesion__capacidad">
                            ...
                            <span><?php printf(esc_html__('Hasta %d personas', FLAVOR_PLATFORM_TEXT_DOMAIN), $cesion->capacidad); ?></span>
                        </div>
                    <?php endif; ?>

                    <div class="ec-cesion__cedente">
                        <?php echo get_avatar($cesion->cedente_id, 24); ?>
                        <span><?php printf(esc_html__('Cedido por %s', FLAVOR_PLATFORM_TEXT_DOMAIN), esc_html($cedente->display_name)); ?></span>
                    </div>
                        ...
                        <?php if ($usuario_logueado): ?>
                            <?php if ($cesion->cedente_id != get_current_user_id()): ?>
                                <button type="button" class="ec-btn ec-btn--primary ec-cesion__reclamar" data-cesion="<?php echo esc_attr($cesion->id); ?>">
                                    ...
                                    <?php esc_html_e('Reclamar', FLAVOR_PLATFORM_TEXT_DOMAIN); ?>
                                </button>
                            <?php else: ?>
                                <span class="ec-cesion__propia"><?php esc_html_e('Tu cesión', FLAVOR_PLATFORM_TEXT_DOMAIN); ?></span>
                            <?php endif; ?>
                        <?php else: ?>
                            <a href="<?php echo esc_url(wp_login_url(flavor_current_request_url())); ?>" class="ec-btn ec-btn--outline">
                                <?php esc_html_e('Inicia sesión para reclamar', FLAVOR_PLATFORM_TEXT_DOMAIN); ?>
                            </a>
                        <?php endif; ?>
                    </div>
                </div>
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        container.querySelectorAll('.ec-cesion__reclamar').forEach(btn => {
            btn.addEventListener('click', function() {
                ...
                if (!confirm('<?php echo esc_js(__('¿Reclamar esta reserva? Pasará a ser tuya.', FLAVOR_PLATFORM_TEXT_DOMAIN)); ?>')) {
                    return;
                }

                this.disabled = true;
                this.innerHTML = '<span class="dashicons dashicons-update"></span> <?php echo esc_js(__('Procesando...', FLAVOR_PLATFORM_TEXT_DOMAIN)); ?>';
                ...
                .then(data => {
                    ...
                    if (data.success) {
                        ...
                        this.disabled = false;
                        this.innerHTML = '<span class="dashicons dashicons-yes"></span> <?php echo esc_js(__('Reclamar', FLAVOR_PLATFORM_TEXT_DOMAIN)); ?>';
                    }
                });
            });
        });
