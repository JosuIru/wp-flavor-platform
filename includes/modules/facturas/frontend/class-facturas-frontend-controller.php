<?php
/**
 * Frontend Controller para Facturas
 *
...
/**
 * Controlador Frontend para el módulo de Facturas
 */
class Flavor_Facturas_Frontend_Controller {
    ...
    public function registrar_assets() {
        ...
        wp_register_style(
            'flavor-facturas',
            $base_url . '/assets/css/facturas.css',
        ...
        wp_register_script(
            'flavor-facturas',
            $base_url . '/assets/js/facturas.js',
            ['jquery'],
            $version,
            true
        );
        ...
        // Localizar script
        wp_localize_script('flavor-facturas', 'flavorFacturas', [
    ...
    public function encolar_assets() {
        wp_enqueue_style('flavor-facturas');
        wp_enqueue_script('flavor-facturas');
    }
    ...
    public function registrar_shortcodes() {
        if (!shortcode_exists('facturas_listado')) {
            add_shortcode('facturas_listado', [$this, 'shortcode_listado']);
        }
    }
    ...
    public function shortcode_listado($atts) {
        ...
        echo '<p>' . __('Módulo Facturas - Listado', FLAVOR_PLATFORM_TEXT_DOMAIN) . '</p>';
        echo '</div>';
        return ob_get_clean();
    }
    ...
    public function registrar_tabs($tabs) {
        $tabs['facturas'] = [
            'titulo' => __('Facturas', FLAVOR_PLATFORM_TEXT_DOMAIN),
    ...
    public function render_tab_principal() {
        ...
        echo '<h3>' . esc_html__('Facturas', FLAVOR_PLATFORM_TEXT_DOMAIN) . '</h3>';
        echo '<p>' . esc_html__('Contenido del tab de Facturas.', FLAVOR_PLATFORM_TEXT_DOMAIN) . '</p>';
        echo '</div>';
    }
}
