...
class Flavor_GC_Facturas_Integration {
    ...
    public function registrar_metabox_productor() {
        add_meta_box(
            ...
            __('Facturacion', FLAVOR_PLATFORM_TEXT_DOMAIN),
        ...
        <div class="gc-facturas-box">
            ...
            <div class="gc-facturas-stats" style="margin-bottom: 15px; padding: 10px; background: #f9f9f9; border-radius: 4px;">
                <div style="display: flex; justify-content: space-between; margin-bottom: 5px;">
                    <span><?php _e('Total facturas:', FLAVOR_PLATFORM_TEXT_DOMAIN); ?></span>
                ...
                <div style="display: flex; justify-content: space-between; color: #d63638;">
                    <span><?php _e('Pendientes de pago:', FLAVOR_PLATFORM_TEXT_DOMAIN); ?></span>
            ...
            <p style="margin-bottom: 10px;">
                <a href="<?php echo esc_url($url_nueva_factura); ?>" class="button button-primary" style="width: 100%; text-align: center;">
                    ...
                    <?php _e('Crear Factura', FLAVOR_PLATFORM_TEXT_DOMAIN); ?>
                </a>
            </p>
            ...
            <p>
                <a href="<?php echo esc_url($url_ver_facturas); ?>" class="button" style="width: 100%; text-align: center;">
                    ...
                    <?php _e('Ver Facturas', FLAVOR_PLATFORM_TEXT_DOMAIN); ?>
                </a>
            </p>
            <?php endif; ?>
        </div>
    ...
    public function agregar_columna_facturas($columns) {
        ...
        foreach ($columns as $key => $value) {
            ...
            if ($key === 'title') {
                $nueva_columnas['gc_facturas'] = __('Facturas', FLAVOR_PLATFORM_TEXT_DOMAIN);
            }
        }
        ...
        <div class="gc-facturas-col">
            <?php if ($total > 0): ?>
                <span class="gc-facturas-count">
                    <?php echo $total; ?>
                    <?php if ($pendientes > 0): ?>
                        <span style="color: #d63638;" title="<?php esc_attr_e('Pendientes', FLAVOR_PLATFORM_TEXT_DOMAIN); ?>">
                            (<?php echo $pendientes; ?>)
                        </span>
                    <?php endif; ?>
                </span>
    ...
    public function ajax_crear_factura_productor() {
        ...
        if (!$productor_id) {
            wp_send_json_error(__('ID de productor invalido', FLAVOR_PLATFORM_TEXT_DOMAIN));
        }

        $productor = get_post($productor_id);

        if (!$productor || $productor->post_type !== 'gc_productor') {
            wp_send_json_error(__('Productor no encontrado', FLAVOR_PLATFORM_TEXT_DOMAIN));
        }
    ...
    public function ajax_crear_factura_consumidor() {
        ...
        if (!$usuario_id) {
            wp_send_json_error(__('ID de usuario invalido', FLAVOR_PLATFORM_TEXT_DOMAIN));
        }

        $usuario = get_user_by('ID', $usuario_id);

        if (!$usuario) {
            wp_send_json_error(__('Usuario no encontrado', FLAVOR_PLATFORM_TEXT_DOMAIN));
        }
