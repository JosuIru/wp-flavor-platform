...
class Flavor_GC_Notification_Channels {
    ...
    public function enviar($evento, $destinatarios, $datos, $canales_especificos = null) {
        if (!isset(self::EVENTOS[$evento])) {
            return ['error' => __('Evento no registrado', FLAVOR_PLATFORM_TEXT_DOMAIN)];
        }
    ...
    private function enviar_por_canal($canal_id, $evento, $destinatarios, $datos) {
        ...
        return ['error' => __('Canal no implementado', FLAVOR_PLATFORM_TEXT_DOMAIN)];
    }
    ...
    private function enviar_push($evento, $destinatarios, $datos) {
        if (!class_exists('Flavor_Push_Notifications')) {
            return ['error' => __('Push notifications no disponibles', FLAVOR_PLATFORM_TEXT_DOMAIN)];
        }
...
<body>
    ...
    public function notificar_entrega_lista($entrega_id, $usuario_id) {
        ...
        $datos = [
            ...
            'mensaje' => __('Tu pedido está preparado y listo para recoger.', FLAVOR_PLATFORM_TEXT_DOMAIN),
            'enlace' => Flavor_Platform_Helpers::get_action_url('grupos-consumo', 'mis-pedidos'),
