...
class Flavor_GC_Subscriptions {
    ...
    private function verificar_nonce_ajax() {
        ...
        wp_send_json_error(
            ['mensaje' => __('Token de seguridad invalido. Recarga la pagina e intentalo de nuevo.', FLAVOR_PLATFORM_TEXT_DOMAIN)],
            403
        );
    }
    ...
    public function crear_suscripcion($consumidor_id, $tipo_cesta_id, $frecuencia = 'semanal', $opciones = []) {
        ...
        if (!$consumidor || $consumidor->estado !== 'activo') {
            return [
                'success' => false,
                'error' => __('El consumidor no está activo o no existe.', FLAVOR_PLATFORM_TEXT_DOMAIN),
            ];
        }
        ...
        if (!$cesta || !$cesta->activa) {
            return [
                'success' => false,
                'error' => __('El tipo de cesta no existe o no está disponible.', FLAVOR_PLATFORM_TEXT_DOMAIN),
            ];
        }
        ...
        if ($existente) {
            return [
                'success' => false,
                'error' => __('Ya tienes una suscripción activa a esta cesta.', FLAVOR_PLATFORM_TEXT_DOMAIN),
            ];
        }
        ...
        if ($resultado === false) {
            return [
                'success' => false,
                'error' => __('Error al crear la suscripción.', FLAVOR_PLATFORM_TEXT_DOMAIN),
            ];
        }
        ...
        return [
            ...
            'mensaje' => sprintf(
                __('Suscripción creada correctamente. Próxima cesta: %s', FLAVOR_PLATFORM_TEXT_DOMAIN),
    ...
    public function pausar_suscripcion($suscripcion_id) {
        ...
        if (!$suscripcion) {
            return [
                'success' => false,
                'error' => __('Suscripción no encontrada.', FLAVOR_PLATFORM_TEXT_DOMAIN),
            ];
        }
        ...
        if ($suscripcion->estado !== 'activa') {
            return [
                'success' => false,
                'error' => __('Solo se pueden pausar suscripciones activas.', FLAVOR_PLATFORM_TEXT_DOMAIN),
            ];
        }
        ...
        if ($resultado === false) {
            return [
                'success' => false,
                'error' => __('Error al pausar la suscripción.', FLAVOR_PLATFORM_TEXT_DOMAIN),
            ];
        }
        ...
        return [
            'success' => true,
            'mensaje' => __('Suscripción pausada correctamente.', FLAVOR_PLATFORM_TEXT_DOMAIN),
        ];
    }
    ...
    public function reanudar_suscripcion($suscripcion_id) {
        ...
        if ($suscripcion->estado !== 'pausada') {
            return [
                'success' => false,
                'error' => __('Solo se pueden reanudar suscripciones pausadas.', FLAVOR_PLATFORM_TEXT_DOMAIN),
            ];
        }
        ...
        if ($resultado === false) {
            return [
                'success' => false,
                'error' => __('Error al reanudar la suscripción.', FLAVOR_PLATFORM_TEXT_DOMAIN),
            ];
        }
        ...
        return [
            'success' => true,
            'mensaje' => sprintf(
                __('Suscripción reanudada. Próxima cesta: %s', FLAVOR_PLATFORM_TEXT_DOMAIN),
                date_i18n(get_option('date_format'), strtotime($fecha_proximo_cargo))
            ),
        ];
    }
    ...
    public function cancelar_suscripcion($suscripcion_id, $motivo = '') {
        ...
        if ($suscripcion->estado === 'cancelada') {
            return [
                'success' => false,
                'error' => __('La suscripción ya está cancelada.', FLAVOR_PLATFORM_TEXT_DOMAIN),
            ];
        }
        ...
        if ($resultado === false) {
            return [
                'success' => false,
                'error' => __('Error al cancelar la suscripción.', FLAVOR_PLATFORM_TEXT_DOMAIN),
            ];
        }
        ...
        return [
            'success' => true,
            'mensaje' => __('Suscripción cancelada correctamente.', FLAVOR_PLATFORM_TEXT_DOMAIN),
        ];
    }
    ...
    public function cambiar_frecuencia($suscripcion_id, $nueva_frecuencia) {
        ...
        if (!isset(self::FRECUENCIAS[$nueva_frecuencia])) {
            return [
                'success' => false,
                'error' => __('Frecuencia no válida.', FLAVOR_PLATFORM_TEXT_DOMAIN),
            ];
        }
        ...
        if (!$suscripcion || $suscripcion->estado === 'cancelada') {
            return [
                'success' => false,
                'error' => __('Suscripción no encontrada o cancelada.', FLAVOR_PLATFORM_TEXT_DOMAIN),
            ];
        }
        ...
        if ($resultado === false) {
            return [
                'success' => false,
                'error' => __('Error al cambiar la frecuencia.', FLAVOR_PLATFORM_TEXT_DOMAIN),
            ];
        }
        ...
        return [
            'success' => true,
            'mensaje' => sprintf(
                __('Frecuencia cambiada a %s. Próxima cesta: %s', FLAVOR_PLATFORM_TEXT_DOMAIN),
        ...
        foreach ($productos as $producto) {
            ...
            $wpdb->insert(
                ...
                [
                    ...
                    'notas' => sprintf(__('Pedido automático - Suscripción %s', FLAVOR_PLATFORM_TEXT_DOMAIN), $cesta->nombre),
                ],
                ['%d', '%d', '%d', '%f', '%f', '%s', '%s', '%s']
            );
    ...
    public function crear_tipo_cesta($datos) {
        ...
        if (empty($nombre)) {
            return [
                'success' => false,
                'error' => __('El nombre es obligatorio.', FLAVOR_PLATFORM_TEXT_DOMAIN),
            ];
        }
        ...
        if ($resultado === false) {
            return [
                'success' => false,
                'error' => __('Error al crear el tipo de cesta.', FLAVOR_PLATFORM_TEXT_DOMAIN),
            ];
        }
        ...
        return [
            ...
            'mensaje' => __('Tipo de cesta creado correctamente.', FLAVOR_PLATFORM_TEXT_DOMAIN),
        ];
    }
    ...
    public function actualizar_tipo_cesta($tipo_cesta_id, $datos) {
        ...
        if (!$cesta) {
            return [
                'success' => false,
                'error' => __('Tipo de cesta no encontrado.', FLAVOR_PLATFORM_TEXT_DOMAIN),
            ];
        }
        ...
        if ($resultado === false) {
            return [
                'success' => false,
                'error' => __('Error al actualizar.', FLAVOR_PLATFORM_TEXT_DOMAIN),
            ];
        }

        return [
            'success' => true,
            'mensaje' => __('Tipo de cesta actualizado.', FLAVOR_PLATFORM_TEXT_DOMAIN),
        ];
    }
    ...
    public function ajax_pausar_suscripcion() {
        ...
        if (!$suscripcion) {
            wp_send_json_error(['mensaje' => __('Suscripción no encontrada.', FLAVOR_PLATFORM_TEXT_DOMAIN)]);
        }
