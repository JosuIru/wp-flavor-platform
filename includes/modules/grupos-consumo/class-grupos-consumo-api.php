...
class Flavor_Grupos_Consumo_API {
    ...
    public function get_pedido($request) {
        ...
        if (!$post || $post->post_type !== 'pedido_colectivo') {
            return new WP_Error('not_found', __('Pedido no encontrado', FLAVOR_PLATFORM_TEXT_DOMAIN), ['status' => 404]);
        }
    ...
    public function unirse_pedido($request) {
        ...
        if ($estado !== 'abierto') {
            return new WP_Error('closed', __('El pedido no está abierto', FLAVOR_PLATFORM_TEXT_DOMAIN), ['status' => 400]);
        }
        ...
        foreach ($participantes as $part) {
            if ($part['user_id'] == $user_id) {
                return new WP_Error('already_joined', __('Ya participas en este pedido', FLAVOR_PLATFORM_TEXT_DOMAIN), ['status' => 400]);
            }
        }
        ...
        return new WP_REST_Response([
            'success' => true,
            'message' => __('¡Te has unido al pedido!', FLAVOR_PLATFORM_TEXT_DOMAIN),
            'data' => [
    ...
    public function marcar_pagado($request) {
        ...
        foreach ($participantes as $index => $part) {
            if ($part['user_id'] == $user_id) {
                ...
                return new WP_REST_Response([
                    'success' => true,
                    'message' => __('_precio_base', FLAVOR_PLATFORM_TEXT_DOMAIN),
                ], 200);
            }
        }

        return new WP_Error('not_participant', __('No participas en este pedido', FLAVOR_PLATFORM_TEXT_DOMAIN), ['status' => 400]);
    }
    ...
    public function actualizar_preferencias($request) {
        ...
        if (empty($grupos)) {
            return new WP_Error('no_grupo', __('No hay grupos disponibles', FLAVOR_PLATFORM_TEXT_DOMAIN), ['status' => 404]);
        }

        $consumidor = $consumidor_manager->obtener_consumidor($usuario_id, $grupos[0]->ID);

        if (!$consumidor) {
            return new WP_Error('no_miembro', __('No eres miembro del grupo', FLAVOR_PLATFORM_TEXT_DOMAIN), ['status' => 403]);
        }
    ...
    public function crear_suscripcion($request) {
        ...
        if (!$tipo_cesta_id) {
            return new WP_Error('missing_basket', __('Debes seleccionar una cesta valida', FLAVOR_PLATFORM_TEXT_DOMAIN), ['status' => 400]);
        }
    ...
    public function pausar_suscripcion($request) {
        ...
        if (!$this->verificar_propiedad_suscripcion($suscripcion_id)) {
            return new WP_Error('not_owner', __('No tienes permisos', FLAVOR_PLATFORM_TEXT_DOMAIN), ['status' => 403]);
        }
