...
class Flavor_GC_Gateway_WooCommerce extends Flavor_GC_Payment_Gateway {
    ...
    public function __construct() {
        $this->name = __('Checkout WooCommerce', FLAVOR_PLATFORM_TEXT_DOMAIN);
        $this->description = __('Usa tu método de pago favorito a través de WooCommerce.', FLAVOR_PLATFORM_TEXT_DOMAIN);
    ...
    public function process_payment(int $entrega_id, float $amount): array {
        if (!class_exists('WooCommerce')) {
            return [
                'success' => false,
                'error' => __('WooCommerce no está instalado.', FLAVOR_PLATFORM_TEXT_DOMAIN),
            ];
        }
        ...
        if (!$product_id) {
            return [
                'success' => false,
                'error' => __('Error al configurar el producto de pago.', FLAVOR_PLATFORM_TEXT_DOMAIN),
            ];
        }
        ...
        if (!$cart_item_key) {
            return [
                'success' => false,
                'error' => __('Error al agregar al carrito.', FLAVOR_PLATFORM_TEXT_DOMAIN),
            ];
        }
    ...
    private function get_or_create_gc_product(): ?int {
        ...
        $product->set_name(__('Pedido Grupos de Consumo', FLAVOR_PLATFORM_TEXT_DOMAIN));
    ...
    public function render_checkout_fields(): void {
        if (!class_exists('WooCommerce')) {
            echo '<p class="gc-wc-error">' . esc_html__('WooCommerce no está disponible.', FLAVOR_PLATFORM_TEXT_DOMAIN) . '</p>';
            return;
        }
        ...
        if (empty($available_gateways)) {
            echo '<p class="gc-wc-error">' . esc_html__('No hay métodos de pago configurados en WooCommerce.', FLAVOR_PLATFORM_TEXT_DOMAIN) . '</p>';
            return;
        }
        ...
        <div class="gc-gateway-wc-wrapper">
            <p class="gc-wc-message">
                ...
                <?php esc_html_e('Serás redirigido al checkout de WooCommerce donde podrás elegir tu método de pago preferido.', FLAVOR_PLATFORM_TEXT_DOMAIN); ?>
            </p>

            <div class="gc-wc-available-methods">
                <strong><?php esc_html_e('Métodos de pago disponibles:', FLAVOR_PLATFORM_TEXT_DOMAIN); ?></strong>
                <ul>
    ...
    public function get_admin_fields(): array {
        $fields = [
            [
                'id' => 'enabled',
                'type' => 'checkbox',
                'label' => __('Habilitar pago por WooCommerce', FLAVOR_PLATFORM_TEXT_DOMAIN),
                'default' => false,
            ],
        ];
        ...
        if (!class_exists('WooCommerce')) {
            $fields[] = [
                'id' => 'wc_notice',
                'type' => 'notice',
                'content' => __('WooCommerce no está instalado. Esta pasarela requiere WooCommerce activo.', FLAVOR_PLATFORM_TEXT_DOMAIN),
                'level' => 'warning',
            ];
        }
    ...
    public function process_refund(int $transaction_id, float $amount, string $reason = ''): array {
        ...
        if (!$wc_order_id) {
            return [
                'success' => false,
                'error' => __('No se encontró el pedido de WooCommerce asociado.', FLAVOR_PLATFORM_TEXT_DOMAIN),
            ];
        }
        ...
        if (!$order) {
            return [
                'success' => false,
                'error' => __('Pedido de WooCommerce no encontrado.', FLAVOR_PLATFORM_TEXT_DOMAIN),
            ];
        }
        ...
        return [
            ...
            'message' => __('Reembolso procesado a través de WooCommerce.', FLAVOR_PLATFORM_TEXT_DOMAIN),
        ];
    }
}
