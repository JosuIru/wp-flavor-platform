...
<div class="flavor-gc-carrito-flotante <?php echo $total_items > 0 ? 'tiene-items' : ''; ?>"
    ...
    <button type="button" class="flavor-gc-carrito-boton" id="gc-carrito-toggle" aria-label="<?php esc_attr_e('Ver carrito', FLAVOR_PLATFORM_TEXT_DOMAIN); ?>">
    ...
    <div class="flavor-gc-carrito-panel" id="gc-carrito-panel">
        <header class="flavor-gc-carrito-panel-header">
            ...
            <button type="button" class="flavor-gc-carrito-cerrar" id="gc-carrito-cerrar" aria-label="<?php esc_attr_e('Cerrar', FLAVOR_PLATFORM_TEXT_DOMAIN); ?>">
            ...
            <div class="flavor-gc-carrito-ciclo">
                ...
                <div class="flavor-gc-carrito-ciclo-info">
                    ...
                    <span class="flavor-gc-ciclo-cierre">
                        <?php printf(__('Cierre: %s', FLAVOR_PLATFORM_TEXT_DOMAIN), date_i18n('j M, H:i', strtotime($ciclo_activo['fecha_cierre']))); ?>
                    </span>
                </div>
            </div>
        <?php endif; ?>
        ...
        <div class="flavor-gc-carrito-contenido" id="gc-carrito-contenido">
            <?php if (empty($items_carrito)): ?>
                <div class="flavor-gc-carrito-vacio">
                    ...
                    <p><?php _e('Tu pedido esta vacio', FLAVOR_PLATFORM_TEXT_DOMAIN); ?></p>
                    <span class="flavor-gc-carrito-vacio-hint">
                        <?php _e('Anade productos desde el catalogo', FLAVOR_PLATFORM_TEXT_DOMAIN); ?>
                    </span>
                </div>
            <?php else: ?>
                <ul class="flavor-gc-carrito-items">
                    <?php foreach (array_slice($items_carrito, 0, 5) as $item): ?>
                        <li class="flavor-gc-carrito-item" data-item-id="<?php echo esc_attr($item['id']); ?>" data-producto-id="<?php echo esc_attr($item['producto_id']); ?>">
                            ...
                            <button type="button" class="flavor-gc-item-eliminar" data-action="eliminar-item" title="<?php esc_attr_e('Eliminar', FLAVOR_PLATFORM_TEXT_DOMAIN); ?>">
                ...
                <?php if (count($items_carrito) > 5): ?>
                    <div class="flavor-gc-carrito-mas">
                        <?php printf(__('+ %d productos mas', FLAVOR_PLATFORM_TEXT_DOMAIN), count($items_carrito) - 5); ?>
                    </div>
                <?php endif; ?>
            <?php endif; ?>
        </div>
        ...
        <?php if (!empty($items_carrito)): ?>
            <div class="flavor-gc-carrito-resumen">
                <div class="flavor-gc-resumen-linea">
                    <span><?php _e('Subtotal', FLAVOR_PLATFORM_TEXT_DOMAIN); ?></span>
                ...
                <?php if ($porcentaje_gestion > 0): ?>
                    <div class="flavor-gc-resumen-linea flavor-gc-gestion">
                        <span>
                            <?php printf(__('Gastos gestion (%s%%)', FLAVOR_PLATFORM_TEXT_DOMAIN), $porcentaje_gestion); ?>
                            <span class="dashicons dashicons-info flavor-gc-tooltip" title="<?php esc_attr_e('Porcentaje para cubrir gastos operativos del grupo', FLAVOR_PLATFORM_TEXT_DOMAIN); ?>"></span>
                        </span>
                ...
                <div class="flavor-gc-resumen-linea flavor-gc-total">
                    <strong><?php _e('Total', FLAVOR_PLATFORM_TEXT_DOMAIN); ?></strong>
            ...
            <footer class="flavor-gc-carrito-panel-footer">
                <a href="<?php echo esc_url($url_carrito); ?>" class="flavor-gc-btn flavor-gc-btn-secondary">
                    <?php _e('Ver pedido completo', FLAVOR_PLATFORM_TEXT_DOMAIN); ?>
                </a>
                <?php if ($ciclo_activo && $ciclo_activo['estado'] === 'abierto'): ?>
                    <button type="button" class="flavor-gc-btn flavor-gc-btn-primary" id="gc-confirmar-pedido-flotante">
                        <?php _e('Confirmar pedido', FLAVOR_PLATFORM_TEXT_DOMAIN); ?>
                    </button>
                <?php endif; ?>
            </footer>
