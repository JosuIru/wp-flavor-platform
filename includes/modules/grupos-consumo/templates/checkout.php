...
if (!is_user_logged_in()) {
    ...
    echo '<p>' . esc_html__('Debes iniciar sesión para continuar.', FLAVOR_PLATFORM_TEXT_DOMAIN) . '</p>';
    echo '<a href="' . esc_url(wp_login_url(Flavor_Platform_Helpers::get_action_url('grupos_consumo', 'checkout'))) . '" class="gc-btn gc-btn-primary">';
...
if (!$entrega_id) {
    echo '<div class="gc-checkout-error">';
    echo '<p>' . esc_html__('No se especificó una entrega válida.', FLAVOR_PLATFORM_TEXT_DOMAIN) . '</p>';
    echo '<a href="' . esc_url(Flavor_Platform_Helpers::get_action_url('grupos_consumo', 'mi-pedido')) . '" class="gc-btn gc-btn-secondary">';
    echo esc_html__('Ir a mi cesta', FLAVOR_PLATFORM_TEXT_DOMAIN);
    echo '</a></div>';
    return;
}
...
if (!$checkout_data) {
    echo '<div class="gc-checkout-error">';
    echo '<p>' . esc_html__('La entrega no existe o no tienes permisos para acceder.', FLAVOR_PLATFORM_TEXT_DOMAIN) . '</p>';
    echo '</div>';
    return;
}
...
if (!$entrega || (int) $entrega->usuario_id !== get_current_user_id()) {
    echo '<div class="gc-checkout-error">';
    echo '<p>' . esc_html__('No tienes permisos para acceder a esta entrega.', FLAVOR_PLATFORM_TEXT_DOMAIN) . '</p>';
    echo '</div>';
    return;
}
...
if ($checkout_data['estado_pago'] === 'completado') {
    ...
    echo '<h2>' . esc_html__('Pedido ya pagado', FLAVOR_PLATFORM_TEXT_DOMAIN) . '</h2>';
    echo '<p>' . esc_html__('Este pedido ya ha sido pagado correctamente.', FLAVOR_PLATFORM_TEXT_DOMAIN) . '</p>';
    echo '<a href="' . esc_url(Flavor_Platform_Helpers::get_action_url('grupos_consumo', 'mis-pedidos')) . '" class="gc-btn gc-btn-primary">';
    echo esc_html__('Ver mis pedidos', FLAVOR_PLATFORM_TEXT_DOMAIN);
    echo '</a></div>';
    return;
}
...
<div class="gc-checkout-container">
    <div class="gc-checkout-main">
        <h2 class="gc-checkout-title"><?php esc_html_e('Finalizar pedido', FLAVOR_PLATFORM_TEXT_DOMAIN); ?></h2>
        ...
        <div class="gc-checkout-summary">
            ...
            <div class="gc-checkout-ciclo">
                <strong><?php esc_html_e('Ciclo:', FLAVOR_PLATFORM_TEXT_DOMAIN); ?></strong>
            ...
            <table class="gc-checkout-items">
                <thead>
                    <tr>
                        ...
                        <th><?php esc_html_e('Subtotal', FLAVOR_PLATFORM_TEXT_DOMAIN); ?></th>
                    </tr>
                </thead>
                <tbody>
                ...
                <tfoot>
                    <tr class="gc-checkout-subtotal">
                        <td colspan="3"><?php esc_html_e('Subtotal', FLAVOR_PLATFORM_TEXT_DOMAIN); ?></td>
                        <td><?php echo number_format($checkout_data['subtotal'], 2, ',', '.'); ?> €</td>
                    </tr>
                    <?php if ($checkout_data['descuento'] > 0) : ?>
                    <tr class="gc-checkout-discount">
                        <td colspan="3"><?php esc_html_e('Descuento', FLAVOR_PLATFORM_TEXT_DOMAIN); ?></td>
                        <td>-<?php echo number_format($checkout_data['descuento'], 2, ',', '.'); ?> €</td>
                    </tr>
                    <?php endif; ?>
                    <tr class="gc-checkout-total">
                        <td colspan="3"><?php esc_html_e('Total a pagar', FLAVOR_PLATFORM_TEXT_DOMAIN); ?></td>
        ...
        <div class="gc-checkout-payment">
            <h3><?php esc_html_e('Método de pago', FLAVOR_PLATFORM_TEXT_DOMAIN); ?></h3>

            <?php if (empty($pasarelas)) : ?>
            <div class="gc-checkout-no-gateways">
                <p><?php esc_html_e('No hay métodos de pago disponibles. Contacta con el administrador.', FLAVOR_PLATFORM_TEXT_DOMAIN); ?></p>
            </div>
            <?php else : ?>
            ...
            <form id="gc-checkout-form" method="post" class="gc-payment-form">
                ...
                <div class="gc-checkout-actions">
                    <a href="<?php echo esc_url(Flavor_Platform_Helpers::get_action_url('grupos_consumo', 'mi-pedido')); ?>" class="gc-btn gc-btn-secondary">
                        <?php esc_html_e('Volver a la cesta', FLAVOR_PLATFORM_TEXT_DOMAIN); ?>
                    </a>
                    <button type="submit" class="gc-btn gc-btn-primary gc-btn-pay" id="gc-btn-pay">
                        <span class="gc-btn-text"><?php esc_html_e('Pagar', FLAVOR_PLATFORM_TEXT_DOMAIN); ?> <?php echo number_format($checkout_data['total'], 2, ',', '.'); ?> €</span>
                        <span class="gc-btn-loading" style="display: none;">
                            <span class="gc-spinner"></span>
                            <?php esc_html_e('Procesando...', FLAVOR_PLATFORM_TEXT_DOMAIN); ?>
                        </span>
                    </button>
                </div>
            </form>
    ...
    <aside class="gc-checkout-sidebar">
        <div class="gc-checkout-secure">
            ...
            <span><?php esc_html_e('Pago seguro', FLAVOR_PLATFORM_TEXT_DOMAIN); ?></span>
        </div>

        <div class="gc-checkout-help">
            <h4><?php esc_html_e('¿Necesitas ayuda?', FLAVOR_PLATFORM_TEXT_DOMAIN); ?></h4>
            <p><?php esc_html_e('Si tienes dudas sobre el proceso de pago, contacta con nosotros.', FLAVOR_PLATFORM_TEXT_DOMAIN); ?></p>
        </div>
    </aside>
</div>
...
(function($) {
    ...
    function loadGatewayFields(gatewayId) {
        ...
        $.ajax({
            ...
            error: function() {
                $gatewayFields.html('<p class="gc-error"><?php echo esc_js(__('Error al cargar el formulario.', FLAVOR_PLATFORM_TEXT_DOMAIN)); ?></p>');
            }
        });
    }
    ...
    $form.on('submit', function(e) {
        ...
        $.ajax({
            ...
            success: function(response) {
                if (response.success) {
                    if (response.data.redirect_url) {
                        ...
                        window.location.href = '<?php echo esc_url(Flavor_Platform_Helpers::get_action_url('grupos_consumo', 'mis-pedidos') . '?payment=success'); ?>';
                    }
                } else {
                    mostrarAviso(response.data.error || '<?php echo esc_js(__('Error al procesar el pago.', FLAVOR_PLATFORM_TEXT_DOMAIN)); ?>', 'error');
            ...
            error: function() {
                mostrarAviso('<?php echo esc_js(__('Error de conexión. Inténtalo de nuevo.', FLAVOR_PLATFORM_TEXT_DOMAIN)); ?>', 'error');
                $btn.prop('disabled', false);
                $btn.find('.gc-btn-text').show();
                $btn.find('.gc-btn-loading').hide();
            }
