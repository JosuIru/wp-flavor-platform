...
<div class="gc-excedentes" data-nonce="<?php echo esc_attr($nonce); ?>">
    <div class="gc-excedentes__header">
        <h3 class="gc-excedentes__titulo">
            ...
            <?php esc_html_e('Excedentes Solidarios', FLAVOR_PLATFORM_TEXT_DOMAIN); ?>
        </h3>
        <p class="gc-excedentes__descripcion">
            <?php esc_html_e('Productos sobrantes a precio reducido o gratuito. Evitamos el desperdicio y creamos comunidad.', FLAVOR_PLATFORM_TEXT_DOMAIN); ?>
        </p>
    </div>
    ...
    <?php if (empty($excedentes)): ?>
        <div class="gc-excedentes__vacio">
            ...
            <p><?php esc_html_e('No hay excedentes disponibles en este momento. Todos los productos han encontrado hogar.', FLAVOR_PLATFORM_TEXT_DOMAIN); ?></p>
        </div>
    <?php else: ?>
                    ...
                    <div class="gc-excedente-card__contenido">
                            ...
                            <form class="gc-excedente-card__form gc-reclamar-form">
                                ...
                                <div class="gc-excedente-card__input-group">
                                    <label for="cantidad-<?php echo esc_attr($excedente['id']); ?>" class="screen-reader-text">
                                        <?php esc_html_e('Cantidad', FLAVOR_PLATFORM_TEXT_DOMAIN); ?>
                                    </label>
                                    <input
                                        type="number"
                        ...
                        <?php else: ?>
                            <p class="gc-excedente-card__login">
                                <a href="<?php echo esc_url(wp_login_url(Flavor_Platform_Helpers::get_action_url('grupos_consumo', ''))); ?>">
        ...
        container.addEventListener('submit', function(e) {
            ...
            .then(data => {
                if (data.success) {
                    ...
                    btn.disabled = false;
                    btn.innerHTML = '<span class="dashicons dashicons-yes"></span> <?php echo esc_js(__('Reclamar', FLAVOR_PLATFORM_TEXT_DOMAIN)); ?>';
                }
            })
            .catch(() => {
                if (window.gcToast) {
                    window.gcToast('<?php echo esc_js(__('Error de conexión', FLAVOR_PLATFORM_TEXT_DOMAIN)); ?>', 'error');
                }
                btn.disabled = false;
                btn.innerHTML = '<span class="dashicons dashicons-yes"></span> <?php echo esc_js(__('Reclamar', FLAVOR_PLATFORM_TEXT_DOMAIN)); ?>';
            });
