...
if (!is_user_logged_in()) {
    ...
    echo '<p>' . esc_html__('Inicia sesión para ver tus pedidos.', FLAVOR_PLATFORM_TEXT_DOMAIN) . '</p>';
    echo '<a href="' . esc_url(wp_login_url(Flavor_Platform_Helpers::get_action_url('grupos_consumo', 'mis-pedidos'))) . '" class="gc-btn gc-btn-primary">';
...
if ($payment_status === 'success') {
    $mensaje = '<div class="gc-notice gc-notice-success"><span class="dashicons dashicons-yes"></span> ' . esc_html__('Pago realizado correctamente.', FLAVOR_PLATFORM_TEXT_DOMAIN) . '</div>';
} elseif ($payment_status === 'cancelled') {
    $mensaje = '<div class="gc-notice gc-notice-warning"><span class="dashicons dashicons-info"></span> ' . esc_html__('El pago fue cancelado.', FLAVOR_PLATFORM_TEXT_DOMAIN) . '</div>';
}
?>
...
<div class="gc-pedidos-container">
    ...
    <div class="gc-pedidos-empty">
        <span class="dashicons dashicons-clipboard"></span>
        <p><?php esc_html_e('Aún no tienes ningún pedido.', FLAVOR_PLATFORM_TEXT_DOMAIN); ?></p>
        ...
        <?php foreach ($entregas as $entrega) :
            ...
            switch ($entrega->estado_pago) {
                case 'completado':
                    ...
                    $estado_pago_label = __('Pagado', FLAVOR_PLATFORM_TEXT_DOMAIN);
                    break;
                case 'pendiente':
                case 'pendiente_recogida':
                    $estado_pago_class = 'gc-status-warning';
                    $estado_pago_label = __('Pendiente', FLAVOR_PLATFORM_TEXT_DOMAIN);
                    break;
                case 'procesando':
                    $estado_pago_class = 'gc-status-info';
                    $estado_pago_label = __('Procesando', FLAVOR_PLATFORM_TEXT_DOMAIN);
                    break;
                default:
            ...
            switch ($entrega->estado_recogida ?? 'pendiente') {
                case 'recogido':
                    ...
                    $estado_recogida_label = __('Recogido', FLAVOR_PLATFORM_TEXT_DOMAIN);
                    break;
                case 'pendiente':
                    $estado_recogida_class = 'gc-status-warning';
                    $estado_recogida_label = __('Por recoger', FLAVOR_PLATFORM_TEXT_DOMAIN);
                    break;
                default:
        ...
        <div class="gc-pedido-card <?php echo $pedido_destacado_id === (int) $entrega->id ? 'gc-pedido-card--destacado' : ''; ?>"
            ...
            <div class="gc-pedido-header">
                <div class="gc-pedido-info">
                    ...
                    <span class="gc-pedido-ciclo"><?php echo esc_html($entrega->ciclo_nombre ?: __('Ciclo', FLAVOR_PLATFORM_TEXT_DOMAIN)); ?></span>
                </div>
            ...
            <div class="gc-pedido-body">
                <div class="gc-pedido-items">
                    ...
                    <div class="gc-pedido-mas">
                        +<?php echo (count($items) - 3); ?> <?php esc_html_e('más', FLAVOR_PLATFORM_TEXT_DOMAIN); ?>
                    </div>
                    <?php endif; ?>
                </div>
                ...
                <div class="gc-pedido-meta">
                    ...
                    <div class="gc-meta-item">
                        ...
                        <?php esc_html_e('Pedido:', FLAVOR_PLATFORM_TEXT_DOMAIN); ?>
            ...
            <div class="gc-pedido-footer">
                <div class="gc-pedido-total">
                    <span class="gc-total-label"><?php esc_html_e('Total:', FLAVOR_PLATFORM_TEXT_DOMAIN); ?></span>
            ...
            <div class="gc-pedido-details" id="gc-details-<?php echo esc_attr($entrega->id); ?>" style="display: none;">
                <h4><?php esc_html_e('Detalle del pedido', FLAVOR_PLATFORM_TEXT_DOMAIN); ?></h4>
                <table class="gc-details-table">
                    ...
                    <tfoot>
                        <tr>
                            <td colspan="3"><?php esc_html_e('Total pedido', FLAVOR_PLATFORM_TEXT_DOMAIN); ?></td>
                ...
                <div class="gc-pedido-notas">
                    <strong><?php esc_html_e('Notas:', FLAVOR_PLATFORM_TEXT_DOMAIN); ?></strong>
    ...
    $('.gc-btn-toggle-details').on('click', function() {
        ...
        $details.slideToggle(200, function() {
            if ($details.is(':visible')) {
                $btn.text('<?php echo esc_js(__('Ocultar detalles', FLAVOR_PLATFORM_TEXT_DOMAIN)); ?>');
            } else {
                $btn.text('<?php echo esc_js(__('Ver detalles', FLAVOR_PLATFORM_TEXT_DOMAIN)); ?>');
            }
        });
    });
