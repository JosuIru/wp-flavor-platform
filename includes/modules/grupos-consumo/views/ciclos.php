...
<div class="wrap dm-dashboard">
    <div class="dm-header">
        <div class="dm-header__content">
            <h1 class="dm-header__title">
                ...
                <?php esc_html_e('Ciclos de Pedido', FLAVOR_PLATFORM_TEXT_DOMAIN); ?>
            </h1>
            <p class="dm-header__description">
                <?php esc_html_e('Seguimiento del estado de apertura, cierre y logística de cada ciclo.', FLAVOR_PLATFORM_TEXT_DOMAIN); ?>
            </p>
        </div>
    ...
    <div class="dm-stats-grid" style="margin-bottom:16px;">
        ...
        <div class="dm-stat-card dm-stat-card--success">
            <div class="dm-stat-card__content">
                ...
                <div class="dm-stat-card__label"><?php esc_html_e('Abiertos / activos', FLAVOR_PLATFORM_TEXT_DOMAIN); ?></div>
            </div>
        </div>
        <div class="dm-stat-card dm-stat-card--warning">
            <div class="dm-stat-card__content">
                <div class="dm-stat-card__value"><?php echo number_format_i18n($total_cerrados); ?></div>
                <div class="dm-stat-card__label"><?php esc_html_e('Cerrados', FLAVOR_PLATFORM_TEXT_DOMAIN); ?></div>
            </div>
        </div>
    ...
    <form method="get" class="dm-card" style="padding:16px; margin-bottom:16px; display:flex; gap:10px; align-items:end;">
        ...
        <div>
            <label for="gc-ciclos-estado"><strong><?php esc_html_e('Estado del ciclo', FLAVOR_PLATFORM_TEXT_DOMAIN); ?></strong></label>
            <select id="gc-ciclos-estado" name="estado">
                ...
                <option value="cerrado" <?php selected($filtro_estado, 'cerrado'); ?>><?php esc_html_e('Cerrado', FLAVOR_PLATFORM_TEXT_DOMAIN); ?></option>
                <option value="borrador" <?php selected($filtro_estado, 'borrador'); ?>><?php esc_html_e('Borrador', FLAVOR_PLATFORM_TEXT_DOMAIN); ?></option>
            </select>
        </div>
        <div>
    ...
    <div class="dm-card" style="padding:0; overflow:auto;">
        <table class="wp-list-table widefat fixed striped">
            <thead>
                <tr>
                    <th><?php esc_html_e('Ciclo', FLAVOR_PLATFORM_TEXT_DOMAIN); ?></th>
                    <th><?php esc_html_e('Estado', FLAVOR_PLATFORM_TEXT_DOMAIN); ?></th>
                    <th><?php esc_html_e('Apertura', FLAVOR_PLATFORM_TEXT_DOMAIN); ?></th>
                    <th><?php esc_html_e('Cierre', FLAVOR_PLATFORM_TEXT_DOMAIN); ?></th>
                    <th><?php esc_html_e('Entrega', FLAVOR_PLATFORM_TEXT_DOMAIN); ?></th>
                    <th><?php esc_html_e('Pedidos', FLAVOR_PLATFORM_TEXT_DOMAIN); ?></th>
            ...
            <tbody>
                <?php if ($ciclos_query->have_posts()) : ?>
                    <?php while ($ciclos_query->have_posts()) : $ciclos_query->the_post(); ?>
                        ...
                        <tr>
                            ...
                            <td>
                                <a class="button button-small" href="<?php echo esc_url(get_edit_post_link($ciclo_id)); ?>"><?php esc_html_e('Editar', FLAVOR_PLATFORM_TEXT_DOMAIN); ?></a>
                                <a class="button button-small" href="<?php echo esc_url(admin_url('admin.php?page=gc-pedidos&ciclo=' . $ciclo_id)); ?>"><?php esc_html_e('Pedidos', FLAVOR_PLATFORM_TEXT_DOMAIN); ?></a>
                            </td>
                        </tr>
                    <?php endwhile; ?>
                ...
                <?php else : ?>
                    <tr>
                        <td colspan="7"><?php esc_html_e('No hay ciclos para los filtros seleccionados.', FLAVOR_PLATFORM_TEXT_DOMAIN); ?></td>
                    </tr>
                <?php endif; ?>
            </tbody>
        </table>
