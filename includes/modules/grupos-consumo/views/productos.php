...
<div class="wrap dm-dashboard">
    <div class="dm-header">
        <div class="dm-header__content">
            <h1 class="dm-header__title">
                ...
                <?php esc_html_e('Productos del Grupo de Consumo', FLAVOR_PLATFORM_TEXT_DOMAIN); ?>
            </h1>
            <p class="dm-header__description">
                <?php esc_html_e('Catálogo interno de productos, precios y disponibilidad por productor.', FLAVOR_PLATFORM_TEXT_DOMAIN); ?>
            </p>
        </div>
        <div class="dm-header__actions">
            <a href="<?php echo esc_url(admin_url('post-new.php?post_type=gc_producto')); ?>" class="dm-btn dm-btn--primary">
                <span class="dashicons dashicons-plus-alt2"></span> <?php esc_html_e('Nuevo producto', FLAVOR_PLATFORM_TEXT_DOMAIN); ?>
            </a>
    ...
    <div class="dm-stats-grid" style="margin-bottom:16px;">
        ...
        <div class="dm-stat-card dm-stat-card--pink">
            <div class="dm-stat-card__content">
                ...
                <div class="dm-stat-card__label"><?php esc_html_e('Sin stock', FLAVOR_PLATFORM_TEXT_DOMAIN); ?></div>
            </div>
        </div>
    ...
    <form method="get" class="dm-card" style="padding:16px; margin-bottom:16px; display:grid; gap:12px; grid-template-columns: 1.4fr 1fr 1fr auto; align-items:end;">
        ...
        <div>
            <label for="gc-productos-search"><strong><?php esc_html_e('Buscar', FLAVOR_PLATFORM_TEXT_DOMAIN); ?></strong></label>
            <input id="gc-productos-search" type="search" name="s" value="<?php echo esc_attr($search); ?>" class="regular-text" style="width:100%;" placeholder="<?php esc_attr_e('Título o descripción', FLAVOR_PLATFORM_TEXT_DOMAIN); ?>" />
        </div>
        <div>
            <label for="gc-productos-productor"><strong><?php esc_html_e('Productor', FLAVOR_PLATFORM_TEXT_DOMAIN); ?></strong></label>
            <select id="gc-productos-productor" name="productor" style="width:100%;">
                <option value="0"><?php esc_html_e('Todos', FLAVOR_PLATFORM_TEXT_DOMAIN); ?></option>
        ...
        <div>
            <label for="gc-productos-estado"><strong><?php esc_html_e('Estado', FLAVOR_PLATFORM_TEXT_DOMAIN); ?></strong></label>
        ...
        <table class="wp-list-table widefat fixed striped">
            <thead>
                <tr>
                    ...
                    <th><?php esc_html_e('Unidad', FLAVOR_PLATFORM_TEXT_DOMAIN); ?></th>
                    <th><?php esc_html_e('Stock', FLAVOR_PLATFORM_TEXT_DOMAIN); ?></th>
            ...
            <tbody>
                <?php if ($productos_query->have_posts()) : ?>
                    <?php while ($productos_query->have_posts()) : $productos_query->the_post(); ?>
                        ...
                        $productor_nombre = $productor_id ? get_the_title($productor_id) : __('Sin asignar', FLAVOR_PLATFORM_TEXT_DOMAIN);
                        ?>
                        <tr>
                            <td>
                            ...
                            <td>
                                <?php if ($stock === '' || $stock === null) : ?>
                                    <span class="dm-badge dm-badge--info"><?php esc_html_e('Ilimitado', FLAVOR_PLATFORM_TEXT_DOMAIN); ?></span>
                                <?php else : ?>
                            ...
                            <td>
                                <a class="button button-small" href="<?php echo esc_url(get_edit_post_link($producto_id)); ?>"><?php esc_html_e('Editar', FLAVOR_PLATFORM_TEXT_DOMAIN); ?></a>
                                <a class="button button-small" href="<?php echo esc_url(get_permalink($producto_id)); ?>" target="_blank" rel="noopener"><?php esc_html_e('Ver', FLAVOR_PLATFORM_TEXT_DOMAIN); ?></a>
                            </td>
                        </tr>
                    <?php endwhile; ?>
                ...
                <?php else : ?>
                    <tr>
                        <td colspan="7"><?php esc_html_e('No hay productos para los filtros seleccionados.', FLAVOR_PLATFORM_TEXT_DOMAIN); ?></td>
                    </tr>
                <?php endif; ?>
            </tbody>
        </table>
