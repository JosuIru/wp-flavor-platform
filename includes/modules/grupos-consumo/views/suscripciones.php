...
<div class="wrap gc-admin-suscripciones">
    <h1 class="wp-heading-inline">
        <?php _e('Gestión de Suscripciones', FLAVOR_PLATFORM_TEXT_DOMAIN); ?>
    </h1>
    <button type="button" class="page-title-action gc-modal-trigger" data-modal="modal-gestionar-cestas">
        <?php _e('Gestionar Cestas', FLAVOR_PLATFORM_TEXT_DOMAIN); ?>
    </button>
    ...
    <div class="gc-stats-grid">
        ...
        <div class="gc-stat-card gc-stat-ingresos">
            ...
            <span class="gc-stat-label"><?php _e('Ingresos/mes estimados', FLAVOR_PLATFORM_TEXT_DOMAIN); ?></span>
        </div>
    </div>
    ...
    <div class="gc-distribucion-cestas">
        <h3><?php _e('Distribución por tipo de cesta', FLAVOR_PLATFORM_TEXT_DOMAIN); ?></h3>
    ...
    <div class="gc-filtros-wrapper">
        <form method="get" class="gc-filtros-form">
            <input type="hidden" name="page" value="<?php echo esc_attr__('gc-suscripciones', FLAVOR_PLATFORM_TEXT_DOMAIN); ?>">
            ...
            <div class="gc-filtro">
                ...
                <select id="filtro-estado" name="estado">
                    ...
                    <option value="<?php echo esc_attr__('activa', FLAVOR_PLATFORM_TEXT_DOMAIN); ?>" <?php selected($estado_filtro, 'activa'); ?>><?php _e('Activa', FLAVOR_PLATFORM_TEXT_DOMAIN); ?></option>
                    <option value="<?php echo esc_attr__('pausada', FLAVOR_PLATFORM_TEXT_DOMAIN); ?>" <?php selected($estado_filtro, 'pausada'); ?>><?php _e('Pausada', FLAVOR_PLATFORM_TEXT_DOMAIN); ?></option>
                    <option value="<?php echo esc_attr__('cancelada', FLAVOR_PLATFORM_TEXT_DOMAIN); ?>" <?php selected($estado_filtro, 'cancelada'); ?>><?php _e('Cancelada', FLAVOR_PLATFORM_TEXT_DOMAIN); ?></option>
                </select>
            </div>

            <button type="submit" class="button"><?php _e('Filtrar', FLAVOR_PLATFORM_TEXT_DOMAIN); ?></button>
            <a href="<?php echo admin_url('admin.php?page=gc-suscripciones'); ?>" class="button"><?php _e('Limpiar', FLAVOR_PLATFORM_TEXT_DOMAIN); ?></a>
        </form>
    </div>
    ...
    <table class="wp-list-table widefat fixed striped gc-tabla-suscripciones">
        <thead>
            <tr>
                <th scope="col" class="column-usuario"><?php _e('Usuario', FLAVOR_PLATFORM_TEXT_DOMAIN); ?></th>
                <th scope="col" class="column-cesta"><?php _e('Cesta', FLAVOR_PLATFORM_TEXT_DOMAIN); ?></th>
                <th scope="col" class="column-frecuencia"><?php _e('Frecuencia', FLAVOR_PLATFORM_TEXT_DOMAIN); ?></th>
                <th scope="col" class="column-importe"><?php _e('Importe', FLAVOR_PLATFORM_TEXT_DOMAIN); ?></th>
                <th scope="col" class="column-estado"><?php _e('Estado', FLAVOR_PLATFORM_TEXT_DOMAIN); ?></th>
                <th scope="col" class="column-proximo"><?php _e('Próximo Cargo', FLAVOR_PLATFORM_TEXT_DOMAIN); ?></th>
        ...
        <tbody>
            <?php if (empty($suscripciones)): ?>
                <tr>
                    <td colspan="7"><?php _e('No se encontraron suscripciones.', FLAVOR_PLATFORM_TEXT_DOMAIN); ?></td>
                </tr>
            <?php else: ?>
...
<div id="modal-gestionar-cestas" class="gc-modal" style="display:none;">
    <div class="gc-modal-content gc-modal-lg">
        <div class="gc-modal-header">
            <h2><?php _e('Tipos de Cestas', FLAVOR_PLATFORM_TEXT_DOMAIN); ?></h2>
            ...
            <table class="wp-list-table widefat fixed striped">
                <thead>
                    <tr>
                        ...
                        <th><?php _e('Precio Base', FLAVOR_PLATFORM_TEXT_DOMAIN); ?></th>
                        ...
                        <tr>
                            ...
                            <td>
                                <span class="gc-estado-badge <?php echo $cesta->activa ? 'gc-estado-activo' : 'gc-estado-inactivo'; ?>">
                                    <?php echo $cesta->activa ? __('Activa', FLAVOR_PLATFORM_TEXT_DOMAIN) : __('Inactiva', FLAVOR_PLATFORM_TEXT_DOMAIN); ?>
                                </span>
                            </td>
                            <td>
...
jQuery(document).ready(function($) {
    ...
    $('.gc-pausar-suscripcion').on('click', function() {
        var id = $(this).data('id');
        gcConfirmar('<?php echo esc_js(__('¿Pausar esta suscripción?', FLAVOR_PLATFORM_TEXT_DOMAIN)); ?>', function() {
            $.post(ajaxurl, {
    ...
    $('.gc-cancelar-suscripcion').on('click', function() {
        var id = $(this).data('id');
        gcConfirmar('<?php echo esc_js(__('¿Cancelar esta suscripción? Esta acción no se puede deshacer.', FLAVOR_PLATFORM_TEXT_DOMAIN)); ?>', function() {
            $.post(ajaxurl, {
