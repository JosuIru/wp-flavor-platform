...
<div class="jr-mediadores">
    <header class="jr-mediadores__header">
        <h2><?php esc_html_e('Mediadores Voluntarios', 'flavor-chat-ia'); ?></h2>
        <p><?php esc_html_e('Personas formadas en facilitación de diálogos y resolución de conflictos', 'flavor-chat-ia'); ?></p>
    </header>
        ...
        <article class="jr-mediador-card">
            ...
            <div class="jr-mediador-card__stats">
                <div class="jr-mediador-card__stat">
                    ...
                    <div class="jr-mediador-card__stat-label"><?php esc_html_e('Procesos', 'flavor-chat-ia'); ?></div>
                </div>
                <div class="jr-mediador-card__stat">
                    <div class="jr-mediador-card__stat-valor"><?php echo esc_html($acuerdos_alcanzados); ?></div>
                    <div class="jr-mediador-card__stat-label"><?php esc_html_e('Acuerdos', 'flavor-chat-ia'); ?></div>
                </div>
            </div>
        </article>
        <?php endforeach; ?>
    ...
    <div class="jr-empty-state">
        ...
        <p><?php esc_html_e('Aún no hay mediadores registrados.', 'flavor-chat-ia'); ?></p>
    </div>
    <?php endif; ?>

    <?php if (is_user_logged_in() && !$es_mediador && !$solicitud_pendiente) : ?>
    <div class="jr-cta-mediador">
        <h3><?php esc_html_e('¿Te gustaría ser mediador/a?', 'flavor-chat-ia'); ?></h3>
        <p><?php esc_html_e('Si tienes formación o experiencia en mediación, resolución de conflictos o facilitación, únete a nuestro equipo.', 'flavor-chat-ia'); ?></p>

        <button class="jr-btn" onclick="document.getElementById('jr-form-mediador').scrollIntoView({behavior: 'smooth'})">
            <?php esc_html_e('Quiero ser mediador/a', 'flavor-chat-ia'); ?>
        </button>
    </div>
    ...
    <section id="jr-form-mediador" class="jr-solicitar" style="margin-top: 3rem;">
        <form class="jr-solicitar__form jr-form-mediador">
            <header class="jr-solicitar__header">
                <h2><?php esc_html_e('Solicitar ser mediador/a', 'flavor-chat-ia'); ?></h2>
            </header>

            <div class="jr-form-grupo">
                <label for="jr-formacion"><?php esc_html_e('Formación y experiencia', 'flavor-chat-ia'); ?></label>
                <textarea name="formacion" id="jr-formacion" rows="4" required
                          placeholder="<?php esc_attr_e('Describe tu formación en mediación, facilitación o resolución de conflictos...', 'flavor-chat-ia'); ?>"></textarea>
            </div>

            <div class="jr-form-grupo">
                <label for="jr-motivacion"><?php esc_html_e('¿Por qué quieres ser mediador/a?', 'flavor-chat-ia'); ?></label>
                <textarea name="motivacion" id="jr-motivacion" rows="3"
                          placeholder="<?php esc_attr_e('Cuéntanos tu motivación...', 'flavor-chat-ia'); ?>"></textarea>
            </div>
    ...
    <div class="jr-aviso-confidencial" style="max-width: 600px; margin: 2rem auto;">
        ...
        <p><?php esc_html_e('Tu solicitud para ser mediador/a está siendo revisada. Nos pondremos en contacto contigo pronto.', 'flavor-chat-ia'); ?></p>
    </div>
    ...
    <div class="jr-aviso-confidencial" style="max-width: 600px; margin: 2rem auto; border-color: var(--jr-success); background: color-mix(in srgb, var(--jr-success) 10%, #fff);">
        ...
        <p><?php esc_html_e('Eres parte del equipo de mediadores. ¡Gracias por tu contribución!', 'flavor-chat-ia'); ?></p>
    </div>
    <?php endif; ?>
</div>
