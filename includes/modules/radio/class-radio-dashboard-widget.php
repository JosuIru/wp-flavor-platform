...
class Flavor_Radio_Dashboard_Widget extends Flavor_Dashboard_Widget_Base {
    ...
    public function __construct() {
        ...
        $this->title = __('Radio', 'flavor-chat-ia');
        $this->description = __('Radio comunitaria en vivo', 'flavor-chat-ia');
    ...
    private function fetch_widget_data(): array {
        ...
        if ($emision_actual) {
            $stats[] = [
                ...
                'color' => 'success',
                'url' => $es_admin ? admin_url('admin.php?page=flavor-radio-dashboard') : home_url('/mi-portal/radio/'),
            ];
        } else {
            $stats[] = [
                'icon' => 'dashicons-controls-volumeon',
                'valor' => __('Sin emisión', 'flavor-chat-ia'),
        ...
        $stats[] = [
            ...
            'url' => $es_admin ? admin_url('admin.php?page=flavor-radio-programas') : home_url('/mi-portal/radio/programas/'),
        ];
        ...
        return [
            'stats' => $stats,
            'items' => $items,
            'empty_state' => __('No hay programas de radio disponibles', 'flavor-chat-ia'),
            'footer' => [
                [
                    'label' => __('Escuchar radio', 'flavor-chat-ia'),
                    'url' => $es_admin ? admin_url('admin.php?page=flavor-radio-dashboard') : home_url('/mi-portal/radio/'),
                    'icon' => 'dashicons-controls-play',
                ],
            ],
    ...
    private function get_programas_destacados(int $limite): array {
        ...
        foreach ($programas as $programa) {
            $items[] = [
                ...
                'url' => $es_admin ? admin_url('admin.php?page=flavor-radio-programas') : home_url('/mi-portal/radio/programa/' . $programa->id . '/'),
            ];
        }
