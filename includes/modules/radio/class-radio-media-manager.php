...
class Flavor_Radio_Media_Manager {
    ...
    public function ajax_upload_audio() {
        ...
        if (!current_user_can('upload_files')) {
            wp_send_json_error(['message' => __('No tienes permisos para subir archivos', FLAVOR_PLATFORM_TEXT_DOMAIN)]);
        }
        ...
        wp_send_json_success([
            'message' => __('Archivo subido correctamente', FLAVOR_PLATFORM_TEXT_DOMAIN),
            'audio' => $result,
        ]);
    }
    ...
    public function upload_audio_file($file, $tipo = 'cancion', $programa_id = 0) {
        ...
        if (!in_array($extension, $this->allowed_formats)) {
            return new WP_Error('invalid_format', __('Formato de audio no permitido', FLAVOR_PLATFORM_TEXT_DOMAIN));
        }

        // Validar tamaño
        if ($file['size'] > $this->max_file_size) {
            return new WP_Error('file_too_large', __('El archivo excede el tamaño máximo (100MB)', FLAVOR_PLATFORM_TEXT_DOMAIN));
        }
        ...
        if (!move_uploaded_file($file['tmp_name'], $target_path)) {
            return new WP_Error('upload_failed', __('Error al guardar el archivo', FLAVOR_PLATFORM_TEXT_DOMAIN));
        }
    ...
    public function ajax_delete_audio() {
        ...
        wp_send_json_success(['message' => __('Audio eliminado', FLAVOR_PLATFORM_TEXT_DOMAIN)]);
    }
    ...
    public function ajax_create_playlist() {
        ...
        wp_send_json_success([
            'message' => __('Playlist guardada', FLAVOR_PLATFORM_TEXT_DOMAIN),
    ...
    public function ajax_get_playlist() {
        ...
        if (!$playlist) {
            wp_send_json_error(['message' => __('Playlist no encontrada', FLAVOR_PLATFORM_TEXT_DOMAIN)]);
        }
    ...
    public function get_current_track() {
        ...
        if (!$programacion) {
            ...
            if (!$playlist_default) {
                return [
                    'playing' => false,
                    'message' => __('No hay programación activa', FLAVOR_PLATFORM_TEXT_DOMAIN),
                ];
            }
        ...
        if (!$playlist) {
            return [
                'playing' => false,
                'message' => __('Playlist no disponible', FLAVOR_PLATFORM_TEXT_DOMAIN),
            ];
        }
        ...
        if (empty($audios_ids)) {
            return [
                'playing' => false,
                'message' => __('Playlist vacía', FLAVOR_PLATFORM_TEXT_DOMAIN),
            ];
        }
        ...
        if (!$audio) {
            return [
                'playing' => false,
                'message' => __('Audio no disponible', FLAVOR_PLATFORM_TEXT_DOMAIN),
            ];
        }
