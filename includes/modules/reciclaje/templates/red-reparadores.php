...
<div class="rec-reparadores" data-nonce="<?php echo esc_attr($nonce); ?>">
    <div class="rec-reparadores__header">
        ...
        <div>
            <h3><?php esc_html_e('Red de Reparadores', FLAVOR_PLATFORM_TEXT_DOMAIN); ?></h3>
            <p><?php esc_html_e('Conecta con vecinos que pueden reparar tus objetos', FLAVOR_PLATFORM_TEXT_DOMAIN); ?></p>
        </div>
    </div>
    ...
    <div class="rec-reparadores__panel rec-reparadores__panel--active" id="tab-buscar">
        ...
        <div class="rec-reparadores__filtros">
            <select class="rec-reparadores__filtro-esp" id="filtro-especialidad">
                <option value=""><?php esc_html_e('Todas las especialidades', FLAVOR_PLATFORM_TEXT_DOMAIN); ?></option>
        ...
        <?php if (empty($reparadores)): ?>
            <div class="rec-reparadores__vacio">
                <span class="dashicons dashicons-admin-tools"></span>
                <p><?php esc_html_e('No hay reparadores disponibles.', FLAVOR_PLATFORM_TEXT_DOMAIN); ?></p>
                <p class="rec-reparadores__vacio-sub"><?php esc_html_e('¡Sé el primero en ofrecer tus habilidades!', FLAVOR_PLATFORM_TEXT_DOMAIN); ?></p>
            </div>
        <?php else: ?>
                    ...
                    <div class="rec-reparadores__card" data-id="<?php echo esc_attr($rep->id); ?>" data-esp="<?php echo esc_attr($rep->especialidad); ?>">
                        ...
                        <div class="rec-reparadores__stats">
                            <span class="rec-reparadores__stat">
                                ...
                                <?php printf(esc_html__('%d reparaciones', FLAVOR_PLATFORM_TEXT_DOMAIN), $rep->total_reparaciones ?? 0); ?>
                            </span>
    ...
    <div class="rec-reparadores__panel" id="tab-solicitudes">
        <?php if (empty($mis_solicitudes)): ?>
            <div class="rec-reparadores__vacio">
                ...
                <p><?php esc_html_e('No tienes solicitudes de reparación.', FLAVOR_PLATFORM_TEXT_DOMAIN); ?></p>
            </div>
        <?php else: ?>
            <div class="rec-reparadores__solicitudes">
                <?php foreach ($mis_solicitudes as $sol):
                    ...
                    $estados = [
                        'pendiente' => ['label' => __('Pendiente', FLAVOR_PLATFORM_TEXT_DOMAIN), 'color' => '#ff9800'],
                        'aceptada' => ['label' => __('Aceptada', FLAVOR_PLATFORM_TEXT_DOMAIN), 'color' => '#2196f3'],
                        'en_proceso' => ['label' => __('En proceso', FLAVOR_PLATFORM_TEXT_DOMAIN), 'color' => '#9c27b0'],
                        'completada' => ['label' => __('Completada', FLAVOR_PLATFORM_TEXT_DOMAIN), 'color' => '#4caf50'],
                        'cancelada' => ['label' => __('Cancelada', FLAVOR_PLATFORM_TEXT_DOMAIN), 'color' => '#f44336'],
                    ];
                        ...
                        <div class="rec-reparadores__solicitud-header">
                            ...
                            <div class="rec-reparadores__solicitud-info">
                                ...
                                <span class="rec-reparadores__reparador-nombre">
                                    <?php printf(esc_html__('Reparador: %s', FLAVOR_PLATFORM_TEXT_DOMAIN), $sol->reparador_nombre); ?>
                                </span>
                            </div>
    ...
    <div class="rec-reparadores__panel" id="tab-ofrecer">
        <form class="rec-reparadores__form" id="form-ofrecer-reparacion">
            <div class="rec-reparadores__form-intro">
                ...
                <p><?php esc_html_e('Comparte tus habilidades con la comunidad y ayuda a reducir residuos.', FLAVOR_PLATFORM_TEXT_DOMAIN); ?></p>
            </div>

            <div class="rec-reparadores__field">
                <label><?php esc_html_e('Especialidad principal', FLAVOR_PLATFORM_TEXT_DOMAIN); ?></label>
                <select name="especialidad" required>
                    <option value=""><?php esc_html_e('Selecciona...', FLAVOR_PLATFORM_TEXT_DOMAIN); ?></option>
                    <option value="electronica"><?php esc_html_e('Electrónica', FLAVOR_PLATFORM_TEXT_DOMAIN); ?></option>
                    <option value="ropa"><?php esc_html_e('Ropa/Costura', FLAVOR_PLATFORM_TEXT_DOMAIN); ?></option>
                    <option value="muebles"><?php esc_html_e('Muebles', FLAVOR_PLATFORM_TEXT_DOMAIN); ?></option>
                    <option value="bicicletas"><?php esc_html_e('Bicicletas', FLAVOR_PLATFORM_TEXT_DOMAIN); ?></option>
                    <option value="electrodomesticos"><?php esc_html_e('Electrodomésticos', FLAVOR_PLATFORM_TEXT_DOMAIN); ?></option>
                    <option value="otros"><?php esc_html_e('Otros', FLAVOR_PLATFORM_TEXT_DOMAIN); ?></option>
                </select>
            </div>

            <div class="rec-reparadores__field">
                <label><?php esc_html_e('Descripción de tus habilidades', FLAVOR_PLATFORM_TEXT_DOMAIN); ?></label>
                <textarea name="descripcion" rows="3" required placeholder="<?php esc_attr_e('Ej: Reparo móviles, tablets y pequeños electrodomésticos...', FLAVOR_PLATFORM_TEXT_DOMAIN); ?>"></textarea>
            </div>
            ...
            <div class="rec-reparadores__field">
                <label>
                    ...
                    <?php esc_html_e('Acepto ofrecer mis servicios de forma solidaria o a precios justos', FLAVOR_PLATFORM_TEXT_DOMAIN); ?>
                </label>
            </div>

            <button type="submit" class="rec-btn rec-btn--primary rec-btn--full">
                <span class="dashicons dashicons-yes"></span>
                <?php esc_html_e('Unirme a la red', FLAVOR_PLATFORM_TEXT_DOMAIN); ?>
            </button>
        </form>
    </div>
    ...
    <div class="rec-reparadores__impacto-global">
        <h4><?php esc_html_e('Impacto de nuestra red', FLAVOR_PLATFORM_TEXT_DOMAIN); ?></h4>
        <div class="rec-reparadores__impacto-grid">
            <div class="rec-reparadores__impacto-item">
                ...
                <span class="rec-reparadores__impacto-label"><?php esc_html_e('Reparadores', FLAVOR_PLATFORM_TEXT_DOMAIN); ?></span>
            </div>
            <div class="rec-reparadores__impacto-item">
                <?php
                ...
                <span class="rec-reparadores__impacto-label"><?php esc_html_e('Reparaciones', FLAVOR_PLATFORM_TEXT_DOMAIN); ?></span>
            </div>
            <div class="rec-reparadores__impacto-item">
                <?php
                ...
                <span class="rec-reparadores__impacto-label"><?php esc_html_e('kg CO₂ ahorrados', FLAVOR_PLATFORM_TEXT_DOMAIN); ?></span>
            </div>
        </div>
    </div>
</div>
