...
if ('POST' === $_SERVER['REQUEST_METHOD'] && isset($_POST['reciclaje_guardar_campana'])) {
    if (!isset($_POST['reciclaje_campana_nonce']) || !wp_verify_nonce(sanitize_text_field(wp_unslash($_POST['reciclaje_campana_nonce'])), 'reciclaje_guardar_campana')) {
        add_settings_error('reciclaje_campanas', 'nonce_error', __('No se pudo validar la solicitud.', FLAVOR_PLATFORM_TEXT_DOMAIN), 'error');
    } else {
        ...
        if ('' === $titulo) {
            add_settings_error('reciclaje_campanas', 'titulo_error', __('El título es obligatorio.', FLAVOR_PLATFORM_TEXT_DOMAIN), 'error');
        } elseif ($fecha_inicio && $fecha_fin && strtotime($fecha_fin) < strtotime($fecha_inicio)) {
            add_settings_error('reciclaje_campanas', 'fecha_error', __('La fecha de fin debe ser posterior a la fecha de inicio.', FLAVOR_PLATFORM_TEXT_DOMAIN), 'error');
        } else {
            $data = [
                'titulo' => $titulo,
            ...
            if ($edit_id > 0) {
                ...
                if (false !== $updated) {
                    add_settings_error('reciclaje_campanas', 'updated', __('Campaña actualizada correctamente.', FLAVOR_PLATFORM_TEXT_DOMAIN), 'updated');
                    $action = '';
                } else {
                    add_settings_error('reciclaje_campanas', 'update_error', __('No se pudo actualizar la campaña.', FLAVOR_PLATFORM_TEXT_DOMAIN), 'error');
                }
            } else {
                ...
                if ($inserted) {
                    add_settings_error('reciclaje_campanas', 'created', __('Campaña creada correctamente.', FLAVOR_PLATFORM_TEXT_DOMAIN), 'updated');
                    $action = '';
                } else {
                    add_settings_error('reciclaje_campanas', 'insert_error', __('No se pudo crear la campaña.', FLAVOR_PLATFORM_TEXT_DOMAIN), 'error');
                }
            }
        }
    }
...
if ('delete' === $action && $campana_id > 0 && isset($_GET['_wpnonce'])) {
    ...
    if (wp_verify_nonce($nonce, 'reciclaje_delete_campana_' . $campana_id)) {
        ...
        if ($deleted) {
            add_settings_error('reciclaje_campanas', 'deleted', __('Campaña eliminada.', FLAVOR_PLATFORM_TEXT_DOMAIN), 'updated');
        } else {
            add_settings_error('reciclaje_campanas', 'delete_error', __('No se pudo eliminar la campaña.', FLAVOR_PLATFORM_TEXT_DOMAIN), 'error');
        }
    } else {
        add_settings_error('reciclaje_campanas', 'delete_nonce_error', __('No se pudo validar la eliminación.', FLAVOR_PLATFORM_TEXT_DOMAIN), 'error');
    }
    $action = '';
}
...
if ('cambiar_estado' === $action && $campana_id > 0 && isset($_GET['estado'], $_GET['_wpnonce'])) {
    ...
    if (in_array($nuevo_estado, $estados_validos, true) && wp_verify_nonce($nonce, 'reciclaje_estado_campana_' . $campana_id)) {
        ...
        if (false !== $updated) {
            add_settings_error('reciclaje_campanas', 'estado_updated', __('Estado actualizado.', FLAVOR_PLATFORM_TEXT_DOMAIN), 'updated');
        } else {
            add_settings_error('reciclaje_campanas', 'estado_error', __('No se pudo actualizar el estado.', FLAVOR_PLATFORM_TEXT_DOMAIN), 'error');
        }
    }
    $action = '';
}
...
if ('editar' === $action && $campana_id > 0) {
    ...
    if (!$campana_edicion) {
        add_settings_error('reciclaje_campanas', 'not_found', __('La campaña no existe.', FLAVOR_PLATFORM_TEXT_DOMAIN), 'error');
        $action = '';
    }
}
...
<div class="wrap flavor-modulo-page">
    ...
    $this->render_page_header(__('Campañas de Reciclaje', FLAVOR_PLATFORM_TEXT_DOMAIN), [
        ['label' => __('Nueva Campaña', FLAVOR_PLATFORM_TEXT_DOMAIN), 'url' => admin_url('admin.php?page=' . $page_slug . '&action=nueva'), 'class' => 'button-primary'],
    ]);

    settings_errors('reciclaje_campanas');
    ?>
    <p><?php esc_html_e('Gestiona campañas de concienciación, retos comunitarios y acciones colectivas de reciclaje.', FLAVOR_PLATFORM_TEXT_DOMAIN); ?></p>

    <div class="flavor-stats-grid" style="margin-bottom:16px;">
        <div class="flavor-stat-card"><span class="stat-number"><?php echo esc_html(number_format_i18n($total_campanas)); ?></span><span class="stat-label"><?php esc_html_e('Campañas', FLAVOR_PLATFORM_TEXT_DOMAIN); ?></span></div>
        <div class="flavor-stat-card"><span class="stat-number"><?php echo esc_html(number_format_i18n($total_activas)); ?></span><span class="stat-label"><?php esc_html_e('Activas', FLAVOR_PLATFORM_TEXT_DOMAIN); ?></span></div>
        <div class="flavor-stat-card"><span class="stat-number"><?php echo esc_html(number_format_i18n($total_finalizadas)); ?></span><span class="stat-label"><?php esc_html_e('Finalizadas', FLAVOR_PLATFORM_TEXT_DOMAIN); ?></span></div>
        <div class="flavor-stat-card"><span class="stat-number"><?php echo esc_html(number_format_i18n($objetivo_total_kg, 1)); ?> kg</span><span class="stat-label"><?php esc_html_e('Objetivo acumulado', FLAVOR_PLATFORM_TEXT_DOMAIN); ?></span></div>
        <div class="flavor-stat-card"><span class="stat-number"><?php echo esc_html(number_format_i18n($kg_mes, 1)); ?> kg</span><span class="stat-label"><?php esc_html_e('Kg verificados este mes', FLAVOR_PLATFORM_TEXT_DOMAIN); ?></span></div>
    </div>
    ...
    <?php if ('nueva' === $action || ('editar' === $action && $campana_edicion)) : ?>
        ...
        <div class="postbox" style="padding:16px; margin-bottom:16px;">
            <h2><?php echo $campana_edicion ? esc_html__('Editar campaña', FLAVOR_PLATFORM_TEXT_DOMAIN) : esc_html__('Nueva campaña', FLAVOR_PLATFORM_TEXT_DOMAIN); ?></h2>
            <form method="post" action="<?php echo esc_url(admin_url('admin.php?page=' . $page_slug)); ?>">
                ...
                <table class="form-table" role="presentation">
                    <tr>
                        <th scope="row"><label for="campana_titulo"><?php esc_html_e('Título', FLAVOR_PLATFORM_TEXT_DOMAIN); ?></label></th>
                        <td><input id="campana_titulo" type="text" class="regular-text" name="titulo" required value="<?php echo esc_attr($form_titulo); ?>" /></td>
                    </tr>
                    <tr>
                        <th scope="row"><label for="campana_descripcion"><?php esc_html_e('Descripción', FLAVOR_PLATFORM_TEXT_DOMAIN); ?></label></th>
                    ...
                    <tr>
                        <th scope="row"><label for="campana_fecha_inicio"><?php esc_html_e('Inicio', FLAVOR_PLATFORM_TEXT_DOMAIN); ?></label></th>
                        <td><input id="campana_fecha_inicio" type="datetime-local" name="fecha_inicio" value="<?php echo esc_attr($form_inicio); ?>" /></td>
                    </tr>
                    <tr>
                        <th scope="row"><label for="campana_fecha_fin"><?php esc_html_e('Fin', FLAVOR_PLATFORM_TEXT_DOMAIN); ?></label></th>
                        <td><input id="campana_fecha_fin" type="datetime-local" name="fecha_fin" value="<?php echo esc_attr($form_fin); ?>" /></td>
                    </tr>
                    <tr>
                        <th scope="row"><label for="campana_objetivo_kg"><?php esc_html_e('Objetivo (kg)', FLAVOR_PLATFORM_TEXT_DOMAIN); ?></label></th>
                        <td><input id="campana_objetivo_kg" type="number" name="objetivo_kg" min="0" step="0.1" value="<?php echo esc_attr($form_objetivo_kg); ?>" /></td>
                    </tr>
                    <tr>
                        <th scope="row"><label for="campana_objetivo_participantes"><?php esc_html_e('Objetivo participantes', FLAVOR_PLATFORM_TEXT_DOMAIN); ?></label></th>
                    ...
                    <tr>
                        <th scope="row"><label for="campana_puntos_bonus"><?php esc_html_e('Puntos bonus', FLAVOR_PLATFORM_TEXT_DOMAIN); ?></label></th>
                        <td><input id="campana_puntos_bonus" type="number" name="puntos_bonus" min="0" step="1" value="<?php echo esc_attr($form_puntos_bonus); ?>" /></td>
                    </tr>
                    <tr>
                        <th scope="row"><label for="campana_estado"><?php esc_html_e('Estado', FLAVOR_PLATFORM_TEXT_DOMAIN); ?></label></th>
                        <td>
                    ...
                    <tr>
                        <th scope="row"><label for="campana_ambito"><?php esc_html_e('Ámbito', FLAVOR_PLATFORM_TEXT_DOMAIN); ?></label></th>
                        <td>
                    ...
                    <tr>
                        <th scope="row"><label for="campana_materiales"><?php esc_html_e('Materiales (CSV)', FLAVOR_PLATFORM_TEXT_DOMAIN); ?></label></th>
                        <td>
                            <input id="campana_materiales" type="text" class="regular-text" name="materiales" value="<?php echo esc_attr($form_materiales); ?>" />
                            <p class="description"><?php esc_html_e('Ejemplo: papel, plastico, vidrio, organico', FLAVOR_PLATFORM_TEXT_DOMAIN); ?></p>
                        </td>
                    </tr>
                    <tr>
                        <th scope="row"><label for="campana_ubicacion"><?php esc_html_e('Ubicación', FLAVOR_PLATFORM_TEXT_DOMAIN); ?></label></th>
                ...
                <p class="submit" style="display:flex; gap:8px;">
                    <button type="submit" name="reciclaje_guardar_campana" class="button button-primary"><?php esc_html_e('Guardar campaña', FLAVOR_PLATFORM_TEXT_DOMAIN); ?></button>
                    <a href="<?php echo esc_url(admin_url('admin.php?page=' . $page_slug)); ?>" class="button"><?php esc_html_e('Cancelar', FLAVOR_PLATFORM_TEXT_DOMAIN); ?></a>
                </p>
            </form>
        </div>
    <?php endif; ?>
    ...
    <form method="get" class="postbox" style="padding:12px; margin-bottom:16px; display:flex; gap:8px; align-items:flex-end;">
        ...
        <div>
            <label for="campanas_estado_filtro"><strong><?php esc_html_e('Estado', FLAVOR_PLATFORM_TEXT_DOMAIN); ?></strong></label><br>
        ...
        <div>
            <label for="campanas_search"><strong><?php esc_html_e('Buscar', FLAVOR_PLATFORM_TEXT_DOMAIN); ?></strong></label><br>
            <input id="campanas_search" type="search" name="s" value="<?php echo esc_attr($buscar); ?>" placeholder="<?php esc_attr_e('Título, descripción, ubicación', FLAVOR_PLATFORM_TEXT_DOMAIN); ?>" />
        </div>
        <div>
    ...
    <table class="wp-list-table widefat fixed striped">
        <thead>
            <tr>
                <th><?php esc_html_e('Campaña', FLAVOR_PLATFORM_TEXT_DOMAIN); ?></th>
                <th><?php esc_html_e('Periodo', FLAVOR_PLATFORM_TEXT_DOMAIN); ?></th>
                <th><?php esc_html_e('Objetivo', FLAVOR_PLATFORM_TEXT_DOMAIN); ?></th>
                <th><?php esc_html_e('Progreso', FLAVOR_PLATFORM_TEXT_DOMAIN); ?></th>
        ...
        <tbody>
                ...
                <?php foreach ($campanas as $campana) : ?>
                    ...
                    <tr>
                        ...
                        <td>
                            ...
                            <div class="description"><?php echo esc_html(number_format_i18n((int) $campana->objetivo_participantes)); ?> <?php esc_html_e('participantes', FLAVOR_PLATFORM_TEXT_DOMAIN); ?></div>
                        </td>
                        <td>
                            ...
                            <div class="description"><?php echo esc_html(number_format_i18n($participantes)); ?> <?php esc_html_e('participantes', FLAVOR_PLATFORM_TEXT_DOMAIN); ?></div>
                        </td>
                        <td>
                        ...
                        <td>
                            <a class="button button-small" href="<?php echo esc_url($edit_url); ?>"><?php esc_html_e('Editar', FLAVOR_PLATFORM_TEXT_DOMAIN); ?></a>
                            <a class="button button-small" href="<?php echo esc_url($delete_url); ?>" onclick="return confirm('<?php echo esc_js(__('¿Eliminar campaña?', FLAVOR_PLATFORM_TEXT_DOMAIN)); ?>');"><?php esc_html_e('Eliminar', FLAVOR_PLATFORM_TEXT_DOMAIN); ?></a>
            ...
            <?php else : ?>
                <tr>
                    <td colspan="6"><?php esc_html_e('No hay campañas con los filtros seleccionados.', FLAVOR_PLATFORM_TEXT_DOMAIN); ?></td>
                </tr>
            <?php endif; ?>
        </tbody>
    </table>
