...
class Flavor_Platform_Trading_IA_Module extends Flavor_Platform_Module_Base {
    ...
    public function get_activation_error() {
        if (!$this->can_activate()) {
            return __('Las tablas del Trading IA no estan creadas. Se crearan automaticamente al activar el plugin.', FLAVOR_PLATFORM_TEXT_DOMAIN);
        }
...
/**
    ...
    public function render_experimental_disabled_notice() {
        ...
        echo esc_html__('Trading IA está deshabilitado por defecto por ser un módulo experimental. Actívalo en ajustes avanzados si quieres usarlo.', FLAVOR_PLATFORM_TEXT_DOMAIN);
        echo '</p></div>';
    }
    ...
    public function render_admin_dashboard() {
        $this->render_page_header(__('Trading IA - Dashboard', FLAVOR_PLATFORM_TEXT_DOMAIN));
        echo '<div class="wrap trading-ia-admin-dashboard">';
        echo '<p>' . esc_html__('Panel de control del bot de trading simulado.', FLAVOR_PLATFORM_TEXT_DOMAIN) . '</p>';
    ...
    public function render_admin_operaciones() {
        $this->render_page_header(
            __('Trading IA - Operaciones', FLAVOR_PLATFORM_TEXT_DOMAIN),
            array(
                array(
                    'label' => __('Nueva operacion', FLAVOR_PLATFORM_TEXT_DOMAIN),
                    'url'   => '#nueva-operacion',
        ...
        echo '<p>' . esc_html__('Historial y gestion de operaciones de trading.', FLAVOR_PLATFORM_TEXT_DOMAIN) . '</p>';
    ...
    public function render_admin_configuracion() {
        $this->render_page_header(__('Trading IA - Configuracion', FLAVOR_PLATFORM_TEXT_DOMAIN));
        echo '<div class="wrap trading-ia-admin-configuracion">';
        echo '<p>' . esc_html__('Configuracion del bot de trading y parametros de riesgo.', FLAVOR_PLATFORM_TEXT_DOMAIN) . '</p>';
    ...
    public function render_dashboard_widget() {
        ...
        $estado_bot = $bot_activo ? __('Activo', FLAVOR_PLATFORM_TEXT_DOMAIN) : __('Inactivo', FLAVOR_PLATFORM_TEXT_DOMAIN);
        ?>
        <div class="trading-ia-widget">
            <p><strong><?php esc_html_e('Estado del bot:', FLAVOR_PLATFORM_TEXT_DOMAIN); ?></strong> <?php echo esc_html($estado_bot); ?></p>
            <p><strong><?php esc_html_e('Posiciones abiertas:', FLAVOR_PLATFORM_TEXT_DOMAIN); ?></strong> <?php echo esc_html($posiciones_abiertas); ?></p>
    ...
    public function registrar_cron_schedule($schedules) {
        ...
        $schedules[self::CRON_SCHEDULE] = array(
            ...
            'display'  => sprintf(
                __('Trading IA - Cada %d segundos', FLAVOR_PLATFORM_TEXT_DOMAIN),
                $intervalo_segundos
            ),
        );
    ...
    private function action_obtener_indicadores($parametros) {
        ...
        if (empty($token_solicitado)) {
            return array(
                'success' => false,
                'error'   => __('Debes especificar un token (ej: SOL, BONK)', FLAVOR_PLATFORM_TEXT_DOMAIN),
            );
        }
        ...
        if (!$datos_token || empty($datos_token['precio_usd'])) {
            return array(
                'success' => false,
                'error'   => sprintf(__('No se pudo obtener el precio de %s', FLAVOR_PLATFORM_TEXT_DOMAIN), $token_solicitado),
            );
        }
    ...
    private function action_ejecutar_compra_manual($parametros) {
        ...
        if (empty($token_compra)) {
            return array(
                'success' => false,
                'error'   => __('Debes especificar el token a comprar', FLAVOR_PLATFORM_TEXT_DOMAIN),
            );
        }
        ...
        if ($cantidad_usd <= 0) {
            return array(
                'success' => false,
                'error'   => __('La cantidad en USD debe ser mayor a 0', FLAVOR_PLATFORM_TEXT_DOMAIN),
            );
        }
        ...
        if (!$datos_token || empty($datos_token['precio_usd'])) {
            return array(
                'success' => false,
                'error'   => sprintf(__('No se pudo obtener el precio de %s', FLAVOR_PLATFORM_TEXT_DOMAIN), $token_compra),
            );
        }
    ...
    private function action_ejecutar_venta_manual($parametros) {
        ...
        if (empty($token_venta)) {
            return array(
                'success' => false,
                'error'   => __('Debes especificar el token a vender', FLAVOR_PLATFORM_TEXT_DOMAIN),
            );
        }
    ...
    private function action_iniciar_bot($parametros) {
        if ($this->get_setting('bot_activo', false)) {
            return array(
                'success' => true,
                'mensaje' => __('El bot ya esta activo.', FLAVOR_PLATFORM_TEXT_DOMAIN),
            );
        }
        ...
        return array(
            'success' => true,
            'mensaje' => sprintf(
                __('Bot de trading iniciado. Ciclo de analisis cada %d segundos. IMPORTANTE: Para intervalos menores a 60 segundos, configura un cron real del servidor.', FLAVOR_PLATFORM_TEXT_DOMAIN),
    ...
    private function action_detener_bot($parametros) {
        ...
        return array(
            'success' => true,
            'mensaje' => __('Bot de trading detenido.', FLAVOR_PLATFORM_TEXT_DOMAIN),
        );
    }
    ...
    private function action_crear_regla($parametros) {
        ...
        if (empty($nombre_regla)) {
            return array(
                'success' => false,
                'error'   => __('Debes especificar un nombre para la regla', FLAVOR_PLATFORM_TEXT_DOMAIN),
            );
        }
        ...
        if (!in_array($operador_regla, $operadores_validos, true)) {
            return array(
                'success' => false,
                'error'   => sprintf(
                    __('Operador invalido. Usa: %s', FLAVOR_PLATFORM_TEXT_DOMAIN),
        ...
        if (!in_array($accion_tipo_regla, $acciones_validas, true)) {
            return array(
                'success' => false,
                'error'   => sprintf(
                    __('Tipo de accion invalido. Usa: %s', FLAVOR_PLATFORM_TEXT_DOMAIN),
    ...
    private function action_eliminar_regla($parametros) {
        ...
        if (empty($regla_id)) {
            return array(
                'success' => false,
                'error'   => __('Debes especificar el ID de la regla a eliminar', FLAVOR_PLATFORM_TEXT_DOMAIN),
            );
        }
        ...
        return array(
            'success' => $eliminada,
            'mensaje' => $eliminada
                ? __('Regla eliminada correctamente', FLAVOR_PLATFORM_TEXT_DOMAIN)
                : __('No se pudo eliminar la regla', FLAVOR_PLATFORM_TEXT_DOMAIN),
        );
    }
    ...
    private function action_actualizar_parametros($parametros) {
        ...
        return array(
            ...
            'mensaje' => sprintf(
                __('%d parametros actualizados', FLAVOR_PLATFORM_TEXT_DOMAIN),
    ...
    private function action_reset_paper_trading($parametros) {
        ...
        return array(
            'success' => true,
            'mensaje' => sprintf(
                __('Paper trading reiniciado con balance de $%.2f', FLAVOR_PLATFORM_TEXT_DOMAIN),
                $balance_inicial
            ),
        );
    }
    ...
    public function ajax_agregar_token() {
        ...
        if (empty($token_nuevo)) {
            wp_send_json(array('success' => false, 'error' => __('Token invalido', FLAVOR_PLATFORM_TEXT_DOMAIN)));
        }

        $tokens_actuales = $this->get_setting('tokens_monitoreados', array());

        if (in_array($token_nuevo, $tokens_actuales, true)) {
            wp_send_json(array('success' => false, 'error' => __('El token ya esta en la lista', FLAVOR_PLATFORM_TEXT_DOMAIN)));
        }
        ...
        wp_send_json(array(
            'success' => true,
            'mensaje' => sprintf(__('Token %s agregado al monitoreo', FLAVOR_PLATFORM_TEXT_DOMAIN), $token_nuevo),
    ...
    public function ajax_eliminar_token() {
        ...
        wp_send_json(array(
            'success' => true,
            'mensaje' => sprintf(__('Token %s eliminado del monitoreo', FLAVOR_PLATFORM_TEXT_DOMAIN), $token_eliminar),
    ...
    public function ajax_exportar_historial() {
        ...
        if (empty($trades)) {
            wp_send_json(array('success' => false, 'error' => __('No hay trades para exportar', FLAVOR_PLATFORM_TEXT_DOMAIN)));
        }
    ...
    public function shortcode_dashboard($atts) {
        ...
        if (!is_user_logged_in()) {
            return '<div class="trading-ia-login-required">' .
                   '<p>' . __('Debes iniciar sesion para ver el dashboard de trading.', FLAVOR_PLATFORM_TEXT_DOMAIN) . '</p>' .
                   '<a href="' . wp_login_url(flavor_current_request_url()) . '" class="button">' . __('Iniciar sesion', FLAVOR_PLATFORM_TEXT_DOMAIN) . '</a>' .
                   '</div>';
        }
        ...
        <div class="trading-ia-dashboard" data-mostrar-graficos="<?php echo esc_attr($atts['mostrar_graficos']); ?>">
            <div class="trading-ia-header">
                <h2><?php _e('Trading IA - Paper Trading', FLAVOR_PLATFORM_TEXT_DOMAIN); ?></h2>
                <div class="trading-ia-bot-status <?php echo $estado['estado']['bot_activo'] ? 'activo' : 'inactivo'; ?>">
                    <span class="status-indicator"></span>
                    <span class="status-text">
                        <?php echo $estado['estado']['bot_activo'] ? __('Bot Activo', FLAVOR_PLATFORM_TEXT_DOMAIN) : __('Bot Inactivo', FLAVOR_PLATFORM_TEXT_DOMAIN); ?>
                    </span>
                </div>
            </div>
            ...
            <div class="trading-ia-grid">
                <div class="trading-ia-card balance-card">
                    ...
                    <div class="balance-details">
                        ...
                        <span class="pnl <?php echo ($portfolio['portfolio']['pnl_total'] ?? 0) >= 0 ? 'positivo' : 'negativo'; ?>">
                            <?php _e('P&L:', FLAVOR_PLATFORM_TEXT_DOMAIN); ?>
                ...
                <div class="trading-ia-card stats-card">
                    <h3><?php _e('Estadisticas', FLAVOR_PLATFORM_TEXT_DOMAIN); ?></h3>
                    <div class="stats-grid">
                        <div class="stat">
                            <span class="label"><?php _e('Posiciones', FLAVOR_PLATFORM_TEXT_DOMAIN); ?></span>
                            <span class="value"><?php echo count($portfolio['portfolio']['posiciones'] ?? array()); ?></span>
                        </div>
                        <div class="stat">
                            <span class="label"><?php _e('Agresividad', FLAVOR_PLATFORM_TEXT_DOMAIN); ?></span>
                            <span class="value"><?php echo $estado['estado']['agresividad']; ?>/10</span>
                        </div>
                        <div class="stat">
                            <span class="label"><?php _e('Confianza min.', FLAVOR_PLATFORM_TEXT_DOMAIN); ?></span>
                ...
                <div class="trading-ia-card posiciones-card">
                    <h3><?php _e('Posiciones Abiertas', FLAVOR_PLATFORM_TEXT_DOMAIN); ?></h3>
                    <div class="posiciones-lista">
                        <?php if (!empty($portfolio['portfolio']['posiciones'])): ?>
                            ...
                            <?php endforeach; ?>
                        <?php else: ?>
                            <p class="sin-posiciones"><?php _e('Sin posiciones abiertas', FLAVOR_PLATFORM_TEXT_DOMAIN); ?></p>
                        <?php endif; ?>
                    </div>
                </div>

                <div class="trading-ia-card trading-form-card">
                    <h3><?php _e('Operar', FLAVOR_PLATFORM_TEXT_DOMAIN); ?></h3>
                    <form class="trading-form" id="trading-ia-form">
                        <div class="form-row">
                            <label for="trading-token"><?php _e('Token', FLAVOR_PLATFORM_TEXT_DOMAIN); ?></label>
                        ...
                        <div class="form-row">
                            <label for="trading-cantidad"><?php _e('Cantidad USD', FLAVOR_PLATFORM_TEXT_DOMAIN); ?></label>
                            <input type="number" id="trading-cantidad" name="cantidad_usd" step="0.01" min="1" placeholder="100.00">
                        </div>
                        <div class="form-actions">
                            <button type="button" class="btn-comprar" data-action="comprar"><?php _e('Comprar', FLAVOR_PLATFORM_TEXT_DOMAIN); ?></button>
                            <button type="button" class="btn-vender" data-action="vender"><?php _e('Vender Todo', FLAVOR_PLATFORM_TEXT_DOMAIN); ?></button>
                        </div>
                    </form>
                </div>
            </div>
    ...
    public function shortcode_portfolio($atts) {
        if (!is_user_logged_in()) {
            return '<p class="trading-ia-login-required">' . __('Inicia sesion para ver tu portfolio.', FLAVOR_PLATFORM_TEXT_DOMAIN) . '</p>';
        }
        ...
        <div class="trading-ia-portfolio">
            <h3><?php _e('Mi Portfolio', FLAVOR_PLATFORM_TEXT_DOMAIN); ?></h3>
            <div class="portfolio-resumen">
                <div class="balance-total">
                    <span class="label"><?php _e('Balance Total', FLAVOR_PLATFORM_TEXT_DOMAIN); ?></span>
                    <span class="valor">$<?php echo number_format($portfolio['portfolio']['balance_total_usd'] ?? 0, 2); ?></span>
                </div>
                <div class="pnl-total">
                    <span class="label"><?php _e('Ganancia/Perdida', FLAVOR_PLATFORM_TEXT_DOMAIN); ?></span>
            ...
            <table class="portfolio-tabla">
                <thead>
                    <tr>
                        <th><?php _e('Token', FLAVOR_PLATFORM_TEXT_DOMAIN); ?></th>
                        <th><?php _e('Cantidad', FLAVOR_PLATFORM_TEXT_DOMAIN); ?></th>
                        <th><?php _e('Precio Entrada', FLAVOR_PLATFORM_TEXT_DOMAIN); ?></th>
                        <th><?php _e('Precio Actual', FLAVOR_PLATFORM_TEXT_DOMAIN); ?></th>
                        <th><?php _e('Valor', FLAVOR_PLATFORM_TEXT_DOMAIN); ?></th>
                        <th><?php _e('P&L', FLAVOR_PLATFORM_TEXT_DOMAIN); ?></th>
                    </tr>
                </thead>
                <tbody>
                    <?php if (!empty($portfolio['portfolio']['posiciones'])): ?>
                    ...
                    <?php else: ?>
                        <tr>
                            <td colspan="6" class="sin-datos"><?php _e('No tienes posiciones abiertas', FLAVOR_PLATFORM_TEXT_DOMAIN); ?></td>
                        </tr>
                    <?php endif; ?>
                </tbody>
            </table>
    ...
    public function shortcode_mercado($atts) {
        ...
        <div class="trading-ia-mercado">
            <h3><?php _e('Precios de Mercado', FLAVOR_PLATFORM_TEXT_DOMAIN); ?></h3>
                    ...
                    <?php foreach ($mercado['mercado'] as $token => $datos): ?>
                        <div class="token-card">
                            ...
                            <div class="token-volumen">
                                <?php _e('Vol:', FLAVOR_PLATFORM_TEXT_DOMAIN); ?> $<?php echo $this->formatear_numero_grande($datos['volumen_24h'] ?? 0); ?>
                            </div>
                        </div>
                    <?php endforeach; ?>
                <?php else: ?>
                    <p><?php _e('No se pudieron cargar los datos del mercado', FLAVOR_PLATFORM_TEXT_DOMAIN); ?></p>
                <?php endif; ?>
            </div>
        </div>
        <?php
    ...
    public function shortcode_historial($atts) {
        ...
        if (!is_user_logged_in()) {
            return '<p class="trading-ia-login-required">' . __('Inicia sesion para ver tu historial.', FLAVOR_PLATFORM_TEXT_DOMAIN) . '</p>';
        }
        ...
        <div class="trading-ia-historial">
            <h3><?php _e('Historial de Operaciones', FLAVOR_PLATFORM_TEXT_DOMAIN); ?></h3>
            <table class="historial-tabla">
                ...
                <tbody>
                        ...
                        <?php foreach ($historial['trades'] as $trade): ?>
                            <tr class="trade-<?php echo esc_attr($trade['tipo']); ?>">
                                ...
                                <td class="tipo-<?php echo esc_attr($trade['tipo']); ?>">
                                    <?php echo $trade['tipo'] === 'compra' ? __('Compra', FLAVOR_PLATFORM_TEXT_DOMAIN) : __('Venta', FLAVOR_PLATFORM_TEXT_DOMAIN); ?>
                                </td>
                    ...
                    <?php else: ?>
                        <tr>
                            <td colspan="6" class="sin-datos"><?php _e('No hay operaciones registradas', FLAVOR_PLATFORM_TEXT_DOMAIN); ?></td>
                        </tr>
                    <?php endif; ?>
                </tbody>
            </table>
    ...
    public function shortcode_panel_control($atts) {
        if (!current_user_can('manage_options')) {
            return '<p class="trading-ia-no-permisos">' . __('No tienes permisos para acceder al panel de control.', FLAVOR_PLATFORM_TEXT_DOMAIN) . '</p>';
        }
        ...
        <div class="trading-ia-panel-control">
            <h3><?php _e('Panel de Control - Trading IA', FLAVOR_PLATFORM_TEXT_DOMAIN); ?></h3>

            <div class="panel-seccion bot-control">
                <h4><?php _e('Control del Bot', FLAVOR_PLATFORM_TEXT_DOMAIN); ?></h4>
                <div class="bot-status">
                    <span class="status-indicator <?php echo $estado['estado']['bot_activo'] ? 'activo' : 'inactivo'; ?>"></span>
                    <span><?php echo $estado['estado']['bot_activo'] ? __('Bot Activo', FLAVOR_PLATFORM_TEXT_DOMAIN) : __('Bot Inactivo', FLAVOR_PLATFORM_TEXT_DOMAIN); ?></span>
                </div>
                <div class="bot-actions">
                    <button class="btn-iniciar-bot" <?php echo $estado['estado']['bot_activo'] ? 'disabled' : ''; ?>>
                        <?php _e('Iniciar Bot', FLAVOR_PLATFORM_TEXT_DOMAIN); ?>
                    </button>
                    <button class="btn-detener-bot" <?php echo !$estado['estado']['bot_activo'] ? 'disabled' : ''; ?>>
                        <?php _e('Detener Bot', FLAVOR_PLATFORM_TEXT_DOMAIN); ?>
                    </button>
                    <button class="btn-reset" onclick="return confirm('<?php _e('Estas seguro? Se borrara todo el historial.', FLAVOR_PLATFORM_TEXT_DOMAIN); ?>')">
                        <?php _e('Reset Simulacion', FLAVOR_PLATFORM_TEXT_DOMAIN); ?>
                    </button>
                </div>
            </div>

            <div class="panel-seccion parametros">
                <h4><?php _e('Parametros de Trading', FLAVOR_PLATFORM_TEXT_DOMAIN); ?></h4>
                <form class="parametros-form" id="trading-ia-parametros-form">
                    <div class="form-row">
                        <label><?php _e('Agresividad (1-10)', FLAVOR_PLATFORM_TEXT_DOMAIN); ?></label>
                    ...
                    <div class="form-row">
                        <label><?php _e('Intervalo analisis (seg)', FLAVOR_PLATFORM_TEXT_DOMAIN); ?></label>
                        <input type="number" name="intervalo_analisis" min="30" max="300" value="<?php echo esc_attr($estado['estado']['intervalo_analisis']); ?>">
                    </div>
                    <div class="form-row">
                        <label><?php _e('Confianza minima (%)', FLAVOR_PLATFORM_TEXT_DOMAIN); ?></label>
                        <input type="number" name="confianza_minima_trade" min="30" max="90" value="<?php echo esc_attr($estado['estado']['confianza_minima']); ?>">
                    </div>
                    <div class="form-row">
                        <label><?php _e('Auto-ajuste', FLAVOR_PLATFORM_TEXT_DOMAIN); ?></label>
                        <input type="checkbox" name="auto_ajuste_enabled" <?php checked($estado['estado']['auto_ajuste']['habilitado'] ?? false); ?>>
                    </div>
                    <button type="submit" class="btn-guardar-parametros"><?php _e('Guardar Parametros', FLAVOR_PLATFORM_TEXT_DOMAIN); ?></button>
                </form>
            </div>

            <div class="panel-seccion tokens">
                <h4><?php _e('Tokens Monitoreados', FLAVOR_PLATFORM_TEXT_DOMAIN); ?></h4>
                <div class="tokens-lista">
                    <?php foreach ($estado['estado']['tokens_monitoreados'] as $token): ?>
                        <span class="token-badge">
                            <?php echo esc_html($token); ?>
                            <button class="btn-eliminar-token" data-token="<?php echo esc_attr($token); ?>"><?php echo esc_html__('&times;', FLAVOR_PLATFORM_TEXT_DOMAIN); ?></button>
                        </span>
                    <?php endforeach; ?>
                </div>
                <div class="agregar-token">
                    <input type="text" id="nuevo-token" placeholder="<?php _e('Ej: PYTH', FLAVOR_PLATFORM_TEXT_DOMAIN); ?>">
                    <button class="btn-agregar-token"><?php _e('Agregar', FLAVOR_PLATFORM_TEXT_DOMAIN); ?></button>
                </div>
            </div>
        </div>
        <?php
    ...
    private function enqueue_frontend_assets() {
        ...
        wp_localize_script('trading-ia-frontend', 'tradingIAData', array(
            ...
            'i18n'     => array(
                ...
                'operacion_exitosa' => __('Operacion realizada con exito', FLAVOR_PLATFORM_TEXT_DOMAIN),
                'error_operacion'   => __('Error en la operacion', FLAVOR_PLATFORM_TEXT_DOMAIN),
            ),
        ));
    ...
    public function generar_reporte_diario() {
        ...
        if ($resumen_diario && $resumen_diario->total_trades > 0) {
            ...
            do_action('flavor_notificacion_enviar', array(
                ...
                'titulo'    => __('Reporte Diario Trading IA', FLAVOR_PLATFORM_TEXT_DOMAIN),
    ...
    private function disparar_alerta_precio($alerta, $precio_actual) {
        $tipo_texto = $alerta->tipo === 'above' ? __('supero', FLAVOR_PLATFORM_TEXT_DOMAIN) : __('bajo de', FLAVOR_PLATFORM_TEXT_DOMAIN);

        $mensaje_alerta = sprintf(
            __('Alerta de precio: %s %s $%s (objetivo: $%s)', FLAVOR_PLATFORM_TEXT_DOMAIN),
            $alerta->token,
            $tipo_texto,
        ...
        do_action('flavor_notificacion_enviar', array(
            ...
            'titulo'     => sprintf(__('Alerta: %s', FLAVOR_PLATFORM_TEXT_DOMAIN), $alerta->token),
    ...
    private function integrar_gamificacion_compra($resultado) {
        ...
        do_action('flavor_gamificacion_agregar_puntos', array(
            ...
            'descripcion' => sprintf(
                __('Compra de %s por $%s', FLAVOR_PLATFORM_TEXT_DOMAIN),
    ...
    private function integrar_gamificacion_venta($resultado) {
        ...
        do_action('flavor_gamificacion_agregar_puntos', array(
            ...
            'razon'       => 'trading_venta',
            'descripcion' => sprintf(
                __('Venta de %s - P&L: $%s', FLAVOR_PLATFORM_TEXT_DOMAIN),
                $resultado['resultado']['token'] ?? '',
                number_format($pnl, 2)
            ),
        ));
    ...
    private function verificar_logros_trading($usuario_id) {
        ...
        if ($estadisticas->total_trades === 1) {
            do_action('flavor_gamificacion_desbloquear_logro', array(
                ...
                'nombre'     => __('Primer Paso', FLAVOR_PLATFORM_TEXT_DOMAIN),
                'descripcion' => __('Realizaste tu primer trade', FLAVOR_PLATFORM_TEXT_DOMAIN),
            ));
        }
        ...
        if ($estadisticas->total_trades >= 10) {
            do_action('flavor_gamificacion_desbloquear_logro', array(
                ...
                'nombre'     => __('Trader Activo', FLAVOR_PLATFORM_TEXT_DOMAIN),
                'descripcion' => __('Realizaste 10 trades', FLAVOR_PLATFORM_TEXT_DOMAIN),
            ));
        }
        ...
        if ($estadisticas->trades_ganadores >= 1) {
            do_action('flavor_gamificacion_desbloquear_logro', array(
                ...
                'nombre'     => __('En Verde', FLAVOR_PLATFORM_TEXT_DOMAIN),
                'descripcion' => __('Cerraste tu primer trade con ganancias', FLAVOR_PLATFORM_TEXT_DOMAIN),
            ));
        }
        ...
        if ($estadisticas->pnl_total >= 100) {
            do_action('flavor_gamificacion_desbloquear_logro', array(
                ...
                'nombre'     => __('Centenario', FLAVOR_PLATFORM_TEXT_DOMAIN),
                'descripcion' => __('Acumulaste $100 en ganancias', FLAVOR_PLATFORM_TEXT_DOMAIN),
            ));
        }
    }
    ...
    private function enviar_notificacion_trade($tipo, $resultado) {
        ...
        $titulo = $tipo === 'compra'
            ? sprintf(__('Compra ejecutada: %s', FLAVOR_PLATFORM_TEXT_DOMAIN), $datos_trade['token'] ?? '')
            : sprintf(__('Venta ejecutada: %s', FLAVOR_PLATFORM_TEXT_DOMAIN), $datos_trade['token'] ?? '');

        $mensaje = $tipo === 'compra'
            ? sprintf(
                __('Compraste %s %s por $%s', FLAVOR_PLATFORM_TEXT_DOMAIN),
                number_format($datos_trade['cantidad'] ?? 0, 6),
                $datos_trade['token'] ?? '',
                number_format($datos_trade['total_usd'] ?? 0, 2)
            )
            : sprintf(
                __('Vendiste %s %s por $%s (P&L: $%s)', FLAVOR_PLATFORM_TEXT_DOMAIN),
                number_format($datos_trade['cantidad'] ?? 0, 6),
                $datos_trade['token'] ?? '',
                number_format($datos_trade['total_usd'] ?? 0, 2),
    ...
    private function crear_alerta_precio($token, $tipo, $precio) {
        ...
        if (!$usuario_id) {
            return array('success' => false, 'error' => __('Usuario no autenticado', FLAVOR_PLATFORM_TEXT_DOMAIN));
        }
        ...
        if (empty($token) || $precio <= 0) {
            return array('success' => false, 'error' => __('Datos invalidos', FLAVOR_PLATFORM_TEXT_DOMAIN));
        }
        ...
        if ($resultado === false) {
            return array('success' => false, 'error' => __('Error al crear la alerta', FLAVOR_PLATFORM_TEXT_DOMAIN));
        }

        return array(
            'success'  => true,
            'mensaje'  => sprintf(
                __('Alerta creada: %s %s $%s', FLAVOR_PLATFORM_TEXT_DOMAIN),
                $token,
                $tipo === 'above' ? __('supere', FLAVOR_PLATFORM_TEXT_DOMAIN) : __('baje de', FLAVOR_PLATFORM_TEXT_DOMAIN),
...
KNOWLEDGE;
    ...
    public function get_web_components() {
        return array(
            'hero' => array(
                ...
                'description' => __('Seccion hero de Trading IA con estadisticas de senales, precision y mercados', FLAVOR_PLATFORM_TEXT_DOMAIN),
                'category'    => 'hero',
                'icon'        => 'dashicons-chart-area',
                'fields'      => array(
                    'titulo_hero'           => array('type' => 'text', 'default' => __('Trading con IA', FLAVOR_PLATFORM_TEXT_DOMAIN)),
                    'subtitulo_hero'        => array('type' => 'textarea', 'default' => __('Analisis predictivo y senales de trading impulsados por inteligencia artificial', FLAVOR_PLATFORM_TEXT_DOMAIN)),
            ...
            'features' => array(
                'label'       => __('Features Trading IA', FLAVOR_PLATFORM_TEXT_DOMAIN),
                'description' => __('Grid de herramientas de trading inteligente: analisis tecnico, senales, backtesting, gestion de riesgo', FLAVOR_PLATFORM_TEXT_DOMAIN),
                'category'    => 'features',
                'icon'        => 'dashicons-grid-view',
                'fields'      => array(
                    'titulo_features'          => array('type' => 'text', 'default' => __('Herramientas de Trading Inteligente', FLAVOR_PLATFORM_TEXT_DOMAIN)),
                    'funcionalidades_trading'  => array('type' => 'repeater', 'default' => array()),
                ),
                'template'    => 'trading-ia/features',
            ),
            'stats' => array(
                'label'       => __('Panel de Rendimiento Trading IA', FLAVOR_PLATFORM_TEXT_DOMAIN),
                'description' => __('Dashboard de estadisticas con KPIs y grafico de rendimiento mensual', FLAVOR_PLATFORM_TEXT_DOMAIN),
                'category'    => 'stats',
                'icon'        => 'dashicons-chart-bar',
                'fields'      => array(
                    'titulo_stats'            => array('type' => 'text', 'default' => __('Panel de Rendimiento', FLAVOR_PLATFORM_TEXT_DOMAIN)),
    ...
    public function get_form_config($nombre_accion) {
        $configuraciones_formulario = array(
            'ejecutar_compra_manual' => array(
                'title'       => __('Compra Manual de Token', FLAVOR_PLATFORM_TEXT_DOMAIN),
                'description' => __('Ejecuta una compra simulada (paper trading) de un token con USD', FLAVOR_PLATFORM_TEXT_DOMAIN),
                'fields'      => array(
                    'token' => array(
                        'type'     => 'select',
                        'label'    => __('Token a comprar', FLAVOR_PLATFORM_TEXT_DOMAIN),
                        'required' => true,
                        'options'  => array(
                            'SOL'  => 'SOL (Solana)',
                            'BONK' => 'BONK',
                    ...
                    'cantidad_usd' => array(
                        'type'        => 'number',
                        'label'       => __('Cantidad en USD', FLAVOR_PLATFORM_TEXT_DOMAIN),
                        'required'    => true,
                        'min'         => 1,
                        'step'        => '0.01',
                        'placeholder' => __('Cantidad en dolares a invertir', FLAVOR_PLATFORM_TEXT_DOMAIN),
                        'description' => __('Monto en USD que deseas invertir en este token', FLAVOR_PLATFORM_TEXT_DOMAIN),
                    ),
                ),
                'submit_text'     => __('Ejecutar Compra', FLAVOR_PLATFORM_TEXT_DOMAIN),
                'success_message' => __('Compra simulada ejecutada correctamente.', FLAVOR_PLATFORM_TEXT_DOMAIN),
            ),
            'ejecutar_venta_manual' => array(
                'title'       => __('Venta Manual de Token', FLAVOR_PLATFORM_TEXT_DOMAIN),
                'description' => __('Ejecuta una venta simulada (paper trading) de un token', FLAVOR_PLATFORM_TEXT_DOMAIN),
                'fields'      => array(
                    'token' => array(
                        'type'     => 'select',
                        'label'    => __('Token a vender', FLAVOR_PLATFORM_TEXT_DOMAIN),
                        'required' => true,
                        'options'  => array(
                            'SOL'  => 'SOL (Solana)',
                            'BONK' => 'BONK',
                    ...
                    'cantidad' => array(
                        'type'        => 'number',
                        'label'       => __('Cantidad de tokens', FLAVOR_PLATFORM_TEXT_DOMAIN),
                        'min'         => 0,
                        'step'        => '0.000001',
                        'placeholder' => __('Cantidad de tokens a vender', FLAVOR_PLATFORM_TEXT_DOMAIN),
                        'description' => __('Deja vacio para vender toda la posicion', FLAVOR_PLATFORM_TEXT_DOMAIN),
                    ),
                ),
                'submit_text'     => __('Ejecutar Venta', FLAVOR_PLATFORM_TEXT_DOMAIN),
                'success_message' => __('Venta simulada ejecutada correctamente.', FLAVOR_PLATFORM_TEXT_DOMAIN),
            ),
            'crear_regla' => array(
                'title'       => __('Crear Regla de Trading', FLAVOR_PLATFORM_TEXT_DOMAIN),
                'description' => __('Define una regla personalizada que se evalua automaticamente sobre indicadores tecnicos', FLAVOR_PLATFORM_TEXT_DOMAIN),
                'fields'      => array(
                    'nombre' => array(
                        'type'        => 'text',
                        'label'       => __('Nombre de la regla', FLAVOR_PLATFORM_TEXT_DOMAIN),
                        'required'    => true,
                        'placeholder' => __('Ej: Alerta RSI sobrecompra SOL', FLAVOR_PLATFORM_TEXT_DOMAIN),
                    ),
                    'token' => array(
                        'type'     => 'select',
                        'label'    => __('Token', FLAVOR_PLATFORM_TEXT_DOMAIN),
                        'required' => true,
                        'options'  => array(
                            '*'    => __('Todos los tokens', FLAVOR_PLATFORM_TEXT_DOMAIN),
                            'SOL'  => 'SOL',
                            'BONK' => 'BONK',
                            'JUP'  => 'JUP',
                            'WIF'  => 'WIF',
                    ...
                    'indicador' => array(
                        'type'     => 'select',
                        'label'    => __('Indicador', FLAVOR_PLATFORM_TEXT_DOMAIN),
                        'required' => true,
                        'options'  => array(
                            'rsi'   => 'RSI',
                            'macd'  => 'MACD',
                    ...
                    'operador' => array(
                        'type'     => 'select',
                        'label'    => __('Operador', FLAVOR_PLATFORM_TEXT_DOMAIN),
                        'required' => true,
                        'options'  => array(
                            'mayor'  => __('Mayor que (>)', FLAVOR_PLATFORM_TEXT_DOMAIN),
                            'menor'  => __('Menor que (<)', FLAVOR_PLATFORM_TEXT_DOMAIN),
                            'igual'  => __('Igual a (==)', FLAVOR_PLATFORM_TEXT_DOMAIN),
                            'cruce'  => __('Cruce de medias', FLAVOR_PLATFORM_TEXT_DOMAIN),
                        ),
                    ),
                    'valor' => array(
                        'type'        => 'number',
                        'label'       => __('Valor de referencia', FLAVOR_PLATFORM_TEXT_DOMAIN),
                        'required'    => true,
                        'step'        => '0.01',
                        'placeholder' => __('Ej: 70 para RSI sobrecompra', FLAVOR_PLATFORM_TEXT_DOMAIN),
                        'description' => __('Valor numerico con el que se compara el indicador', FLAVOR_PLATFORM_TEXT_DOMAIN),
                    ),
                    'accion_tipo' => array(
                        'type'     => 'select',
                        'label'    => __('Accion a ejecutar', FLAVOR_PLATFORM_TEXT_DOMAIN),
                        'required' => true,
                        'options'  => array(
                            'comprar' => __('Comprar', FLAVOR_PLATFORM_TEXT_DOMAIN),
                            'vender'  => __('Vender', FLAVOR_PLATFORM_TEXT_DOMAIN),
                        ),
                    ),
                ),
                'submit_text'     => __('Crear Regla', FLAVOR_PLATFORM_TEXT_DOMAIN),
                'success_message' => __('Regla de trading creada correctamente. Se evaluara en cada ciclo de analisis.', FLAVOR_PLATFORM_TEXT_DOMAIN),
            ),
        );
        ...
        if (Flavor_Platform_Helpers::tabla_existe($tabla_portfolio)) {
            ...
            if ($portfolio && !$wpdb->last_error) {
                $estadisticas['balance'] = [
                    ...
                    'label' => __('Balance USD', FLAVOR_PLATFORM_TEXT_DOMAIN),
                    'color' => 'green',
                ];
                ...
                if ((int) $portfolio->contador_trades > 0) {
                    $estadisticas['trades'] = [
                        ...
                        'label' => __('Trades', FLAVOR_PLATFORM_TEXT_DOMAIN),
                        'color' => 'blue',
                    ];
                }
            }
    ...
    public function get_pages_definition() {
        return [
            [
                ...
                'slug' => 'trading-ia',
                'content' => '<h1>' . __('Trading IA', FLAVOR_PLATFORM_TEXT_DOMAIN) . '</h1>
<p>' . __('Análisis de mercados con inteligencia artificial', FLAVOR_PLATFORM_TEXT_DOMAIN) . '</p>

[flavor_module_listing module="trading_ia" action="dashboard" columnas="3" limite="12"]',
                'parent' => 0,
            ],
            [
                'title' => __('Señales', FLAVOR_PLATFORM_TEXT_DOMAIN),
                'slug' => 'senales-trading',
                'content' => '<h1>' . __('Señales de Trading', FLAVOR_PLATFORM_TEXT_DOMAIN) . '</h1>
<p>' . __('Consulta las señales generadas por IA', FLAVOR_PLATFORM_TEXT_DOMAIN) . '</p>

[flavor_module_listing module="trading_ia" action="senales"]',
                'parent' => 'trading-ia',
            ],
            [
                'title' => __('Estrategias', FLAVOR_PLATFORM_TEXT_DOMAIN),
                'slug' => 'estrategias-trading',
                'content' => '<h1>' . __('Estrategias de Trading', FLAVOR_PLATFORM_TEXT_DOMAIN) . '</h1>
<p>' . __('Configura tus estrategias automatizadas', FLAVOR_PLATFORM_TEXT_DOMAIN) . '</p>
...
[flavor_module_dashboard module="trading_ia" action="estrategias"]',
            ...
            [
                ...
                'content' => '<h1>' . __('Historial de Operaciones', FLAVOR_PLATFORM_TEXT_DOMAIN) . '</h1>
<p>' . __('Revisa tu historial de operaciones', FLAVOR_PLATFORM_TEXT_DOMAIN) . '</p>
