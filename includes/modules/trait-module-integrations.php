        ...
        <div class="flavor-integration-box" data-type="<?php echo esc_attr($content_type['id']); ?>">
            ...
            <div class="flavor-integration-items" style="max-height: 200px; overflow-y: auto; border: 1px solid #ddd; padding: 10px; margin: 10px 0; border-radius: 4px;">
                ...
                <?php else: ?>
                    <p class="flavor-no-items" style="color: #666; font-style: italic; margin: 0;">
                        <?php printf(__('Sin %s vinculados', FLAVOR_PLATFORM_TEXT_DOMAIN), strtolower($content_type['label'])); ?>
                    </p>
                <?php endif; ?>
            </div>
            ...
            <div style="display: flex; gap: 5px;">
                <select class="flavor-integration-selector widefat" style="flex: 1;">
                    <option value=""><?php printf(__('Seleccionar %s...', FLAVOR_PLATFORM_TEXT_DOMAIN), strtolower($content_type['label'])); ?></option>
            ...
            <p style="margin-top: 10px;">
                <a href="<?php echo admin_url('post-new.php?post_type=' . $content_type['post_type']); ?>" class="button button-small" target="_blank">
                    ...
                    <?php printf(__('Crear %s', FLAVOR_PLATFORM_TEXT_DOMAIN), $content_type['label']); ?>
                </a>
            </p>
            <?php endif; ?>
        </div>
...
trait Flavor_Module_Integration_Consumer {
    ...
    protected function register_integration_metaboxes($accepted, $targets) {
        ...
        foreach ($targets as $target) {
            ...
            add_meta_box(
                ...
                __('Contenido Relacionado', FLAVOR_PLATFORM_TEXT_DOMAIN),
    ...
    protected function render_integrations_metabox($post, $providers) {
        ...
        if (empty($providers)) {
            echo '<p>' . __('No hay modulos de contenido activos para vincular.', FLAVOR_PLATFORM_TEXT_DOMAIN) . '</p>';
            return;
        }
...
class Flavor_Functional_Integrations {
    ...
    public function register_integrations() {
        ...
        $this->integraciones['eventos_gc_catering'] = [
            ...
            'label'       => __('Pedido de Catering', FLAVOR_PLATFORM_TEXT_DOMAIN),
            'descripcion' => __('Crear pedido grupal de productos para el catering del evento', FLAVOR_PLATFORM_TEXT_DOMAIN),
        ...
        $this->integraciones['cursos_banco_tiempo'] = [
            ...
            'label'       => __('Pago con Horas', FLAVOR_PLATFORM_TEXT_DOMAIN),
            'descripcion' => __('Permitir pagar inscripción al curso con horas del banco de tiempo', FLAVOR_PLATFORM_TEXT_DOMAIN),
        ...
        $this->integraciones['carpooling_eventos'] = [
            ...
            'origen'      => 'carpooling',
            'destino'     => 'eventos',
            'label'       => __('Transporte Compartido', FLAVOR_PLATFORM_TEXT_DOMAIN),
            'descripcion' => __('Ofrecer/buscar transporte compartido para asistir al evento', FLAVOR_PLATFORM_TEXT_DOMAIN),
            'icon'        => 'dashicons-car',
        ...
        $this->integraciones['incidencias_huertos'] = [
            ...
            'label'       => __('Incidencia en Parcela', FLAVOR_PLATFORM_TEXT_DOMAIN),
            'descripcion' => __('Reportar un problema específico en una parcela del huerto', FLAVOR_PLATFORM_TEXT_DOMAIN),
        ...
        $this->integraciones['talleres_comunidades'] = [
            ...
            'origen'      => 'talleres',
            'destino'     => 'comunidades',
            'label'       => __('Taller para Comunidad', FLAVOR_PLATFORM_TEXT_DOMAIN),
            'descripcion' => __('Restringir taller solo a miembros de una comunidad específica', FLAVOR_PLATFORM_TEXT_DOMAIN),
        ...
        $this->integraciones['recetas_gc_ingredientes'] = [
            ...
            'origen'      => 'recetas',
            'destino'     => 'grupos_consumo',
            'label'       => __('Ingredientes Disponibles', FLAVOR_PLATFORM_TEXT_DOMAIN),
            'descripcion' => __('Vincular receta con productos disponibles en grupos de consumo', FLAVOR_PLATFORM_TEXT_DOMAIN),
    ...
    public function callback_catering($datos, $user_id) {
        ...
        if (!$evento_id || !$grupo_id || empty($productos)) {
            return new \WP_Error('datos_incompletos', __('Datos incompletos', FLAVOR_PLATFORM_TEXT_DOMAIN));
        }
    ...
    public function callback_carpooling_evento($datos, $user_id) {
        ...
        if (!$evento) {
            return new \WP_Error('evento_invalido', __('Evento no válido', FLAVOR_PLATFORM_TEXT_DOMAIN));
        }
    ...
    public function ajax_ejecutar_integracion() {
        ...
        if (!isset($this->integraciones[$integracion_id])) {
            wp_send_json_error(['mensaje' => __('Integración no válida', FLAVOR_PLATFORM_TEXT_DOMAIN)]);
        }
        ...
        if (!flavor_is_module_active($integracion['origen']) ||
            ...
            wp_send_json_error(['mensaje' => __('Módulos requeridos no activos', FLAVOR_PLATFORM_TEXT_DOMAIN)]);
        }
