...
part 'huertos_urbanos_screen_parts.dart';
...
class _HuertosUrbanosScreenState extends ConsumerState<HuertosUrbanosScreen> {
    ...
    return Scaffold(
      ...
      body: FutureBuilder<ApiResponse<Map<String, dynamic>>>(
        ...
        builder: (context, snapshot) {
          ...
          final hasContent = misParcelas.isNotEmpty ||
              tareasPendientes.isNotEmpty ||
              proximosCultivos.isNotEmpty ||
              intercambios.isNotEmpty ||
              parcelasDisponibles > 0;

          if (!hasContent) {
            return FlavorEmptyState(
              icon: Icons.grass_outlined,
              title: i18n.huertosError,
            );
          }
  ...
  Future<void> _solicitarParcela(BuildContext context) async {
    ...
    if (result == true && context.mounted) {
      await FlavorMutation.runApiResponse(
        context,
        request: () => api.solicitarParcelaHuerto(
          tamanio: tamanioPreferido,
          observaciones: observacionesController.text.trim(),
        ),
        successMessage: i18n.huertosRequestSuccess,
        fallbackErrorMessage: i18n.huertosRequestError,
        onSuccess: _refresh,
      );
    }
  ...
  Future<void> _completarTarea(BuildContext context, int tareaId) async {
    ...
    await FlavorMutation.runApiResponse(
      context,
      request: () => api.completarTareaHuerto(tareaId),
      successMessage: i18n.huertosTaskCompleted,
      fallbackErrorMessage: i18n.huertosTaskError,
      onSuccess: _refresh,
    );
  }
  ...
  Future<void> _contactarIntercambio(BuildContext context, int intercambioId) async {
    ...
    if (result == true && context.mounted) {
      await FlavorMutation.runApiResponse(
        context,
        request: () => api.contactarIntercambioHuerto(
          intercambioId: intercambioId,
          mensaje: mensajeController.text.trim(),
        ),
        successMessage: i18n.huertosMessageSent,
        fallbackErrorMessage: i18n.huertosMessageError,
      );
    }
